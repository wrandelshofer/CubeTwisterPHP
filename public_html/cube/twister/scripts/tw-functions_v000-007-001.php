<?php 
  ...
  * 
  * SSE to TWIZZLE:
  * - 2xCube: Numbered layer twists: N1R, N2R added.
  * - 3xCube: Numbered layer twists: N1R, N2R, N3R, T2R added.
  * - 4xCube: Numbered layer twists: N1R, N2R, N4R, T2R added.
  * - 5xCube: Numbered layer twists: N1R, N2R, N5R, T2R added.
  * - 6xCube: Numbered layer twists: N1R, N2R, N6R, T2R added.
  * - 7xCube: Numbered layer twists: N1R, N2R, N7R, T2R added.
  * 
  * - 3xPyraminx: Numbered layer twists: N1R, N2R, T2R added.      To do: N3R
  * - 4xPyraminx: Numbered layer twists: N1R, N2R, T2R added.      To do: N4R
  * - 5xPyraminx: Numbered layer twists: N1R, N2R, T2R added.      To do: N5R
  * 
  * - 3xMegaminx: Numbered layer twists: N1R, N2R, N3R, T2R added.
  * - 5xMegaminx: Numbered layer twists: N1R, N2R, N5R, T2R added.
  * 
  * TWIZZLE to SSE:
  * - 2xCube: Numbered layer twists: 1R, 2R added.                 (currently not supported tokens by Twisty Player and Twizzle Editor, but by Twizzle Explorer!)
  * - 3xCube: Numbered layer twists: 1-2R, 2-3R, 1R, 2R, 3R added. (currently not supported tokens by Twisty Player and Twizzle Editor, but by Twizzle Explorer!)
  * - 4xCube: Numbered layer twists: 2-4R, 3-4R added.
  * - 5xCube: Numbered layer twists: 2-5R, 3-5R, 4-5R added.
  * - 6xCube: Numbered layer twists: 2-6R, 3-6R, 4-6R, 5-6R added.
  * - 6xCube: Numbered layer twists: 2-7R, 3-7R, 4-7R, 5-7R, 6-7R added.
  * 
  * - 3xMegaminx: Numbered layer twists: 2-3R added.               To do: Old Twizzle
  * - 5xMegaminx: Numbered layer twists: 2-5R, 3-5R, 4-5R added.   To do: Old Twizzle
  * 
  * BTM Move Count SSE:
  * - General Move Count for all sorts of Twists and Cube Rotations added.
  * - Support for // (Annotations) added.
  * - Bux Fix: Carriage return "\r\n" caused wrong move count (replaced "\r\n" with "\n").
  * 
  * - Optimize SSE ($optSSE):
  *   Non-slice-twists: Fixes uncorrect translations:
  *     3x Cube: R' L' -> SR'' (Missing Non-slice-twists fix!)
  *   Non-slice-twists: Fix added for 3x to 5x Cubes.
  *   Non-slice-twists: To do for 6x to 7x Cubes, Megaminx, Gigaminx!
  * 
  * 
  * To do:
  * - Complete code revision (3x to 5x Pyraminx).
  * - Alternative move tokens (3x to 5x Pyraminx).
  * - Asymmetric Slice twists (6x to 7x Cubes, Megaminx, Gigaminx).
  ***************************************************************************************************
  */
  ...
  */
  function getPar($parID) {
    $pPost = array_key_exists($parID,$_POST) ? $_POST[$parID] : "";
    $pGet  = array_key_exists($parID,$_GET) ? $_GET[$parID] : "";
  ...
  */
  function alg2xC_SseToTwizzle($alg) {
    ...
    /* ··································································································· */
    /* --- 2xC: SSE -> CODE: [1] Numbered layer twists --- */
    /* N1 | N, N2 (Face twists) */
    $alg = preg_replace("/N1R'/","<901>",$alg); $alg = preg_replace("/N1R-/","<901>",$alg);   $alg = preg_replace("/N1R2/","<902>",$alg);   $alg = preg_replace("/N1R/","<903>",$alg);
    $alg = preg_replace("/N1L'/","<904>",$alg); $alg = preg_replace("/N1L-/","<904>",$alg);   $alg = preg_replace("/N1L2/","<905>",$alg);   $alg = preg_replace("/N1L/","<906>",$alg);
    $alg = preg_replace("/N1F'/","<907>",$alg); $alg = preg_replace("/N1F-/","<907>",$alg);   $alg = preg_replace("/N1F2/","<908>",$alg);   $alg = preg_replace("/N1F/","<909>",$alg);
    $alg = preg_replace("/N1B'/","<910>",$alg); $alg = preg_replace("/N1B-/","<910>",$alg);   $alg = preg_replace("/N1B2/","<911>",$alg);   $alg = preg_replace("/N1B/","<912>",$alg);
    $alg = preg_replace("/N1U'/","<913>",$alg); $alg = preg_replace("/N1U-/","<913>",$alg);   $alg = preg_replace("/N1U2/","<914>",$alg);   $alg = preg_replace("/N1U/","<915>",$alg);
    $alg = preg_replace("/N1D'/","<916>",$alg); $alg = preg_replace("/N1D-/","<916>",$alg);   $alg = preg_replace("/N1D2/","<917>",$alg);   $alg = preg_replace("/N1D/","<918>",$alg);
    
    $alg = preg_replace("/NR'/","<906>",$alg); $alg = preg_replace("/NR-/","<906>",$alg);   $alg = preg_replace("/NR2/","<905>",$alg);   $alg = preg_replace("/NR/","<904>",$alg);
    $alg = preg_replace("/NL'/","<903>",$alg); $alg = preg_replace("/NL-/","<903>",$alg);   $alg = preg_replace("/NL2/","<902>",$alg);   $alg = preg_replace("/NL/","<901>",$alg);
    $alg = preg_replace("/NF'/","<912>",$alg); $alg = preg_replace("/NF-/","<912>",$alg);   $alg = preg_replace("/NF2/","<911>",$alg);   $alg = preg_replace("/NF/","<910>",$alg);
    $alg = preg_replace("/NB'/","<909>",$alg); $alg = preg_replace("/NB-/","<909>",$alg);   $alg = preg_replace("/NB2/","<908>",$alg);   $alg = preg_replace("/NB/","<907>",$alg);
    $alg = preg_replace("/NU'/","<918>",$alg); $alg = preg_replace("/NU-/","<918>",$alg);   $alg = preg_replace("/NU2/","<917>",$alg);   $alg = preg_replace("/NU/","<916>",$alg);
    $alg = preg_replace("/ND'/","<915>",$alg); $alg = preg_replace("/ND-/","<915>",$alg);   $alg = preg_replace("/ND2/","<914>",$alg);   $alg = preg_replace("/ND/","<913>",$alg);
    
    $alg = preg_replace("/N2R'/","<906>",$alg); $alg = preg_replace("/N2R-/","<906>",$alg);   $alg = preg_replace("/N2R2/","<905>",$alg);   $alg = preg_replace("/N2R/","<904>",$alg);
    $alg = preg_replace("/N2L'/","<903>",$alg); $alg = preg_replace("/N2L-/","<903>",$alg);   $alg = preg_replace("/N2L2/","<902>",$alg);   $alg = preg_replace("/N2L/","<901>",$alg);
    $alg = preg_replace("/N2F'/","<912>",$alg); $alg = preg_replace("/N2F-/","<912>",$alg);   $alg = preg_replace("/N2F2/","<911>",$alg);   $alg = preg_replace("/N2F/","<910>",$alg);
    $alg = preg_replace("/N2B'/","<909>",$alg); $alg = preg_replace("/N2B-/","<909>",$alg);   $alg = preg_replace("/N2B2/","<908>",$alg);   $alg = preg_replace("/N2B/","<907>",$alg);
    $alg = preg_replace("/N2U'/","<918>",$alg); $alg = preg_replace("/N2U-/","<918>",$alg);   $alg = preg_replace("/N2U2/","<917>",$alg);   $alg = preg_replace("/N2U/","<916>",$alg);
    $alg = preg_replace("/N2D'/","<915>",$alg); $alg = preg_replace("/N2D-/","<915>",$alg);   $alg = preg_replace("/N2D2/","<914>",$alg);   $alg = preg_replace("/N2D/","<913>",$alg);
  ...
  */
  function alg2xC_TwizzleToSse($alg) {
    ...
    $alg = preg_replace("/2R'/","<906>",$alg); $alg = preg_replace("/2R2/","<905>",$alg); $alg = preg_replace("/2R/","<904>",$alg);
    $alg = preg_replace("/2L'/","<903>",$alg); $alg = preg_replace("/2L2/","<902>",$alg); $alg = preg_replace("/2L/","<901>",$alg);
    $alg = preg_replace("/2F'/","<912>",$alg); $alg = preg_replace("/2F2/","<911>",$alg); $alg = preg_replace("/2F/","<910>",$alg);
    $alg = preg_replace("/2B'/","<909>",$alg); $alg = preg_replace("/2B2/","<908>",$alg); $alg = preg_replace("/2B/","<907>",$alg);
    $alg = preg_replace("/2U'/","<918>",$alg); $alg = preg_replace("/2U2/","<917>",$alg); $alg = preg_replace("/2U/","<916>",$alg);
    $alg = preg_replace("/2D'/","<915>",$alg); $alg = preg_replace("/2D2/","<914>",$alg); $alg = preg_replace("/2D/","<913>",$alg);
  ...
  */
  function alg3xC_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2R'/","<101>",$alg); $alg = preg_replace("/N2R-/","<101>",$alg);   $alg = preg_replace("/N2R2/","<102>",$alg);   $alg = preg_replace("/N2R/","<103>",$alg);
    $alg = preg_replace("/N2L'/","<103>",$alg); $alg = preg_replace("/N2L-/","<103>",$alg);   $alg = preg_replace("/N2L2/","<102>",$alg);   $alg = preg_replace("/N2L/","<101>",$alg);
    $alg = preg_replace("/N2F'/","<104>",$alg); $alg = preg_replace("/N2F-/","<104>",$alg);   $alg = preg_replace("/N2F2/","<105>",$alg);   $alg = preg_replace("/N2F/","<106>",$alg);
    $alg = preg_replace("/N2B'/","<106>",$alg); $alg = preg_replace("/N2B-/","<106>",$alg);   $alg = preg_replace("/N2B2/","<105>",$alg);   $alg = preg_replace("/N2B/","<104>",$alg);
    $alg = preg_replace("/N2U'/","<107>",$alg); $alg = preg_replace("/N2U-/","<107>",$alg);   $alg = preg_replace("/N2U2/","<108>",$alg);   $alg = preg_replace("/N2U/","<109>",$alg);
    $alg = preg_replace("/N2D'/","<109>",$alg); $alg = preg_replace("/N2D-/","<109>",$alg);   $alg = preg_replace("/N2D2/","<108>",$alg);   $alg = preg_replace("/N2D/","<107>",$alg);
    ...
    $alg = preg_replace("/N3R'/","<906>",$alg); $alg = preg_replace("/N3R-/","<906>",$alg);   $alg = preg_replace("/N3R2/","<905>",$alg);   $alg = preg_replace("/N3R/","<904>",$alg);
    $alg = preg_replace("/N3L'/","<903>",$alg); $alg = preg_replace("/N3L-/","<903>",$alg);   $alg = preg_replace("/N3L2/","<902>",$alg);   $alg = preg_replace("/N3L/","<901>",$alg);
    $alg = preg_replace("/N3F'/","<912>",$alg); $alg = preg_replace("/N3F-/","<912>",$alg);   $alg = preg_replace("/N3F2/","<911>",$alg);   $alg = preg_replace("/N3F/","<910>",$alg);
    $alg = preg_replace("/N3B'/","<909>",$alg); $alg = preg_replace("/N3B-/","<909>",$alg);   $alg = preg_replace("/N3B2/","<908>",$alg);   $alg = preg_replace("/N3B/","<907>",$alg);
    $alg = preg_replace("/N3U'/","<918>",$alg); $alg = preg_replace("/N3U-/","<918>",$alg);   $alg = preg_replace("/N3U2/","<917>",$alg);   $alg = preg_replace("/N3U/","<916>",$alg);
    $alg = preg_replace("/N3D'/","<915>",$alg); $alg = preg_replace("/N3D-/","<915>",$alg);   $alg = preg_replace("/N3D2/","<914>",$alg);   $alg = preg_replace("/N3D/","<913>",$alg);
    ...
    $alg = preg_replace("/T2R'/","<301>",$alg); $alg = preg_replace("/T2R-/","<301>",$alg);   $alg = preg_replace("/T2R2/","<302>",$alg);   $alg = preg_replace("/T2R/","<303>",$alg);
    $alg = preg_replace("/T2L'/","<304>",$alg); $alg = preg_replace("/T2L-/","<304>",$alg);   $alg = preg_replace("/T2L2/","<305>",$alg);   $alg = preg_replace("/T2L/","<306>",$alg);
    $alg = preg_replace("/T2F'/","<307>",$alg); $alg = preg_replace("/T2F-/","<307>",$alg);   $alg = preg_replace("/T2F2/","<308>",$alg);   $alg = preg_replace("/T2F/","<309>",$alg);
    $alg = preg_replace("/T2B'/","<310>",$alg); $alg = preg_replace("/T2B-/","<310>",$alg);   $alg = preg_replace("/T2B2/","<311>",$alg);   $alg = preg_replace("/T2B/","<312>",$alg);
    $alg = preg_replace("/T2U'/","<313>",$alg); $alg = preg_replace("/T2U-/","<313>",$alg);   $alg = preg_replace("/T2U2/","<314>",$alg);   $alg = preg_replace("/T2U/","<315>",$alg);
    $alg = preg_replace("/T2D'/","<316>",$alg); $alg = preg_replace("/T2D-/","<316>",$alg);   $alg = preg_replace("/T2D2/","<317>",$alg);   $alg = preg_replace("/T2D/","<318>",$alg);
  ...
  */
  function alg3xC_TwizzleToSse($alg) {
    ...
    /* ··································································································· */
    /* --- 3xC: TWIZZLE -> CODE: [3] Tier twists (TWIZZLE) --- */
    /* T */
    $alg = preg_replace("/1-2R'/","<301>",$alg); $alg = preg_replace("/1-2R2/","<302>",$alg); $alg = preg_replace("/1-2R/","<303>",$alg);
    $alg = preg_replace("/1-2L'/","<304>",$alg); $alg = preg_replace("/1-2L2/","<305>",$alg); $alg = preg_replace("/1-2L/","<306>",$alg);
    $alg = preg_replace("/1-2F'/","<307>",$alg); $alg = preg_replace("/1-2F2/","<308>",$alg); $alg = preg_replace("/1-2F/","<309>",$alg);
    $alg = preg_replace("/1-2B'/","<310>",$alg); $alg = preg_replace("/1-2B2/","<311>",$alg); $alg = preg_replace("/1-2B/","<312>",$alg);
    $alg = preg_replace("/1-2U'/","<313>",$alg); $alg = preg_replace("/1-2U2/","<314>",$alg); $alg = preg_replace("/1-2U/","<315>",$alg);
    $alg = preg_replace("/1-2D'/","<316>",$alg); $alg = preg_replace("/1-2D2/","<317>",$alg); $alg = preg_replace("/1-2D/","<318>",$alg);
    
    $alg = preg_replace("/2-3R'/","<306>",$alg); $alg = preg_replace("/2-3R2/","<305>",$alg); $alg = preg_replace("/2-3R/","<304>",$alg);
    $alg = preg_replace("/2-3L'/","<303>",$alg); $alg = preg_replace("/2-3L2/","<302>",$alg); $alg = preg_replace("/2-3L/","<301>",$alg);
    $alg = preg_replace("/2-3F'/","<312>",$alg); $alg = preg_replace("/2-3F2/","<311>",$alg); $alg = preg_replace("/2-3F/","<310>",$alg);
    $alg = preg_replace("/2-3B'/","<309>",$alg); $alg = preg_replace("/2-3B2/","<308>",$alg); $alg = preg_replace("/2-3B/","<307>",$alg);
    $alg = preg_replace("/2-3U'/","<318>",$alg); $alg = preg_replace("/2-3U2/","<317>",$alg); $alg = preg_replace("/2-3U/","<316>",$alg);
    $alg = preg_replace("/2-3D'/","<315>",$alg); $alg = preg_replace("/2-3D2/","<314>",$alg); $alg = preg_replace("/2-3D/","<313>",$alg);
    
    /* ··································································································· */
    /* --- 3xC: TWIZZLE -> CODE: [2] Slice twists --- */
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/2R' Rv'/","<210>",$alg);
      $alg = preg_replace("/2L' Lv'/","<211>",$alg);
      $alg = preg_replace("/2F' Fv'/","<212>",$alg);
      $alg = preg_replace("/2B' Bv'/","<213>",$alg);
      $alg = preg_replace("/2U' Uv'/","<214>",$alg);
      $alg = preg_replace("/2D' Dv'/","<215>",$alg);
      
      $alg = preg_replace("/2L Rv'/","<210>",$alg);
      $alg = preg_replace("/2R Lv'/","<211>",$alg);
      $alg = preg_replace("/2B Fv'/","<212>",$alg);
      $alg = preg_replace("/2F Bv'/","<213>",$alg);
      $alg = preg_replace("/2D Uv'/","<214>",$alg);
      $alg = preg_replace("/2U Dv'/","<215>",$alg);
      
      /* S = S2-2 */
      $alg = preg_replace("/2R Rv'/","<201>",$alg); $alg = preg_replace("/2R2 Rv2/","<202>",$alg); $alg = preg_replace("/2R' Rv/","<203>",$alg);
      $alg = preg_replace("/2L Lv'/","<203>",$alg); $alg = preg_replace("/2L2 Lv2/","<202>",$alg); $alg = preg_replace("/2L' Lv/","<201>",$alg);
      $alg = preg_replace("/2F Fv'/","<204>",$alg); $alg = preg_replace("/2F2 Fv2/","<205>",$alg); $alg = preg_replace("/2F' Fv/","<206>",$alg);
      $alg = preg_replace("/2B Bv'/","<206>",$alg); $alg = preg_replace("/2B2 Bv2/","<205>",$alg); $alg = preg_replace("/2B' Bv/","<204>",$alg);
      $alg = preg_replace("/2U Uv'/","<207>",$alg); $alg = preg_replace("/2U2 Uv2/","<208>",$alg); $alg = preg_replace("/2U' Uv/","<209>",$alg);
      $alg = preg_replace("/2D Dv'/","<209>",$alg); $alg = preg_replace("/2D2 Dv2/","<208>",$alg); $alg = preg_replace("/2D' Dv/","<207>",$alg);
      
      $alg = preg_replace("/2L' Rv'/","<201>",$alg); $alg = preg_replace("/2L2 Rv2/","<202>",$alg); $alg = preg_replace("/2L Rv/","<203>",$alg);
      $alg = preg_replace("/2R' Lv'/","<203>",$alg); $alg = preg_replace("/2R2 Lv2/","<202>",$alg); $alg = preg_replace("/2R Lv/","<201>",$alg);
      $alg = preg_replace("/2B' Fv'/","<204>",$alg); $alg = preg_replace("/2B2 Fv2/","<205>",$alg); $alg = preg_replace("/2B Fv/","<206>",$alg);
      $alg = preg_replace("/2F' Bv'/","<206>",$alg); $alg = preg_replace("/2F2 Bv2/","<205>",$alg); $alg = preg_replace("/2F Bv/","<204>",$alg);
      $alg = preg_replace("/2D' Uv'/","<207>",$alg); $alg = preg_replace("/2D2 Uv2/","<208>",$alg); $alg = preg_replace("/2D Uv/","<209>",$alg);
      $alg = preg_replace("/2U' Dv'/","<209>",$alg); $alg = preg_replace("/2U2 Dv2/","<208>",$alg); $alg = preg_replace("/2U Dv/","<207>",$alg);
    }
    ...
    $alg = preg_replace("/3R'/","<906>",$alg); $alg = preg_replace("/3R2/","<905>",$alg); $alg = preg_replace("/3R/","<904>",$alg);
    $alg = preg_replace("/3L'/","<903>",$alg); $alg = preg_replace("/3L2/","<902>",$alg); $alg = preg_replace("/3L/","<901>",$alg);
    $alg = preg_replace("/3F'/","<912>",$alg); $alg = preg_replace("/3F2/","<911>",$alg); $alg = preg_replace("/3F/","<910>",$alg);
    $alg = preg_replace("/3B'/","<909>",$alg); $alg = preg_replace("/3B2/","<908>",$alg); $alg = preg_replace("/3B/","<907>",$alg);
    $alg = preg_replace("/3U'/","<918>",$alg); $alg = preg_replace("/3U2/","<917>",$alg); $alg = preg_replace("/3U/","<916>",$alg);
    $alg = preg_replace("/3D'/","<915>",$alg); $alg = preg_replace("/3D2/","<914>",$alg); $alg = preg_replace("/3D/","<913>",$alg);
    
    /* --- 3xC: TWIZZLE -> CODE: [2] Slice twists --- */
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/M Rv'/","<210>",$alg);
      $alg = preg_replace("/M' Lv'/","<211>",$alg);
      $alg = preg_replace("/S' Fv'/","<212>",$alg);
      $alg = preg_replace("/S Bv'/","<213>",$alg);
      $alg = preg_replace("/E Uv'/","<214>",$alg);
      $alg = preg_replace("/E' Dv'/","<215>",$alg);
      
      /* S = S2-2 */
      $alg = preg_replace("/M' Rv'/","<201>",$alg); $alg = preg_replace("/M2 Rv2/","<202>",$alg); $alg = preg_replace("/M Rv/", "<203>",$alg);
      $alg = preg_replace("/M Lv'/", "<203>",$alg); $alg = preg_replace("/M2 Lv2/","<202>",$alg); $alg = preg_replace("/M' Lv/","<201>",$alg);
      $alg = preg_replace("/S Fv'/", "<204>",$alg); $alg = preg_replace("/S2 Fv2/","<205>",$alg); $alg = preg_replace("/S' Fv/","<206>",$alg);
      $alg = preg_replace("/S' Bv'/","<206>",$alg); $alg = preg_replace("/S2 Bv2/","<205>",$alg); $alg = preg_replace("/S Bv/", "<204>",$alg);
      $alg = preg_replace("/E' Uv'/","<207>",$alg); $alg = preg_replace("/E2 Uv2/","<208>",$alg); $alg = preg_replace("/E Uv/", "<209>",$alg);
      $alg = preg_replace("/E Dv'/", "<209>",$alg); $alg = preg_replace("/E2 Dv2/","<208>",$alg); $alg = preg_replace("/E' Dv/","<207>",$alg);
      
      
      /* Non-slice-twists */
      $alg = preg_replace("/R' L'/","<216>",$alg);
      $alg = preg_replace("/L' R'/","<216>",$alg);
      $alg = preg_replace("/F' B'/","<217>",$alg);
      $alg = preg_replace("/B' F'/","<217>",$alg);
      $alg = preg_replace("/U' D'/","<218>",$alg);
      $alg = preg_replace("/D' U'/","<218>",$alg);
      
      /* S = S2-2 */
      $alg = preg_replace("/R L'/","<203>",$alg); $alg = preg_replace("/R2 L2/","<202>",$alg); $alg = preg_replace("/R' L/","<201>",$alg);
      $alg = preg_replace("/L R'/","<201>",$alg); $alg = preg_replace("/L2 R2/","<202>",$alg); $alg = preg_replace("/L' R/","<203>",$alg);
      $alg = preg_replace("/F B'/","<206>",$alg); $alg = preg_replace("/F2 B2/","<205>",$alg); $alg = preg_replace("/F' B/","<204>",$alg);
      $alg = preg_replace("/B F'/","<204>",$alg); $alg = preg_replace("/B2 F2/","<205>",$alg); $alg = preg_replace("/B' F/","<206>",$alg);
      $alg = preg_replace("/U D'/","<209>",$alg); $alg = preg_replace("/U2 D2/","<208>",$alg); $alg = preg_replace("/U' D/","<207>",$alg);
      $alg = preg_replace("/D U'/","<207>",$alg); $alg = preg_replace("/D2 U2/","<208>",$alg); $alg = preg_replace("/D' U/","<209>",$alg);
    }
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/<210>/","MR' CR'",$alg);
      $alg = preg_replace("/<211>/","MR CR",$alg);
      $alg = preg_replace("/<212>/","MF' CF'",$alg);
      $alg = preg_replace("/<213>/","MF CF",$alg);
      $alg = preg_replace("/<214>/","MU' CU'",$alg);
      $alg = preg_replace("/<215>/","MU CU",$alg);
      
      
      $alg = preg_replace("/<216>/","R' L'",$alg);
      $alg = preg_replace("/<217>/","F' B'",$alg);
      $alg = preg_replace("/<218>/","U' D'",$alg);
    }
  ...
  */
  function alg4xC_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2R'/","<101>",$alg); $alg = preg_replace("/N2R-/","<101>",$alg);   $alg = preg_replace("/N2R2/","<102>",$alg);   $alg = preg_replace("/N2R/","<103>",$alg);
    $alg = preg_replace("/N2L'/","<104>",$alg); $alg = preg_replace("/N2L-/","<104>",$alg);   $alg = preg_replace("/N2L2/","<105>",$alg);   $alg = preg_replace("/N2L/","<106>",$alg);
    $alg = preg_replace("/N2F'/","<107>",$alg); $alg = preg_replace("/N2F-/","<107>",$alg);   $alg = preg_replace("/N2F2/","<108>",$alg);   $alg = preg_replace("/N2F/","<109>",$alg);
    $alg = preg_replace("/N2B'/","<110>",$alg); $alg = preg_replace("/N2B-/","<110>",$alg);   $alg = preg_replace("/N2B2/","<111>",$alg);   $alg = preg_replace("/N2B/","<112>",$alg);
    $alg = preg_replace("/N2U'/","<113>",$alg); $alg = preg_replace("/N2U-/","<113>",$alg);   $alg = preg_replace("/N2U2/","<114>",$alg);   $alg = preg_replace("/N2U/","<115>",$alg);
    $alg = preg_replace("/N2D'/","<116>",$alg); $alg = preg_replace("/N2D-/","<116>",$alg);   $alg = preg_replace("/N2D2/","<117>",$alg);   $alg = preg_replace("/N2D/","<118>",$alg);
    ...
    $alg = preg_replace("/N4R'/","<906>",$alg); $alg = preg_replace("/N4R-/","<906>",$alg);   $alg = preg_replace("/N4R2/","<905>",$alg);   $alg = preg_replace("/N4R/","<904>",$alg);
    $alg = preg_replace("/N4L'/","<903>",$alg); $alg = preg_replace("/N4L-/","<903>",$alg);   $alg = preg_replace("/N4L2/","<902>",$alg);   $alg = preg_replace("/N4L/","<901>",$alg);
    $alg = preg_replace("/N4F'/","<912>",$alg); $alg = preg_replace("/N4F-/","<912>",$alg);   $alg = preg_replace("/N4F2/","<911>",$alg);   $alg = preg_replace("/N4F/","<910>",$alg);
    $alg = preg_replace("/N4B'/","<909>",$alg); $alg = preg_replace("/N4B-/","<909>",$alg);   $alg = preg_replace("/N4B2/","<908>",$alg);   $alg = preg_replace("/N4B/","<907>",$alg);
    $alg = preg_replace("/N4U'/","<918>",$alg); $alg = preg_replace("/N4U-/","<918>",$alg);   $alg = preg_replace("/N4U2/","<917>",$alg);   $alg = preg_replace("/N4U/","<916>",$alg);
    $alg = preg_replace("/N4D'/","<915>",$alg); $alg = preg_replace("/N4D-/","<915>",$alg);   $alg = preg_replace("/N4D2/","<914>",$alg);   $alg = preg_replace("/N4D/","<913>",$alg);
    ...
    $alg = preg_replace("/T2R'/","<319>",$alg); $alg = preg_replace("/T2R-/","<319>",$alg);   $alg = preg_replace("/T2R2/","<320>",$alg);   $alg = preg_replace("/T2R/","<321>",$alg);
    $alg = preg_replace("/T2L'/","<322>",$alg); $alg = preg_replace("/T2L-/","<322>",$alg);   $alg = preg_replace("/T2L2/","<323>",$alg);   $alg = preg_replace("/T2L/","<324>",$alg);
    $alg = preg_replace("/T2F'/","<325>",$alg); $alg = preg_replace("/T2F-/","<325>",$alg);   $alg = preg_replace("/T2F2/","<326>",$alg);   $alg = preg_replace("/T2F/","<327>",$alg);
    $alg = preg_replace("/T2B'/","<328>",$alg); $alg = preg_replace("/T2B-/","<328>",$alg);   $alg = preg_replace("/T2B2/","<329>",$alg);   $alg = preg_replace("/T2B/","<330>",$alg);
    $alg = preg_replace("/T2U'/","<331>",$alg); $alg = preg_replace("/T2U-/","<331>",$alg);   $alg = preg_replace("/T2U2/","<332>",$alg);   $alg = preg_replace("/T2U/","<333>",$alg);
    $alg = preg_replace("/T2D'/","<334>",$alg); $alg = preg_replace("/T2D-/","<334>",$alg);   $alg = preg_replace("/T2D2/","<335>",$alg);   $alg = preg_replace("/T2D/","<336>",$alg);
  ...
  */
  function alg4xC_TwizzleToSse($alg) {
    ...
    $alg = preg_replace("/2-4R'/","<306>",$alg); $alg = preg_replace("/2-4R2/","<305>",$alg); $alg = preg_replace("/2-4R/","<304>",$alg);
    $alg = preg_replace("/2-4L'/","<303>",$alg); $alg = preg_replace("/2-4L2/","<302>",$alg); $alg = preg_replace("/2-4L/","<301>",$alg);
    $alg = preg_replace("/2-4F'/","<312>",$alg); $alg = preg_replace("/2-4F2/","<311>",$alg); $alg = preg_replace("/2-4F/","<310>",$alg);
    $alg = preg_replace("/2-4B'/","<309>",$alg); $alg = preg_replace("/2-4B2/","<308>",$alg); $alg = preg_replace("/2-4B/","<307>",$alg);
    $alg = preg_replace("/2-4U'/","<318>",$alg); $alg = preg_replace("/2-4U2/","<317>",$alg); $alg = preg_replace("/2-4U/","<316>",$alg);
    $alg = preg_replace("/2-4D'/","<315>",$alg); $alg = preg_replace("/2-4D2/","<314>",$alg); $alg = preg_replace("/2-4D/","<313>",$alg);
    ...
    $alg = preg_replace("/3-4R'/","<324>",$alg); $alg = preg_replace("/3-4R2/","<323>",$alg); $alg = preg_replace("/3-4R/","<322>",$alg);
    $alg = preg_replace("/3-4L'/","<321>",$alg); $alg = preg_replace("/3-4L2/","<320>",$alg); $alg = preg_replace("/3-4L/","<319>",$alg);
    $alg = preg_replace("/3-4F'/","<330>",$alg); $alg = preg_replace("/3-4F2/","<329>",$alg); $alg = preg_replace("/3-4F/","<328>",$alg);
    $alg = preg_replace("/3-4B'/","<327>",$alg); $alg = preg_replace("/3-4B2/","<326>",$alg); $alg = preg_replace("/3-4B/","<325>",$alg);
    $alg = preg_replace("/3-4U'/","<336>",$alg); $alg = preg_replace("/3-4U2/","<335>",$alg); $alg = preg_replace("/3-4U/","<334>",$alg);
    $alg = preg_replace("/3-4D'/","<333>",$alg); $alg = preg_replace("/3-4D2/","<332>",$alg); $alg = preg_replace("/3-4D/","<331>",$alg);
    
    /* --- 4xC: TWIZZLE -> CODE: [2] Slice twists --- */
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/2-3R' Rv'/","<228>",$alg);
      $alg = preg_replace("/2-3L' Lv'/","<229>",$alg);
      $alg = preg_replace("/2-3F' Fv'/","<230>",$alg);
      $alg = preg_replace("/2-3B' Bv'/","<231>",$alg);
      $alg = preg_replace("/2-3U' Uv'/","<232>",$alg);
      $alg = preg_replace("/2-3D' Dv'/","<233>",$alg);
      
      $alg = preg_replace("/2-3L Rv'/","<228>",$alg);
      $alg = preg_replace("/2-3R Lv'/","<229>",$alg);
      $alg = preg_replace("/2-3B Fv'/","<230>",$alg);
      $alg = preg_replace("/2-3F Bv'/","<231>",$alg);
      $alg = preg_replace("/2-3D Uv'/","<232>",$alg);
      $alg = preg_replace("/2-3U Dv'/","<233>",$alg);
      
      /* S = S2-3 */
      $alg = preg_replace("/2-3R Rv'/","<201>",$alg); $alg = preg_replace("/2-3R2 Rv2/","<202>",$alg); $alg = preg_replace("/2-3R' Rv/","<203>",$alg);
      $alg = preg_replace("/2-3L Lv'/","<203>",$alg); $alg = preg_replace("/2-3L2 Lv2/","<202>",$alg); $alg = preg_replace("/2-3L' Lv/","<201>",$alg);
      $alg = preg_replace("/2-3F Fv'/","<204>",$alg); $alg = preg_replace("/2-3F2 Fv2/","<205>",$alg); $alg = preg_replace("/2-3F' Fv/","<206>",$alg);
      $alg = preg_replace("/2-3B Bv'/","<206>",$alg); $alg = preg_replace("/2-3B2 Bv2/","<205>",$alg); $alg = preg_replace("/2-3B' Bv/","<204>",$alg);
      $alg = preg_replace("/2-3U Uv'/","<207>",$alg); $alg = preg_replace("/2-3U2 Uv2/","<208>",$alg); $alg = preg_replace("/2-3U' Uv/","<209>",$alg);
      $alg = preg_replace("/2-3D Dv'/","<209>",$alg); $alg = preg_replace("/2-3D2 Dv2/","<208>",$alg); $alg = preg_replace("/2-3D' Dv/","<207>",$alg);
      
      $alg = preg_replace("/2-3L' Rv'/","<201>",$alg); $alg = preg_replace("/2-3L2 Rv2/","<202>",$alg); $alg = preg_replace("/2-3L Rv/","<203>",$alg);
      $alg = preg_replace("/2-3R' Lv'/","<203>",$alg); $alg = preg_replace("/2-3R2 Lv2/","<202>",$alg); $alg = preg_replace("/2-3R Lv/","<201>",$alg);
      $alg = preg_replace("/2-3B' Fv'/","<204>",$alg); $alg = preg_replace("/2-3B2 Fv2/","<205>",$alg); $alg = preg_replace("/2-3B Fv/","<206>",$alg);
      $alg = preg_replace("/2-3F' Bv'/","<206>",$alg); $alg = preg_replace("/2-3F2 Bv2/","<205>",$alg); $alg = preg_replace("/2-3F Bv/","<204>",$alg);
      $alg = preg_replace("/2-3D' Uv'/","<207>",$alg); $alg = preg_replace("/2-3D2 Uv2/","<208>",$alg); $alg = preg_replace("/2-3D Uv/","<209>",$alg);
      $alg = preg_replace("/2-3U' Dv'/","<209>",$alg); $alg = preg_replace("/2-3U2 Dv2/","<208>",$alg); $alg = preg_replace("/2-3U Dv/","<207>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2R' Rv'/","<234>",$alg);
      $alg = preg_replace("/2L' Lv'/","<235>",$alg);
      $alg = preg_replace("/2F' Fv'/","<236>",$alg);
      $alg = preg_replace("/2B' Bv'/","<237>",$alg);
      $alg = preg_replace("/2U' Uv'/","<238>",$alg);
      $alg = preg_replace("/2D' Dv'/","<239>",$alg);
      
      $alg = preg_replace("/3L Rv'/","<234>",$alg);
      $alg = preg_replace("/3R Lv'/","<235>",$alg);
      $alg = preg_replace("/3B Fv'/","<236>",$alg);
      $alg = preg_replace("/3F Bv'/","<237>",$alg);
      $alg = preg_replace("/3D Uv'/","<238>",$alg);
      $alg = preg_replace("/3U Dv'/","<239>",$alg);
      
      /* S2-2 | S3-3 */
      $alg = preg_replace("/2R Rv'/","<210>",$alg); $alg = preg_replace("/2R2 Rv2/","<211>",$alg); $alg = preg_replace("/2R' Rv/","<212>",$alg);
      $alg = preg_replace("/2L Lv'/","<213>",$alg); $alg = preg_replace("/2L2 Lv2/","<214>",$alg); $alg = preg_replace("/2L' Lv/","<215>",$alg);
      $alg = preg_replace("/2F Fv'/","<216>",$alg); $alg = preg_replace("/2F2 Fv2/","<217>",$alg); $alg = preg_replace("/2F' Fv/","<218>",$alg);
      $alg = preg_replace("/2B Bv'/","<219>",$alg); $alg = preg_replace("/2B2 Bv2/","<220>",$alg); $alg = preg_replace("/2B' Bv/","<221>",$alg);
      $alg = preg_replace("/2U Uv'/","<222>",$alg); $alg = preg_replace("/2U2 Uv2/","<223>",$alg); $alg = preg_replace("/2U' Uv/","<224>",$alg);
      $alg = preg_replace("/2D Dv'/","<225>",$alg); $alg = preg_replace("/2D2 Dv2/","<226>",$alg); $alg = preg_replace("/2D' Dv/","<227>",$alg);
      
      $alg = preg_replace("/3L' Rv'/","<210>",$alg); $alg = preg_replace("/3L2 Rv2/","<211>",$alg); $alg = preg_replace("/3L Rv/","<212>",$alg);
      $alg = preg_replace("/3R' Lv'/","<213>",$alg); $alg = preg_replace("/3R2 Lv2/","<214>",$alg); $alg = preg_replace("/3R Lv/","<215>",$alg);
      $alg = preg_replace("/3B' Fv'/","<216>",$alg); $alg = preg_replace("/3B2 Fv2/","<217>",$alg); $alg = preg_replace("/3B Fv/","<218>",$alg);
      $alg = preg_replace("/3F' Bv'/","<219>",$alg); $alg = preg_replace("/3F2 Bv2/","<220>",$alg); $alg = preg_replace("/3F Bv/","<221>",$alg);
      $alg = preg_replace("/3D' Uv'/","<222>",$alg); $alg = preg_replace("/3D2 Uv2/","<223>",$alg); $alg = preg_replace("/3D Uv/","<224>",$alg);
      $alg = preg_replace("/3U' Dv'/","<225>",$alg); $alg = preg_replace("/3U2 Dv2/","<226>",$alg); $alg = preg_replace("/3U Dv/","<227>",$alg);
    }
    ...
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/m Rv'/","<228>",$alg);
      $alg = preg_replace("/m' Lv'/","<229>",$alg);
      $alg = preg_replace("/s' Fv'/","<230>",$alg);
      $alg = preg_replace("/s Bv'/","<231>",$alg);
      $alg = preg_replace("/e Uv'/","<232>",$alg);
      $alg = preg_replace("/e' Dv'/","<233>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/R' L'/","<240>",$alg);
      $alg = preg_replace("/L' R'/","<240>",$alg);
      $alg = preg_replace("/F' B'/","<241>",$alg);
      $alg = preg_replace("/B' F'/","<241>",$alg);
      $alg = preg_replace("/U' D'/","<242>",$alg);
      $alg = preg_replace("/D' U'/","<242>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/R' l'/","<243>",$alg);
      $alg = preg_replace("/L' r'/","<244>",$alg);
      $alg = preg_replace("/F' b'/","<245>",$alg);
      $alg = preg_replace("/B' f'/","<246>",$alg);
      $alg = preg_replace("/U' d'/","<247>",$alg);
      $alg = preg_replace("/D' u'/","<248>",$alg);
      
      $alg = preg_replace("/r' L'/","<249>",$alg);
      $alg = preg_replace("/l' R'/","<250>",$alg);
      $alg = preg_replace("/f' B'/","<251>",$alg);
      $alg = preg_replace("/b' F'/","<252>",$alg);
      $alg = preg_replace("/u' D'/","<253>",$alg);
      $alg = preg_replace("/d' U'/","<254>",$alg);
      
      /* S2-2 | S3-3 */
      $alg = preg_replace("/R l'/","<212>",$alg); $alg = preg_replace("/R2 l2/","<211>",$alg); $alg = preg_replace("/R' l/","<210>",$alg);
      $alg = preg_replace("/L r'/","<215>",$alg); $alg = preg_replace("/L2 r2/","<214>",$alg); $alg = preg_replace("/L' r/","<213>",$alg);
      $alg = preg_replace("/F b'/","<218>",$alg); $alg = preg_replace("/F2 b2/","<217>",$alg); $alg = preg_replace("/F' b/","<216>",$alg);
      $alg = preg_replace("/B f'/","<221>",$alg); $alg = preg_replace("/B2 f2/","<220>",$alg); $alg = preg_replace("/B' f/","<219>",$alg);
      $alg = preg_replace("/U d'/","<224>",$alg); $alg = preg_replace("/U2 d2/","<223>",$alg); $alg = preg_replace("/U' d/","<222>",$alg);
      $alg = preg_replace("/D u'/","<227>",$alg); $alg = preg_replace("/D2 u2/","<226>",$alg); $alg = preg_replace("/D' u/","<225>",$alg);
      
      $alg = preg_replace("/r L'/","<213>",$alg); $alg = preg_replace("/r2 L2/","<214>",$alg); $alg = preg_replace("/r' L/","<215>",$alg);
      $alg = preg_replace("/l R'/","<210>",$alg); $alg = preg_replace("/l2 R2/","<211>",$alg); $alg = preg_replace("/l' R/","<212>",$alg);
      $alg = preg_replace("/f B'/","<219>",$alg); $alg = preg_replace("/f2 B2/","<220>",$alg); $alg = preg_replace("/f' B/","<221>",$alg);
      $alg = preg_replace("/b F'/","<216>",$alg); $alg = preg_replace("/b2 F2/","<217>",$alg); $alg = preg_replace("/b' F/","<218>",$alg);
      $alg = preg_replace("/u D'/","<225>",$alg); $alg = preg_replace("/u2 D2/","<226>",$alg); $alg = preg_replace("/u' D/","<227>",$alg);
      $alg = preg_replace("/d U'/","<222>",$alg); $alg = preg_replace("/d2 U2/","<223>",$alg); $alg = preg_replace("/d' U/","<224>",$alg);
    }
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/<228>/","WR' CR'",$alg);
      $alg = preg_replace("/<229>/","WR CR",$alg);
      $alg = preg_replace("/<230>/","WF' CF'",$alg);
      $alg = preg_replace("/<231>/","WF CF",$alg);
      $alg = preg_replace("/<232>/","WU' CU'",$alg);
      $alg = preg_replace("/<233>/","WU CU",$alg);
      
      
      $alg = preg_replace("/<234>/","MR' CR'",$alg);
      $alg = preg_replace("/<235>/","ML' CR",$alg);
      $alg = preg_replace("/<236>/","MF' CF'",$alg);
      $alg = preg_replace("/<237>/","MB' CF",$alg);
      $alg = preg_replace("/<238>/","MU' CU'",$alg);
      $alg = preg_replace("/<239>/","MD' CU",$alg);
      
      
      $alg = preg_replace("/<240>/","R' L'",$alg);
      $alg = preg_replace("/<241>/","F' B'",$alg);
      $alg = preg_replace("/<242>/","U' D'",$alg);
      
      
      $alg = preg_replace("/<243>/","R' TL'",$alg);
      $alg = preg_replace("/<244>/","TR' L'",$alg);
      $alg = preg_replace("/<245>/","F' TB'",$alg);
      $alg = preg_replace("/<246>/","TF' B'",$alg);
      $alg = preg_replace("/<247>/","U' TD'",$alg);
      $alg = preg_replace("/<248>/","TU' D'",$alg);
      
      $alg = preg_replace("/<249>/","TR' L'",$alg);
      $alg = preg_replace("/<250>/","R TL''",$alg);
      $alg = preg_replace("/<251>/","TF' B'",$alg);
      $alg = preg_replace("/<252>/","F' TB'",$alg);
      $alg = preg_replace("/<253>/","TU' D'",$alg);
      $alg = preg_replace("/<254>/","U' TD'",$alg);
    }
  ...
  */
  function alg5xC_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N5R'/","<906>",$alg); $alg = preg_replace("/N5R-/","<906>",$alg);   $alg = preg_replace("/N5R2/","<905>",$alg);   $alg = preg_replace("/N5R/","<904>",$alg);
    $alg = preg_replace("/N5L'/","<903>",$alg); $alg = preg_replace("/N5L-/","<903>",$alg);   $alg = preg_replace("/N5L2/","<902>",$alg);   $alg = preg_replace("/N5L/","<901>",$alg);
    $alg = preg_replace("/N5F'/","<912>",$alg); $alg = preg_replace("/N5F-/","<912>",$alg);   $alg = preg_replace("/N5F2/","<911>",$alg);   $alg = preg_replace("/N5F/","<910>",$alg);
    $alg = preg_replace("/N5B'/","<909>",$alg); $alg = preg_replace("/N5B-/","<909>",$alg);   $alg = preg_replace("/N5B2/","<908>",$alg);   $alg = preg_replace("/N5B/","<907>",$alg);
    $alg = preg_replace("/N5U'/","<918>",$alg); $alg = preg_replace("/N5U-/","<918>",$alg);   $alg = preg_replace("/N5U2/","<917>",$alg);   $alg = preg_replace("/N5U/","<916>",$alg);
    $alg = preg_replace("/N5D'/","<915>",$alg); $alg = preg_replace("/N5D-/","<915>",$alg);   $alg = preg_replace("/N5D2/","<914>",$alg);   $alg = preg_replace("/N5D/","<913>",$alg);
    ...
    $alg = preg_replace("/T2R'/","<337>",$alg); $alg = preg_replace("/T2R-/","<337>",$alg);   $alg = preg_replace("/T2R2/","<338>",$alg);   $alg = preg_replace("/T2R/","<339>",$alg);
    $alg = preg_replace("/T2L'/","<340>",$alg); $alg = preg_replace("/T2L-/","<340>",$alg);   $alg = preg_replace("/T2L2/","<341>",$alg);   $alg = preg_replace("/T2L/","<342>",$alg);
    $alg = preg_replace("/T2F'/","<343>",$alg); $alg = preg_replace("/T2F-/","<343>",$alg);   $alg = preg_replace("/T2F2/","<344>",$alg);   $alg = preg_replace("/T2F/","<345>",$alg);
    $alg = preg_replace("/T2B'/","<346>",$alg); $alg = preg_replace("/T2B-/","<346>",$alg);   $alg = preg_replace("/T2B2/","<347>",$alg);   $alg = preg_replace("/T2B/","<348>",$alg);
    $alg = preg_replace("/T2U'/","<349>",$alg); $alg = preg_replace("/T2U-/","<349>",$alg);   $alg = preg_replace("/T2U2/","<350>",$alg);   $alg = preg_replace("/T2U/","<351>",$alg);
    $alg = preg_replace("/T2D'/","<352>",$alg); $alg = preg_replace("/T2D-/","<352>",$alg);   $alg = preg_replace("/T2D2/","<353>",$alg);   $alg = preg_replace("/T2D/","<354>",$alg);
  ...
  */
  function alg5xC_TwizzleToSse($alg) {
    ...
    $alg = preg_replace("/2-5R'/","<306>",$alg); $alg = preg_replace("/2-5R2/","<305>",$alg); $alg = preg_replace("/2-5R/","<304>",$alg);
    $alg = preg_replace("/2-5L'/","<303>",$alg); $alg = preg_replace("/2-5L2/","<302>",$alg); $alg = preg_replace("/2-5L/","<301>",$alg);
    $alg = preg_replace("/2-5F'/","<312>",$alg); $alg = preg_replace("/2-5F2/","<311>",$alg); $alg = preg_replace("/2-5F/","<310>",$alg);
    $alg = preg_replace("/2-5B'/","<309>",$alg); $alg = preg_replace("/2-5B2/","<308>",$alg); $alg = preg_replace("/2-5B/","<307>",$alg);
    $alg = preg_replace("/2-5U'/","<318>",$alg); $alg = preg_replace("/2-5U2/","<317>",$alg); $alg = preg_replace("/2-5U/","<316>",$alg);
    $alg = preg_replace("/2-5D'/","<315>",$alg); $alg = preg_replace("/2-5D2/","<314>",$alg); $alg = preg_replace("/2-5D/","<313>",$alg);
    ...
    $alg = preg_replace("/3-5R'/","<324>",$alg); $alg = preg_replace("/3-5R2/","<323>",$alg); $alg = preg_replace("/3-5R/","<322>",$alg);
    $alg = preg_replace("/3-5L'/","<321>",$alg); $alg = preg_replace("/3-5L2/","<320>",$alg); $alg = preg_replace("/3-5L/","<319>",$alg);
    $alg = preg_replace("/3-5F'/","<330>",$alg); $alg = preg_replace("/3-5F2/","<329>",$alg); $alg = preg_replace("/3-5F/","<328>",$alg);
    $alg = preg_replace("/3-5B'/","<327>",$alg); $alg = preg_replace("/3-5B2/","<326>",$alg); $alg = preg_replace("/3-5B/","<325>",$alg);
    $alg = preg_replace("/3-5U'/","<336>",$alg); $alg = preg_replace("/3-5U2/","<335>",$alg); $alg = preg_replace("/3-5U/","<334>",$alg);
    $alg = preg_replace("/3-5D'/","<333>",$alg); $alg = preg_replace("/3-5D2/","<332>",$alg); $alg = preg_replace("/3-5D/","<331>",$alg);
    ...
    $alg = preg_replace("/4-5R'/","<342>",$alg); $alg = preg_replace("/4-5R2/","<341>",$alg); $alg = preg_replace("/4-5R/","<340>",$alg);
    $alg = preg_replace("/4-5L'/","<339>",$alg); $alg = preg_replace("/4-5L2/","<338>",$alg); $alg = preg_replace("/4-5L/","<337>",$alg);
    $alg = preg_replace("/4-5F'/","<348>",$alg); $alg = preg_replace("/4-5F2/","<347>",$alg); $alg = preg_replace("/4-5F/","<346>",$alg);
    $alg = preg_replace("/4-5B'/","<345>",$alg); $alg = preg_replace("/4-5B2/","<344>",$alg); $alg = preg_replace("/4-5B/","<343>",$alg);
    $alg = preg_replace("/4-5U'/","<354>",$alg); $alg = preg_replace("/4-5U2/","<353>",$alg); $alg = preg_replace("/4-5U/","<352>",$alg);
    $alg = preg_replace("/4-5D'/","<351>",$alg); $alg = preg_replace("/4-5D2/","<350>",$alg); $alg = preg_replace("/4-5D/","<349>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2-4R' Rv'/","<255>",$alg);
      $alg = preg_replace("/2-4L' Lv'/","<256>",$alg);
      $alg = preg_replace("/2-4F' Fv'/","<257>",$alg);
      $alg = preg_replace("/2-4B' Bv'/","<258>",$alg);
      $alg = preg_replace("/2-4U' Uv'/","<259>",$alg);
      $alg = preg_replace("/2-4D' Dv'/","<260>",$alg);
      
      $alg = preg_replace("/2-4L Rv'/","<255>",$alg);
      $alg = preg_replace("/2-4R Lv'/","<256>",$alg);
      $alg = preg_replace("/2-4B Fv'/","<257>",$alg);
      $alg = preg_replace("/2-4F Bv'/","<258>",$alg);
      $alg = preg_replace("/2-4D Uv'/","<259>",$alg);
      $alg = preg_replace("/2-4U Dv'/","<260>",$alg);
      
      /* S = S2-4 */
      $alg = preg_replace("/2-4R Rv'/","<210>",$alg); $alg = preg_replace("/2-4R2 Rv2/","<211>",$alg); $alg = preg_replace("/2-4R' Rv/","<212>",$alg);
      $alg = preg_replace("/2-4L Lv'/","<212>",$alg); $alg = preg_replace("/2-4L2 Lv2/","<211>",$alg); $alg = preg_replace("/2-4L' Lv/","<210>",$alg);
      $alg = preg_replace("/2-4F Fv'/","<213>",$alg); $alg = preg_replace("/2-4F2 Fv2/","<214>",$alg); $alg = preg_replace("/2-4F' Fv/","<215>",$alg);
      $alg = preg_replace("/2-4B Bv'/","<215>",$alg); $alg = preg_replace("/2-4B2 Bv2/","<214>",$alg); $alg = preg_replace("/2-4B' Bv/","<213>",$alg);
      $alg = preg_replace("/2-4U Uv'/","<216>",$alg); $alg = preg_replace("/2-4U2 Uv2/","<217>",$alg); $alg = preg_replace("/2-4U' Uv/","<218>",$alg);
      $alg = preg_replace("/2-4D Dv'/","<218>",$alg); $alg = preg_replace("/2-4D2 Dv2/","<217>",$alg); $alg = preg_replace("/2-4D' Dv/","<216>",$alg);
      
      $alg = preg_replace("/2-4L' Rv'/","<210>",$alg); $alg = preg_replace("/2-4L2 Rv2/","<211>",$alg); $alg = preg_replace("/2-4L Rv/","<212>",$alg);
      $alg = preg_replace("/2-4R' Lv'/","<212>",$alg); $alg = preg_replace("/2-4R2 Lv2/","<211>",$alg); $alg = preg_replace("/2-4R Lv/","<210>",$alg);
      $alg = preg_replace("/2-4B' Fv'/","<213>",$alg); $alg = preg_replace("/2-4B2 Fv2/","<214>",$alg); $alg = preg_replace("/2-4B Fv/","<215>",$alg);
      $alg = preg_replace("/2-4F' Bv'/","<215>",$alg); $alg = preg_replace("/2-4F2 Bv2/","<214>",$alg); $alg = preg_replace("/2-4F Bv/","<213>",$alg);
      $alg = preg_replace("/2-4D' Uv'/","<216>",$alg); $alg = preg_replace("/2-4D2 Uv2/","<217>",$alg); $alg = preg_replace("/2-4D Uv/","<218>",$alg);
      $alg = preg_replace("/2-4U' Dv'/","<218>",$alg); $alg = preg_replace("/2-4U2 Dv2/","<217>",$alg); $alg = preg_replace("/2-4U Dv/","<216>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2-3R' Rv'/","<261>",$alg);
      $alg = preg_replace("/2-3L' Lv'/","<262>",$alg);
      $alg = preg_replace("/2-3F' Fv'/","<263>",$alg);
      $alg = preg_replace("/2-3B' Bv'/","<264>",$alg);
      $alg = preg_replace("/2-3U' Uv'/","<265>",$alg);
      $alg = preg_replace("/2-3D' Dv'/","<266>",$alg);
      
      $alg = preg_replace("/3-4L Rv'/","<261>",$alg);
      $alg = preg_replace("/3-4R Lv'/","<262>",$alg);
      $alg = preg_replace("/3-4B Fv'/","<263>",$alg);
      $alg = preg_replace("/3-4F Bv'/","<264>",$alg);
      $alg = preg_replace("/3-4D Uv'/","<265>",$alg);
      $alg = preg_replace("/3-4U Dv'/","<266>",$alg);
      
      /* S2-3 | S3-4 */
      $alg = preg_replace("/2-3R Rv'/","<237>",$alg); $alg = preg_replace("/2-3R2 Rv2/","<238>",$alg); $alg = preg_replace("/2-3R' Rv/","<239>",$alg);
      $alg = preg_replace("/2-3L Lv'/","<240>",$alg); $alg = preg_replace("/2-3L2 Lv2/","<241>",$alg); $alg = preg_replace("/2-3L' Lv/","<242>",$alg);
      $alg = preg_replace("/2-3F Fv'/","<243>",$alg); $alg = preg_replace("/2-3F2 Fv2/","<244>",$alg); $alg = preg_replace("/2-3F' Fv/","<245>",$alg);
      $alg = preg_replace("/2-3B Bv'/","<246>",$alg); $alg = preg_replace("/2-3B2 Bv2/","<247>",$alg); $alg = preg_replace("/2-3B' Bv/","<248>",$alg);
      $alg = preg_replace("/2-3U Uv'/","<249>",$alg); $alg = preg_replace("/2-3U2 Uv2/","<250>",$alg); $alg = preg_replace("/2-3U' Uv/","<251>",$alg);
      $alg = preg_replace("/2-3D Dv'/","<252>",$alg); $alg = preg_replace("/2-3D2 Dv2/","<253>",$alg); $alg = preg_replace("/2-3D' Dv/","<254>",$alg);
      
      $alg = preg_replace("/3-4L' Rv'/","<237>",$alg); $alg = preg_replace("/3-4L2 Rv2/","<238>",$alg); $alg = preg_replace("/3-4L Rv/","<239>",$alg);
      $alg = preg_replace("/3-4R' Lv'/","<240>",$alg); $alg = preg_replace("/3-4R2 Lv2/","<241>",$alg); $alg = preg_replace("/3-4R Lv/","<242>",$alg);
      $alg = preg_replace("/3-4B' Fv'/","<243>",$alg); $alg = preg_replace("/3-4B2 Fv2/","<244>",$alg); $alg = preg_replace("/3-4B Fv/","<245>",$alg);
      $alg = preg_replace("/3-4F' Bv'/","<246>",$alg); $alg = preg_replace("/3-4F2 Bv2/","<247>",$alg); $alg = preg_replace("/3-4F Bv/","<248>",$alg);
      $alg = preg_replace("/3-4D' Uv'/","<249>",$alg); $alg = preg_replace("/3-4D2 Uv2/","<250>",$alg); $alg = preg_replace("/3-4D Uv/","<251>",$alg);
      $alg = preg_replace("/3-4U' Dv'/","<252>",$alg); $alg = preg_replace("/3-4U2 Dv2/","<253>",$alg); $alg = preg_replace("/3-4U Dv/","<254>",$alg);
    }
    ...
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/3R' Rv'/","<267>",$alg);
      $alg = preg_replace("/3L' Lv'/","<268>",$alg);
      $alg = preg_replace("/3F' Fv'/","<269>",$alg);
      $alg = preg_replace("/3B' Bv'/","<270>",$alg);
      $alg = preg_replace("/3U' Uv'/","<271>",$alg);
      $alg = preg_replace("/3D' Dv'/","<272>",$alg);
      
      $alg = preg_replace("/3L Rv'/","<267>",$alg);
      $alg = preg_replace("/3R Lv'/","<268>",$alg);
      $alg = preg_replace("/3B Fv'/","<269>",$alg);
      $alg = preg_replace("/3F Bv'/","<270>",$alg);
      $alg = preg_replace("/3D Uv'/","<271>",$alg);
      $alg = preg_replace("/3U Dv'/","<272>",$alg);
      
      /* S2 = S3-3 */
      $alg = preg_replace("/3R Rv'/","<201>",$alg); $alg = preg_replace("/3R2 Rv2/","<202>",$alg); $alg = preg_replace("/3R' Rv/","<203>",$alg);
      $alg = preg_replace("/3L Lv'/","<203>",$alg); $alg = preg_replace("/3L2 Lv2/","<202>",$alg); $alg = preg_replace("/3L' Lv/","<201>",$alg);
      $alg = preg_replace("/3F Fv'/","<204>",$alg); $alg = preg_replace("/3F2 Fv2/","<205>",$alg); $alg = preg_replace("/3F' Fv/","<206>",$alg);
      $alg = preg_replace("/3B Bv'/","<206>",$alg); $alg = preg_replace("/3B2 Bv2/","<205>",$alg); $alg = preg_replace("/3B' Bv/","<204>",$alg);
      $alg = preg_replace("/3U Uv'/","<207>",$alg); $alg = preg_replace("/3U2 Uv2/","<208>",$alg); $alg = preg_replace("/3U' Uv/","<209>",$alg);
      $alg = preg_replace("/3D Dv'/","<209>",$alg); $alg = preg_replace("/3D2 Dv2/","<208>",$alg); $alg = preg_replace("/3D' Dv/","<207>",$alg);
      
      $alg = preg_replace("/3L' Rv'/","<201>",$alg); $alg = preg_replace("/3L2 Rv2/","<202>",$alg); $alg = preg_replace("/3L Rv/","<203>",$alg);
      $alg = preg_replace("/3R' Lv'/","<203>",$alg); $alg = preg_replace("/3R2 Lv2/","<202>",$alg); $alg = preg_replace("/3R Lv/","<201>",$alg);
      $alg = preg_replace("/3B' Fv'/","<204>",$alg); $alg = preg_replace("/3B2 Fv2/","<205>",$alg); $alg = preg_replace("/3B Fv/","<206>",$alg);
      $alg = preg_replace("/3F' Bv'/","<206>",$alg); $alg = preg_replace("/3F2 Bv2/","<205>",$alg); $alg = preg_replace("/3F Bv/","<204>",$alg);
      $alg = preg_replace("/3D' Uv'/","<207>",$alg); $alg = preg_replace("/3D2 Uv2/","<208>",$alg); $alg = preg_replace("/3D Uv/","<209>",$alg);
      $alg = preg_replace("/3U' Dv'/","<209>",$alg); $alg = preg_replace("/3U2 Dv2/","<208>",$alg); $alg = preg_replace("/3U Dv/","<207>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2R' Rv'/","<273>",$alg);
      $alg = preg_replace("/2L' Lv'/","<274>",$alg);
      $alg = preg_replace("/2F' Fv'/","<275>",$alg);
      $alg = preg_replace("/2B' Bv'/","<276>",$alg);
      $alg = preg_replace("/2U' Uv'/","<277>",$alg);
      $alg = preg_replace("/2D' Dv'/","<278>",$alg);
      
      $alg = preg_replace("/4L Rv'/","<273>",$alg);
      $alg = preg_replace("/4R Lv'/","<274>",$alg);
      $alg = preg_replace("/4B Fv'/","<275>",$alg);
      $alg = preg_replace("/4F Bv'/","<276>",$alg);
      $alg = preg_replace("/4D Uv'/","<277>",$alg);
      $alg = preg_replace("/4U Dv'/","<278>",$alg);
      
      $alg = preg_replace("/3l' R'/","<279>",$alg);
      $alg = preg_replace("/3r' L'/","<280>",$alg);
      $alg = preg_replace("/3b' F'/","<281>",$alg);
      $alg = preg_replace("/3f' B'/","<282>",$alg);
      $alg = preg_replace("/3d' U'/","<283>",$alg);
      $alg = preg_replace("/3u' D'/","<284>",$alg);
      
      /* S2-2 | S4-4 */
      $alg = preg_replace("/2R Rv'/","<219>",$alg);  $alg = preg_replace("/2R2 Rv2/","<220>",$alg); $alg = preg_replace("/2R' Rv/","<221>",$alg);
      $alg = preg_replace("/2L Lv'/","<222>",$alg);  $alg = preg_replace("/2L2 Lv2/","<223>",$alg); $alg = preg_replace("/2L' Lv/","<224>",$alg);
      $alg = preg_replace("/2F Fv'/","<225>",$alg);  $alg = preg_replace("/2F2 Fv2/","<226>",$alg); $alg = preg_replace("/2F' Fv/","<227>",$alg);
      $alg = preg_replace("/2B Bv'/","<228>",$alg);  $alg = preg_replace("/2B2 Bv2/","<229>",$alg); $alg = preg_replace("/2B' Bv/","<230>",$alg);
      $alg = preg_replace("/2U Uv'/","<231>",$alg);  $alg = preg_replace("/2U2 Uv2/","<232>",$alg); $alg = preg_replace("/2U' Uv/","<233>",$alg);
      $alg = preg_replace("/2D Dv'/","<234>",$alg);  $alg = preg_replace("/2D2 Dv2/","<235>",$alg); $alg = preg_replace("/2D' Dv/","<236>",$alg);
      
      $alg = preg_replace("/4L' Rv'/","<219>",$alg); $alg = preg_replace("/4L2 Rv2/","<220>",$alg); $alg = preg_replace("/4L Rv/","<221>",$alg);
      $alg = preg_replace("/4R' Lv'/","<222>",$alg); $alg = preg_replace("/4R2 Lv2/","<223>",$alg); $alg = preg_replace("/4R Lv/","<224>",$alg);
      $alg = preg_replace("/4B' Fv'/","<225>",$alg); $alg = preg_replace("/4B2 Fv2/","<226>",$alg); $alg = preg_replace("/4B Fv/","<227>",$alg);
      $alg = preg_replace("/4F' Bv'/","<228>",$alg); $alg = preg_replace("/4F2 Bv2/","<229>",$alg); $alg = preg_replace("/4F Bv/","<230>",$alg);
      $alg = preg_replace("/4D' Uv'/","<231>",$alg); $alg = preg_replace("/4D2 Uv2/","<232>",$alg); $alg = preg_replace("/4D Uv/","<233>",$alg);
      $alg = preg_replace("/4U' Dv'/","<234>",$alg); $alg = preg_replace("/4U2 Dv2/","<235>",$alg); $alg = preg_replace("/4U Dv/","<236>",$alg);
      
      $alg = preg_replace("/3l R'/","<219>",$alg);   $alg = preg_replace("/3l2 R2/","<220>",$alg);  $alg = preg_replace("/3l' R/","<221>",$alg);
      $alg = preg_replace("/3r L'/","<222>",$alg);   $alg = preg_replace("/3r2 L2/","<223>",$alg);  $alg = preg_replace("/3r' L/","<224>",$alg);
      $alg = preg_replace("/3b F'/","<225>",$alg);   $alg = preg_replace("/3b2 F2/","<226>",$alg);  $alg = preg_replace("/3b' F/","<227>",$alg);
      $alg = preg_replace("/3f B'/","<228>",$alg);   $alg = preg_replace("/3f2 B2/","<229>",$alg);  $alg = preg_replace("/3f' B/","<230>",$alg);
      $alg = preg_replace("/3d U'/","<231>",$alg);   $alg = preg_replace("/3d2 U2/","<232>",$alg);  $alg = preg_replace("/3d' U/","<233>",$alg);
      $alg = preg_replace("/3u D'/","<234>",$alg);   $alg = preg_replace("/3u2 D2/","<235>",$alg);  $alg = preg_replace("/3u' D/","<236>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/L' 3r'/","<285>",$alg);
      $alg = preg_replace("/R' 3l'/","<286>",$alg);
      $alg = preg_replace("/B' 3r'/","<287>",$alg);
      $alg = preg_replace("/F' 3b'/","<288>",$alg);
      $alg = preg_replace("/D' 3u'/","<289>",$alg);
      $alg = preg_replace("/U' 3d'/","<290>",$alg);
      
      /* S2-2 | S4-4 */
      $alg = preg_replace("/L 3r'/","<224>",$alg);   $alg = preg_replace("/L2 3r2/","<223>",$alg);  $alg = preg_replace("/L' 3r/","<222>",$alg);
      $alg = preg_replace("/R 3l'/","<221>",$alg);   $alg = preg_replace("/R2 3l2/","<220>",$alg);  $alg = preg_replace("/R' 3l/","<219>",$alg);
      $alg = preg_replace("/B 3f'/","<230>",$alg);   $alg = preg_replace("/B2 3f2/","<229>",$alg);  $alg = preg_replace("/B' 3f/","<228>",$alg);
      $alg = preg_replace("/F 3b'/","<227>",$alg);   $alg = preg_replace("/F2 3b2/","<226>",$alg);  $alg = preg_replace("/F' 3b/","<225>",$alg);
      $alg = preg_replace("/D 3u'/","<236>",$alg);   $alg = preg_replace("/D2 3u2/","<235>",$alg);  $alg = preg_replace("/D' 3u/","<234>",$alg);
      $alg = preg_replace("/U 3d'/","<233>",$alg);   $alg = preg_replace("/U2 3d2/","<232>",$alg);  $alg = preg_replace("/U' 3d/","<231>",$alg);
    ...
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/Rw' Lw'/","<291>",$alg);
      $alg = preg_replace("/Lw' Rw'/","<291>",$alg);
      $alg = preg_replace("/Fw' Bw'/","<292>",$alg);
      $alg = preg_replace("/Bw' Fw'/","<292>",$alg);
      $alg = preg_replace("/Uw' Dw'/","<293>",$alg);
      $alg = preg_replace("/Dw' Uw'/","<293>",$alg);
      
      $alg = preg_replace("/r' l'/","<291>",$alg);
      $alg = preg_replace("/l' r'/","<291>",$alg);
      $alg = preg_replace("/f' b'/","<292>",$alg);
      $alg = preg_replace("/b' f'/","<292>",$alg);
      $alg = preg_replace("/u' d'/","<293>",$alg);
      $alg = preg_replace("/d' u'/","<293>",$alg);
      
      /* S2 = S3-3 */
      $alg = preg_replace("/Rw Lw'/","<203>",$alg); $alg = preg_replace("/Rw2 Lw2/","<202>",$alg); $alg = preg_replace("/Rw' Lw/","<201>",$alg);
      $alg = preg_replace("/Lw Rw'/","<201>",$alg); $alg = preg_replace("/Lw2 Rw2/","<202>",$alg); $alg = preg_replace("/Lw' Rw/","<203>",$alg);
      $alg = preg_replace("/Fw Bw'/","<206>",$alg); $alg = preg_replace("/Fw2 Bw2/","<205>",$alg); $alg = preg_replace("/Fw' Bw/","<204>",$alg);
      $alg = preg_replace("/Bw Fw'/","<204>",$alg); $alg = preg_replace("/Bw2 Fw2/","<205>",$alg); $alg = preg_replace("/Bw' Fw/","<206>",$alg);
      $alg = preg_replace("/Uw Dw'/","<209>",$alg); $alg = preg_replace("/Uw2 Dw2/","<208>",$alg); $alg = preg_replace("/Uw' Dw/","<207>",$alg);
      $alg = preg_replace("/Dw Uw'/","<207>",$alg); $alg = preg_replace("/Dw2 Uw2/","<208>",$alg); $alg = preg_replace("/Dw' Uw/","<209>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/M Rv'/","<294>",$alg);
      $alg = preg_replace("/M' Lv'/","<295>",$alg);
      $alg = preg_replace("/S' Fv'/","<296>",$alg);
      $alg = preg_replace("/S Bv'/","<297>",$alg);
      $alg = preg_replace("/E Uv'/","<298>",$alg);
      $alg = preg_replace("/E' Dv'/","<299>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/R' L'/","<2100>",$alg);
      $alg = preg_replace("/L' R'/","<2100>",$alg);
      $alg = preg_replace("/F' B'/","<2101>",$alg);
      $alg = preg_replace("/B' F'/","<2101>",$alg);
      $alg = preg_replace("/U' D'/","<2102>",$alg);
      $alg = preg_replace("/D' U'/","<2102>",$alg);
      
      /* S = S2-4 */
      $alg = preg_replace("/R L'/","<212>",$alg); $alg = preg_replace("/R2 L2/","<211>",$alg); $alg = preg_replace("/R' L/","<210>",$alg);
      $alg = preg_replace("/L R'/","<210>",$alg); $alg = preg_replace("/L2 R2/","<211>",$alg); $alg = preg_replace("/L' R/","<212>",$alg);
      $alg = preg_replace("/F B'/","<215>",$alg); $alg = preg_replace("/F2 B2/","<214>",$alg); $alg = preg_replace("/F' B/","<213>",$alg);
      $alg = preg_replace("/B F'/","<213>",$alg); $alg = preg_replace("/B2 F2/","<214>",$alg); $alg = preg_replace("/B' F/","<215>",$alg);
      $alg = preg_replace("/U D'/","<218>",$alg); $alg = preg_replace("/U2 D2/","<217>",$alg); $alg = preg_replace("/U' D/","<216>",$alg);
      $alg = preg_replace("/D U'/","<216>",$alg); $alg = preg_replace("/D2 U2/","<217>",$alg); $alg = preg_replace("/D' U/","<218>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/R' l'/","<2103>",$alg);
      $alg = preg_replace("/L' r'/","<2104>",$alg);
      $alg = preg_replace("/F' b'/","<2105>",$alg);
      $alg = preg_replace("/B' f'/","<2106>",$alg);
      $alg = preg_replace("/U' d'/","<2107>",$alg);
      $alg = preg_replace("/D' u'/","<2108>",$alg);
      
      $alg = preg_replace("/l' R'/","<2103>",$alg);
      $alg = preg_replace("/r' L'/","<2104>",$alg);
      $alg = preg_replace("/b' F'/","<2105>",$alg);
      $alg = preg_replace("/f' B'/","<2106>",$alg);
      $alg = preg_replace("/d' U'/","<2107>",$alg);
      $alg = preg_replace("/u' D'/","<2108>",$alg);
      
      /* S2-3 | S3-4 */
      $alg = preg_replace("/R l'/","<239>",$alg); $alg = preg_replace("/R2 l2/","<238>",$alg); $alg = preg_replace("/R' l/","<237>",$alg);
      $alg = preg_replace("/L r'/","<242>",$alg); $alg = preg_replace("/L2 r2/","<241>",$alg); $alg = preg_replace("/L' r/","<240>",$alg);
      $alg = preg_replace("/F b'/","<246>",$alg); $alg = preg_replace("/F2 b2/","<244>",$alg); $alg = preg_replace("/F' b/","<243>",$alg);
      $alg = preg_replace("/B f'/","<248>",$alg); $alg = preg_replace("/B2 f2/","<247>",$alg); $alg = preg_replace("/B' f/","<246>",$alg);
      $alg = preg_replace("/U d'/","<251>",$alg); $alg = preg_replace("/U2 d2/","<250>",$alg); $alg = preg_replace("/U' d/","<249>",$alg);
      $alg = preg_replace("/D u'/","<254>",$alg); $alg = preg_replace("/D2 u2/","<253>",$alg); $alg = preg_replace("/D' u/","<252>",$alg);
      
      $alg = preg_replace("/l R'/","<237>",$alg); $alg = preg_replace("/l2 R2/","<238>",$alg); $alg = preg_replace("/l' R/","<239>",$alg);
      $alg = preg_replace("/r L'/","<240>",$alg); $alg = preg_replace("/r2 L2/","<241>",$alg); $alg = preg_replace("/r' L/","<242>",$alg);
      $alg = preg_replace("/b F'/","<243>",$alg); $alg = preg_replace("/b2 F2/","<244>",$alg); $alg = preg_replace("/b' F/","<245>",$alg);
      $alg = preg_replace("/f B'/","<246>",$alg); $alg = preg_replace("/f2 B2/","<247>",$alg); $alg = preg_replace("/f' B/","<248>",$alg);
      $alg = preg_replace("/d U'/","<249>",$alg); $alg = preg_replace("/d2 U2/","<250>",$alg); $alg = preg_replace("/d' U/","<251>",$alg);
      $alg = preg_replace("/u D'/","<252>",$alg); $alg = preg_replace("/u2 D2/","<253>",$alg); $alg = preg_replace("/u' D/","<254>",$alg);
    }
    ...
    $alg = preg_replace("/S'/","<122>",$alg); $alg = preg_replace("/S2/","<123>",$alg); $alg = preg_replace("/S/","<124>",$alg);
    $alg = preg_replace("/E'/","<127>",$alg); $alg = preg_replace("/E2/","<126>",$alg); $alg = preg_replace("/E/","<125>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/<255>/","WR' CR'",$alg);
      $alg = preg_replace("/<256>/","WR CR",$alg);
      $alg = preg_replace("/<257>/","WF' CF'",$alg);
      $alg = preg_replace("/<258>/","WF CF",$alg);
      $alg = preg_replace("/<259>/","WU' CU'",$alg);
      $alg = preg_replace("/<260>/","WU CU",$alg);
      
      
      $alg = preg_replace("/<261>/","VR' CR'",$alg);
      $alg = preg_replace("/<262>/","VL' CR",$alg);
      $alg = preg_replace("/<263>/","VF' CF'",$alg);
      $alg = preg_replace("/<264>/","VB' CF",$alg);
      $alg = preg_replace("/<265>/","VU' CU'",$alg);
      $alg = preg_replace("/<266>/","VD' CU",$alg);
      
      
      $alg = preg_replace("/<267>/","MR' CR'",$alg);
      $alg = preg_replace("/<268>/","MR CR",$alg);
      $alg = preg_replace("/<269>/","MF' CF'",$alg);
      $alg = preg_replace("/<270>/","MF CF",$alg);
      $alg = preg_replace("/<271>/","MU' CU'",$alg);
      $alg = preg_replace("/<272>/","MU CU",$alg);
      
      
      $alg = preg_replace("/<273>/","NR' CR'",$alg);
      $alg = preg_replace("/<274>/","NL' CR",$alg);
      $alg = preg_replace("/<275>/","NF' CF'",$alg);
      $alg = preg_replace("/<276>/","NB' CF",$alg);
      $alg = preg_replace("/<277>/","NU' CU'",$alg);
      $alg = preg_replace("/<278>/","ND' CU",$alg);
      
      $alg = preg_replace("/<279>/","R' T3L'",$alg);
      $alg = preg_replace("/<280>/","T3R' L'",$alg);
      $alg = preg_replace("/<281>/","F' T3B'",$alg);
      $alg = preg_replace("/<282>/","T3F' B'",$alg);
      $alg = preg_replace("/<283>/","U' T3D'",$alg);
      $alg = preg_replace("/<284>/","T3U' D'",$alg);
      
      
      $alg = preg_replace("/<285>/","T3R' L'",$alg);
      $alg = preg_replace("/<286>/","R' T3L'",$alg);
      $alg = preg_replace("/<287>/","T3F' B'",$alg);
      $alg = preg_replace("/<288>/","F' T3B'",$alg);
      $alg = preg_replace("/<289>/","T3U' D'",$alg);
      $alg = preg_replace("/<290>/","U' T3D'",$alg);
      
      
      $alg = preg_replace("/<291>/","TR' TL'",$alg);
      $alg = preg_replace("/<292>/","TF' TB'",$alg);
      $alg = preg_replace("/<293>/","TU' TD'",$alg);
      
      $alg = preg_replace("/<294>/","MR' CR'",$alg);
      $alg = preg_replace("/<295>/","MR CR",$alg);
      $alg = preg_replace("/<296>/","MF' CF'",$alg);
      $alg = preg_replace("/<297>/","MF CF",$alg);
      $alg = preg_replace("/<298>/","MU' CU'",$alg);
      $alg = preg_replace("/<299>/","MU CU",$alg);
      
      
      $alg = preg_replace("/<2100>/","R' L'",$alg);
      $alg = preg_replace("/<2101>/","F' B'",$alg);
      $alg = preg_replace("/<2102>/","U' D'",$alg);
      
      
      $alg = preg_replace("/<2103>/","R' TL'",$alg);
      $alg = preg_replace("/<2104>/","TR' L'",$alg);
      $alg = preg_replace("/<2105>/","F' TB'",$alg);
      $alg = preg_replace("/<2106>/","TF' B'",$alg);
      $alg = preg_replace("/<2107>/","U' TD'",$alg);
      $alg = preg_replace("/<2108>/","TU' D'",$alg);
    }
  ...
  */
  function alg6xC_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N6R'/","<906>",$alg); $alg = preg_replace("/N6R-/","<906>",$alg);   $alg = preg_replace("/N6R2/","<905>",$alg);   $alg = preg_replace("/N6R/","<904>",$alg);
    $alg = preg_replace("/N6L'/","<903>",$alg); $alg = preg_replace("/N6L-/","<903>",$alg);   $alg = preg_replace("/N6L2/","<902>",$alg);   $alg = preg_replace("/N6L/","<901>",$alg);
    $alg = preg_replace("/N6F'/","<912>",$alg); $alg = preg_replace("/N6F-/","<912>",$alg);   $alg = preg_replace("/N6F2/","<911>",$alg);   $alg = preg_replace("/N6F/","<910>",$alg);
    $alg = preg_replace("/N6B'/","<909>",$alg); $alg = preg_replace("/N6B-/","<909>",$alg);   $alg = preg_replace("/N6B2/","<908>",$alg);   $alg = preg_replace("/N6B/","<907>",$alg);
    $alg = preg_replace("/N6U'/","<918>",$alg); $alg = preg_replace("/N6U-/","<918>",$alg);   $alg = preg_replace("/N6U2/","<917>",$alg);   $alg = preg_replace("/N6U/","<916>",$alg);
    $alg = preg_replace("/N6D'/","<915>",$alg); $alg = preg_replace("/N6D-/","<915>",$alg);   $alg = preg_replace("/N6D2/","<914>",$alg);   $alg = preg_replace("/N6D/","<913>",$alg);
    ...
    $alg = preg_replace("/T2R'/","<355>",$alg); $alg = preg_replace("/T2R-/","<355>",$alg);   $alg = preg_replace("/T2R2/","<356>",$alg);   $alg = preg_replace("/T2R/","<357>",$alg);
    $alg = preg_replace("/T2L'/","<358>",$alg); $alg = preg_replace("/T2L-/","<358>",$alg);   $alg = preg_replace("/T2L2/","<359>",$alg);   $alg = preg_replace("/T2L/","<360>",$alg);
    $alg = preg_replace("/T2F'/","<361>",$alg); $alg = preg_replace("/T2F-/","<361>",$alg);   $alg = preg_replace("/T2F2/","<362>",$alg);   $alg = preg_replace("/T2F/","<363>",$alg);
    $alg = preg_replace("/T2B'/","<364>",$alg); $alg = preg_replace("/T2B-/","<364>",$alg);   $alg = preg_replace("/T2B2/","<365>",$alg);   $alg = preg_replace("/T2B/","<366>",$alg);
    $alg = preg_replace("/T2U'/","<367>",$alg); $alg = preg_replace("/T2U-/","<367>",$alg);   $alg = preg_replace("/T2U2/","<368>",$alg);   $alg = preg_replace("/T2U/","<369>",$alg);
    $alg = preg_replace("/T2D'/","<370>",$alg); $alg = preg_replace("/T2D-/","<370>",$alg);   $alg = preg_replace("/T2D2/","<371>",$alg);   $alg = preg_replace("/T2D/","<372>",$alg);
  ...
  */
  function alg6xC_TwizzleToSse($alg) {
    ...
    $alg = preg_replace("/2-6R'/","<306>",$alg); $alg = preg_replace("/2-6R2/","<305>",$alg); $alg = preg_replace("/2-6R/","<304>",$alg);
    $alg = preg_replace("/2-6L'/","<303>",$alg); $alg = preg_replace("/2-6L2/","<302>",$alg); $alg = preg_replace("/2-6L/","<301>",$alg);
    $alg = preg_replace("/2-6F'/","<312>",$alg); $alg = preg_replace("/2-6F2/","<311>",$alg); $alg = preg_replace("/2-6F/","<310>",$alg);
    $alg = preg_replace("/2-6B'/","<309>",$alg); $alg = preg_replace("/2-6B2/","<308>",$alg); $alg = preg_replace("/2-6B/","<307>",$alg);
    $alg = preg_replace("/2-6U'/","<318>",$alg); $alg = preg_replace("/2-6U2/","<317>",$alg); $alg = preg_replace("/2-6U/","<316>",$alg);
    $alg = preg_replace("/2-6D'/","<315>",$alg); $alg = preg_replace("/2-6D2/","<314>",$alg); $alg = preg_replace("/2-6D/","<313>",$alg);
    ...
    $alg = preg_replace("/3-6R'/","<324>",$alg); $alg = preg_replace("/3-6R2/","<323>",$alg); $alg = preg_replace("/3-6R/","<322>",$alg);
    $alg = preg_replace("/3-6L'/","<321>",$alg); $alg = preg_replace("/3-6L2/","<320>",$alg); $alg = preg_replace("/3-6L/","<319>",$alg);
    $alg = preg_replace("/3-6F'/","<330>",$alg); $alg = preg_replace("/3-6F2/","<329>",$alg); $alg = preg_replace("/3-6F/","<328>",$alg);
    $alg = preg_replace("/3-6B'/","<327>",$alg); $alg = preg_replace("/3-6B2/","<326>",$alg); $alg = preg_replace("/3-6B/","<325>",$alg);
    $alg = preg_replace("/3-6U'/","<336>",$alg); $alg = preg_replace("/3-6U2/","<335>",$alg); $alg = preg_replace("/3-6U/","<334>",$alg);
    $alg = preg_replace("/3-6D'/","<333>",$alg); $alg = preg_replace("/3-6D2/","<332>",$alg); $alg = preg_replace("/3-6D/","<331>",$alg);
    ...
    $alg = preg_replace("/4-6R'/","<342>",$alg); $alg = preg_replace("/4-6R2/","<341>",$alg); $alg = preg_replace("/4-6R/","<340>",$alg);
    $alg = preg_replace("/4-6L'/","<339>",$alg); $alg = preg_replace("/4-6L2/","<338>",$alg); $alg = preg_replace("/4-6L/","<337>",$alg);
    $alg = preg_replace("/4-6F'/","<348>",$alg); $alg = preg_replace("/4-6F2/","<347>",$alg); $alg = preg_replace("/4-6F/","<346>",$alg);
    $alg = preg_replace("/4-6B'/","<345>",$alg); $alg = preg_replace("/4-6B2/","<344>",$alg); $alg = preg_replace("/4-6B/","<343>",$alg);
    $alg = preg_replace("/4-6U'/","<354>",$alg); $alg = preg_replace("/4-6U2/","<353>",$alg); $alg = preg_replace("/4-6U/","<352>",$alg);
    $alg = preg_replace("/4-6D'/","<351>",$alg); $alg = preg_replace("/4-6D2/","<350>",$alg); $alg = preg_replace("/4-6D/","<349>",$alg);
    ...
    $alg = preg_replace("/5-6R'/","<360>",$alg); $alg = preg_replace("/5-6R2/","<359>",$alg); $alg = preg_replace("/5-6R/","<358>",$alg);
    $alg = preg_replace("/5-6L'/","<357>",$alg); $alg = preg_replace("/5-6L2/","<356>",$alg); $alg = preg_replace("/5-6L/","<355>",$alg);
    $alg = preg_replace("/5-6F'/","<366>",$alg); $alg = preg_replace("/5-6F2/","<365>",$alg); $alg = preg_replace("/5-6F/","<364>",$alg);
    $alg = preg_replace("/5-6B'/","<363>",$alg); $alg = preg_replace("/5-6B2/","<362>",$alg); $alg = preg_replace("/5-6B/","<361>",$alg);
    $alg = preg_replace("/5-6U'/","<372>",$alg); $alg = preg_replace("/5-6U2/","<371>",$alg); $alg = preg_replace("/5-6U/","<370>",$alg);
    $alg = preg_replace("/5-6D'/","<369>",$alg); $alg = preg_replace("/5-6D2/","<368>",$alg); $alg = preg_replace("/5-6D/","<367>",$alg);
    
    /* --- 6xC: TWIZZLE -> CODE: [2] Slice twists --- */
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/3-4R' Rv'/","<291>",$alg);
      $alg = preg_replace("/3-4L' Lv'/","<292>",$alg);
      $alg = preg_replace("/3-4F' Fv'/","<293>",$alg);
      $alg = preg_replace("/3-4B' Bv'/","<294>",$alg);
      $alg = preg_replace("/3-4U' Uv'/","<295>",$alg);
      $alg = preg_replace("/3-4D' Dv'/","<296>",$alg);
      
      $alg = preg_replace("/3-4L Rv'/","<291>",$alg);
      $alg = preg_replace("/3-4R Lv'/","<292>",$alg);
      $alg = preg_replace("/3-4B Fv'/","<293>",$alg);
      $alg = preg_replace("/3-4F Bv'/","<294>",$alg);
      $alg = preg_replace("/3-4D Uv'/","<295>",$alg);
      $alg = preg_replace("/3-4U Dv'/","<296>",$alg);
      
      /* S2 = S3-4 */
      $alg = preg_replace("/3-4R Rv'/","<201>",$alg);  $alg = preg_replace("/3-4R2 Rv2/","<202>",$alg); $alg = preg_replace("/3-4R' Rv/","<203>",$alg);
      $alg = preg_replace("/3-4L Lv'/","<203>",$alg);  $alg = preg_replace("/3-4L2 Lv2/","<202>",$alg); $alg = preg_replace("/3-4L' Lv/","<201>",$alg);
      $alg = preg_replace("/3-4F Fv'/","<204>",$alg);  $alg = preg_replace("/3-4F2 Fv2/","<205>",$alg); $alg = preg_replace("/3-4F' Fv/","<206>",$alg);
      $alg = preg_replace("/3-4B Bv'/","<206>",$alg);  $alg = preg_replace("/3-4B2 Bv2/","<205>",$alg); $alg = preg_replace("/3-4B' Bv/","<204>",$alg);
      $alg = preg_replace("/3-4U Uv'/","<207>",$alg);  $alg = preg_replace("/3-4U2 Uv2/","<208>",$alg); $alg = preg_replace("/3-4U' Uv/","<209>",$alg);
      $alg = preg_replace("/3-4D Dv'/","<209>",$alg);  $alg = preg_replace("/3-4D2 Dv2/","<208>",$alg); $alg = preg_replace("/3-4D' Dv/","<207>",$alg);
      
      $alg = preg_replace("/3-4L' Rv'/","<201>",$alg); $alg = preg_replace("/3-4L2 Rv2/","<202>",$alg); $alg = preg_replace("/3-4L Rv/","<203>",$alg);
      $alg = preg_replace("/3-4R' Lv'/","<203>",$alg); $alg = preg_replace("/3-4R2 Lv2/","<202>",$alg); $alg = preg_replace("/3-4R Lv/","<201>",$alg);
      $alg = preg_replace("/3-4B' Fv'/","<204>",$alg); $alg = preg_replace("/3-4B2 Fv2/","<205>",$alg); $alg = preg_replace("/3-4B Fv/","<206>",$alg);
      $alg = preg_replace("/3-4F' Bv'/","<206>",$alg); $alg = preg_replace("/3-4F2 Bv2/","<205>",$alg); $alg = preg_replace("/3-4F Bv/","<204>",$alg);
      $alg = preg_replace("/3-4D' Uv'/","<207>",$alg); $alg = preg_replace("/3-4D2 Uv2/","<208>",$alg); $alg = preg_replace("/3-4D Uv/","<209>",$alg);
      $alg = preg_replace("/3-4U' Dv'/","<209>",$alg); $alg = preg_replace("/3-4U2 Dv2/","<208>",$alg); $alg = preg_replace("/3-4U Dv/","<207>",$alg);
      
      
      /* Non-slice-twists */
      $alg = preg_replace("/2-5R' Rv'/","<297>",$alg);
      $alg = preg_replace("/2-5L' Lv'/","<298>",$alg);
      $alg = preg_replace("/2-5F' Fv'/","<299>",$alg);
      $alg = preg_replace("/2-5B' Bv'/","<2100>",$alg);
      $alg = preg_replace("/2-5U' Uv'/","<2101>",$alg);
      $alg = preg_replace("/2-5D' Dv'/","<2102>",$alg);
      
      $alg = preg_replace("/2-5L Rv'/","<297>",$alg);
      $alg = preg_replace("/2-5R Lv'/","<298>",$alg);
      $alg = preg_replace("/2-5B Fv'/","<299>",$alg);
      $alg = preg_replace("/2-5F Bv'/","<2100>",$alg);
      $alg = preg_replace("/2-5D Uv'/","<2101>",$alg);
      $alg = preg_replace("/2-5U Dv'/","<2102>",$alg);
      
      /* S = S2-5 */
      $alg = preg_replace("/2-5L' Rv'/","<210>",$alg); $alg = preg_replace("/2-5L2 Rv2/","<211>",$alg); $alg = preg_replace("/2-5L Rv/","<212>",$alg);
      $alg = preg_replace("/2-5R' Lv'/","<212>",$alg); $alg = preg_replace("/2-5R2 Lv2/","<211>",$alg); $alg = preg_replace("/2-5R Lv/","<210>",$alg);
      $alg = preg_replace("/2-5B' Fv'/","<213>",$alg); $alg = preg_replace("/2-5B2 Fv2/","<214>",$alg); $alg = preg_replace("/2-5B Fv/","<215>",$alg);
      $alg = preg_replace("/2-5F' Bv'/","<215>",$alg); $alg = preg_replace("/2-5F2 Bv2/","<214>",$alg); $alg = preg_replace("/2-5F Bv/","<213>",$alg);
      $alg = preg_replace("/2-5D' Uv'/","<216>",$alg); $alg = preg_replace("/2-5D2 Uv2/","<217>",$alg); $alg = preg_replace("/2-5D Uv/","<218>",$alg);
      $alg = preg_replace("/2-5U' Dv'/","<218>",$alg); $alg = preg_replace("/2-5U2 Dv2/","<217>",$alg); $alg = preg_replace("/2-5U Dv/","<216>",$alg);
      
      $alg = preg_replace("/2-5R Rv'/","<210>",$alg);  $alg = preg_replace("/2-5R2 Rv2/","<211>",$alg); $alg = preg_replace("/2-5R' Rv/","<212>",$alg);
      $alg = preg_replace("/2-5L Lv'/","<212>",$alg);  $alg = preg_replace("/2-5L2 Lv2/","<211>",$alg); $alg = preg_replace("/2-5L' Lv/","<210>",$alg);
      $alg = preg_replace("/2-5F Fv'/","<213>",$alg);  $alg = preg_replace("/2-5F2 Fv2/","<214>",$alg); $alg = preg_replace("/2-5F' Fv/","<215>",$alg);
      $alg = preg_replace("/2-5B Bv'/","<215>",$alg);  $alg = preg_replace("/2-5B2 Bv2/","<214>",$alg); $alg = preg_replace("/2-5B' Bv/","<213>",$alg);
      $alg = preg_replace("/2-5U Uv'/","<216>",$alg);  $alg = preg_replace("/2-5U2 Uv2/","<217>",$alg); $alg = preg_replace("/2-5U' Uv/","<218>",$alg);
      $alg = preg_replace("/2-5D Dv'/","<218>",$alg);  $alg = preg_replace("/2-5D2 Dv2/","<217>",$alg); $alg = preg_replace("/2-5D' Dv/","<216>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2-3R' Rv'/","<2103>",$alg);
      $alg = preg_replace("/2-3L' Lv'/","<2104>",$alg);
      $alg = preg_replace("/2-3F' Fv'/","<2105>",$alg);
      $alg = preg_replace("/2-3B' Bv'/","<2106>",$alg);
      $alg = preg_replace("/2-3U' Uv'/","<2107>",$alg);
      $alg = preg_replace("/2-3D' Dv'/","<2108>",$alg);
      
      $alg = preg_replace("/4-5L Rv'/","<2103>",$alg);
      $alg = preg_replace("/4-5R Lv'/","<2104>",$alg);
      $alg = preg_replace("/4-5B Fv'/","<2105>",$alg);
      $alg = preg_replace("/4-5F Bv'/","<2106>",$alg);
      $alg = preg_replace("/4-5D Uv'/","<2107>",$alg);
      $alg = preg_replace("/4-5U Dv'/","<2108>",$alg);
      ...
      $alg = preg_replace("/4-5L' Rv'/","<237>",$alg); $alg = preg_replace("/4-5L2 Rv2/","<238>",$alg); $alg = preg_replace("/4-5L Rv/","<239>",$alg);
      $alg = preg_replace("/4-5R' Lv'/","<240>",$alg); $alg = preg_replace("/4-5R2 Lv2/","<241>",$alg); $alg = preg_replace("/4-5R Lv/","<242>",$alg);
      $alg = preg_replace("/4-5B' Fv'/","<243>",$alg); $alg = preg_replace("/4-5B2 Fv2/","<244>",$alg); $alg = preg_replace("/4-5B Fv/","<245>",$alg);
      $alg = preg_replace("/4-5F' Bv'/","<246>",$alg); $alg = preg_replace("/4-5F2 Bv2/","<247>",$alg); $alg = preg_replace("/4-5F Bv/","<248>",$alg);
      $alg = preg_replace("/4-5D' Uv'/","<249>",$alg); $alg = preg_replace("/4-5D2 Uv2/","<250>",$alg); $alg = preg_replace("/4-5D Uv/","<251>",$alg);
      $alg = preg_replace("/4-5U' Dv'/","<252>",$alg); $alg = preg_replace("/4-5U2 Dv2/","<253>",$alg); $alg = preg_replace("/4-5U Dv/","<254>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2R' Rv'/","<2109>",$alg);
      $alg = preg_replace("/2L' Lv'/","<2110>",$alg);
      $alg = preg_replace("/2F' Fv'/","<2111>",$alg);
      $alg = preg_replace("/2B' Bv'/","<2112>",$alg);
      $alg = preg_replace("/2U' Uv'/","<2113>",$alg);
      $alg = preg_replace("/2D' Dv'/","<2114>",$alg);
      
      $alg = preg_replace("/5L Rv'/","<2109>",$alg);
      $alg = preg_replace("/5R Lv'/","<2110>",$alg);
      $alg = preg_replace("/5B Fv'/","<2111>",$alg);
      $alg = preg_replace("/5F Bv'/","<2112>",$alg);
      $alg = preg_replace("/5D Uv'/","<2113>",$alg);
      $alg = preg_replace("/5U Dv'/","<2114>",$alg);
      
/* xxx */
      $alg = preg_replace("/4l' R'/","<2109>",$alg);
      $alg = preg_replace("/4r' L'/","<2110>",$alg);
      $alg = preg_replace("/4b' F'/","<2111>",$alg);
      $alg = preg_replace("/4f' B'/","<2112>",$alg);
      $alg = preg_replace("/4d' U'/","<2113>",$alg);
      $alg = preg_replace("/4u' D'/","<2114>",$alg);
      ...
      $alg = preg_replace("/5L' Rv'/","<219>",$alg); $alg = preg_replace("/5L2 Rv2/","<220>",$alg); $alg = preg_replace("/5L Rv/","<221>",$alg);
      $alg = preg_replace("/5R' Lv'/","<222>",$alg); $alg = preg_replace("/5R2 Lv2/","<223>",$alg); $alg = preg_replace("/5R Lv/","<224>",$alg);
      $alg = preg_replace("/5B' Fv'/","<225>",$alg); $alg = preg_replace("/5B2 Fv2/","<226>",$alg); $alg = preg_replace("/5B Fv/","<227>",$alg);
      $alg = preg_replace("/5F' Bv'/","<228>",$alg); $alg = preg_replace("/5F2 Bv2/","<229>",$alg); $alg = preg_replace("/5F Bv/","<230>",$alg);
      $alg = preg_replace("/5D' Uv'/","<231>",$alg); $alg = preg_replace("/5D2 Uv2/","<232>",$alg); $alg = preg_replace("/5D Uv/","<233>",$alg);
      $alg = preg_replace("/5U' Dv'/","<234>",$alg); $alg = preg_replace("/5U2 Dv2/","<235>",$alg); $alg = preg_replace("/5U Dv/","<236>",$alg);
      
      $alg = preg_replace("/4l R'/","<219>",$alg); $alg = preg_replace("/4l2 R2/","<220>",$alg); $alg = preg_replace("/4l' R/","<221>",$alg);
      $alg = preg_replace("/4r L'/","<222>",$alg); $alg = preg_replace("/4r2 L2/","<223>",$alg); $alg = preg_replace("/4r' L/","<224>",$alg);
      $alg = preg_replace("/4b F'/","<225>",$alg); $alg = preg_replace("/4b2 F2/","<226>",$alg); $alg = preg_replace("/4b' F/","<227>",$alg);
      $alg = preg_replace("/4f B'/","<228>",$alg); $alg = preg_replace("/4f2 B2/","<229>",$alg); $alg = preg_replace("/4f' B/","<230>",$alg);
      $alg = preg_replace("/4d U'/","<231>",$alg); $alg = preg_replace("/4d2 U2/","<232>",$alg); $alg = preg_replace("/4d' U/","<233>",$alg);
      $alg = preg_replace("/4u D'/","<234>",$alg); $alg = preg_replace("/4u2 D2/","<235>",$alg); $alg = preg_replace("/4u' D/","<236>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/3R' Rv'/","<2115>",$alg);
      $alg = preg_replace("/3L' Lv'/","<2116>",$alg);
      $alg = preg_replace("/3F' Fv'/","<2117>",$alg);
      $alg = preg_replace("/3B' Bv'/","<2118>",$alg);
      $alg = preg_replace("/3U' Uv'/","<2119>",$alg);
      $alg = preg_replace("/3D' Dv'/","<2120>",$alg);
      
      $alg = preg_replace("/4L Rv'/","<2115>",$alg);
      $alg = preg_replace("/4R Lv'/","<2116>",$alg);
      $alg = preg_replace("/4B Fv'/","<2117>",$alg);
      $alg = preg_replace("/4F Bv'/","<2118>",$alg);
      $alg = preg_replace("/4D Uv'/","<2119>",$alg);
      $alg = preg_replace("/4U Dv'/","<2120>",$alg);
      
      /* S3-3 | S4-4 */
      $alg = preg_replace("/3R Rv'/","<273>",$alg);  $alg = preg_replace("/3R2 Rv2/","<274>",$alg); $alg = preg_replace("/3R' Rv/","<275>",$alg);
      $alg = preg_replace("/3L Lv'/","<276>",$alg);  $alg = preg_replace("/3L2 Lv2/","<277>",$alg); $alg = preg_replace("/3L' Lv/","<278>",$alg);
      $alg = preg_replace("/3F Fv'/","<279>",$alg);  $alg = preg_replace("/3F2 Fv2/","<280>",$alg); $alg = preg_replace("/3F' Fv/","<281>",$alg);
      $alg = preg_replace("/3B Bv'/","<282>",$alg);  $alg = preg_replace("/3B2 Bv2/","<283>",$alg); $alg = preg_replace("/3B' Bv/","<284>",$alg);
      $alg = preg_replace("/3U Uv'/","<285>",$alg);  $alg = preg_replace("/3U2 Uv2/","<286>",$alg); $alg = preg_replace("/3U' Uv/","<287>",$alg);
      $alg = preg_replace("/3D Dv'/","<288>",$alg);  $alg = preg_replace("/3D2 Dv2/","<289>",$alg); $alg = preg_replace("/3D' Dv/","<290>",$alg);
      
      $alg = preg_replace("/4L' Rv'/","<273>",$alg); $alg = preg_replace("/4L2 Rv2/","<274>",$alg); $alg = preg_replace("/4L Rv/","<275>",$alg);
      $alg = preg_replace("/4R' Lv'/","<276>",$alg); $alg = preg_replace("/4R2 Lv2/","<277>",$alg); $alg = preg_replace("/4R Lv/","<278>",$alg);
      $alg = preg_replace("/4B' Fv'/","<279>",$alg); $alg = preg_replace("/4B2 Fv2/","<280>",$alg); $alg = preg_replace("/4B Fv/","<281>",$alg);
      $alg = preg_replace("/4F' Bv'/","<282>",$alg); $alg = preg_replace("/4F2 Bv2/","<283>",$alg); $alg = preg_replace("/4F Bv/","<284>",$alg);
      $alg = preg_replace("/4D' Uv'/","<285>",$alg); $alg = preg_replace("/4D2 Uv2/","<286>",$alg); $alg = preg_replace("/4D Uv/","<287>",$alg);
      $alg = preg_replace("/4U' Dv'/","<288>",$alg); $alg = preg_replace("/4U2 Dv2/","<289>",$alg); $alg = preg_replace("/4U Dv/","<290>",$alg);
    }
    ...
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/Lw' Rw'/","<2121>",$alg);
      $alg = preg_replace("/Rw' Lw'/","<2121>",$alg);
      $alg = preg_replace("/Bw' Fw'/","<2122>",$alg);
      $alg = preg_replace("/Fw' Bw'/","<2122>",$alg);
      $alg = preg_replace("/Dw' Uw'/","<2123>",$alg);
      $alg = preg_replace("/Uw' Dw'/","<2123>",$alg);
      
      $alg = preg_replace("/l' r'/","<2121>",$alg);
      $alg = preg_replace("/r' l'/","<2121>",$alg);
      $alg = preg_replace("/b' f'/","<2122>",$alg);
      $alg = preg_replace("/f' b'/","<2122>",$alg);
      $alg = preg_replace("/d' u'/","<2123>",$alg);
      $alg = preg_replace("/u' d'/","<2123>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/L' R'/","<2124>",$alg);
      $alg = preg_replace("/R' L'/","<2124>",$alg);
      $alg = preg_replace("/B' F'/","<2125>",$alg);
      $alg = preg_replace("/F' B'/","<2125>",$alg);
      $alg = preg_replace("/D' U'/","<2126>",$alg);
      $alg = preg_replace("/U' D'/","<2126>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/R' 4l'/","<2109>",$alg);
      $alg = preg_replace("/L' 4r'/","<2110>",$alg);
      $alg = preg_replace("/F' 4b'/","<2111>",$alg);
      $alg = preg_replace("/B' 4f'/","<2112>",$alg);
      $alg = preg_replace("/U' 4d'/","<2113>",$alg);
      $alg = preg_replace("/D' 4u'/","<2114>",$alg);
      
      /* S2-2 | S5-5 */
      $alg = preg_replace("/R 4l'/","<221>",$alg); $alg = preg_replace("/R2 4l2/","<220>",$alg); $alg = preg_replace("/R' 4l/","<219>",$alg);
      $alg = preg_replace("/L 4r'/","<224>",$alg); $alg = preg_replace("/L2 4r2/","<223>",$alg); $alg = preg_replace("/L' 4r/","<222>",$alg);
      $alg = preg_replace("/F 4b'/","<227>",$alg); $alg = preg_replace("/F2 4b2/","<226>",$alg); $alg = preg_replace("/F' 4b/","<225>",$alg);
      $alg = preg_replace("/B 4f'/","<230>",$alg); $alg = preg_replace("/B2 4f2/","<229>",$alg); $alg = preg_replace("/B' 4f/","<228>",$alg);
      $alg = preg_replace("/U 4d'/","<233>",$alg); $alg = preg_replace("/U2 4d2/","<232>",$alg); $alg = preg_replace("/U' 4d/","<231>",$alg);
      $alg = preg_replace("/D 4u'/","<236>",$alg); $alg = preg_replace("/D2 4u2/","<235>",$alg); $alg = preg_replace("/D' 4u/","<234>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/L' r'/","<2127>",$alg);
      $alg = preg_replace("/R' l'/","<2128>",$alg);
      $alg = preg_replace("/B' f'/","<2129>",$alg);
      $alg = preg_replace("/F' b'/","<2130>",$alg);
      $alg = preg_replace("/D' u'/","<2131>",$alg);
      $alg = preg_replace("/U' d'/","<2132>",$alg);
      
      $alg = preg_replace("/l' R'/","<2128>",$alg);
      $alg = preg_replace("/r' L'/","<2127>",$alg);
      $alg = preg_replace("/b' F'/","<2130>",$alg);
      $alg = preg_replace("/f' B'/","<2129>",$alg);
      $alg = preg_replace("/d' U'/","<2132>",$alg);
      $alg = preg_replace("/u' D'/","<2131>",$alg);
      
      /* S2-4 | S3-5 */
      $alg = preg_replace("/L r'/","<260>",$alg); $alg = preg_replace("/L2 r2/","<259>",$alg); $alg = preg_replace("/L' r/","<258>",$alg);
      $alg = preg_replace("/R l'/","<257>",$alg); $alg = preg_replace("/R2 l2/","<256>",$alg); $alg = preg_replace("/R' l/","<255>",$alg);
      $alg = preg_replace("/B f'/","<266>",$alg); $alg = preg_replace("/B2 f2/","<265>",$alg); $alg = preg_replace("/B' f/","<264>",$alg);
      $alg = preg_replace("/F b'/","<263>",$alg); $alg = preg_replace("/F2 b2/","<262>",$alg); $alg = preg_replace("/F' b/","<261>",$alg);
      $alg = preg_replace("/D u'/","<272>",$alg); $alg = preg_replace("/D2 u2/","<271>",$alg); $alg = preg_replace("/D' u/","<270>",$alg);
      $alg = preg_replace("/U d'/","<269>",$alg); $alg = preg_replace("/U2 d2/","<268>",$alg); $alg = preg_replace("/U' d/","<267>",$alg);
      
      $alg = preg_replace("/l R'/","<255>",$alg); $alg = preg_replace("/l2 R2/","<256>",$alg); $alg = preg_replace("/l' R/","<257>",$alg);
      $alg = preg_replace("/r L'/","<258>",$alg); $alg = preg_replace("/r2 L2/","<259>",$alg); $alg = preg_replace("/r' L/","<260>",$alg);
      $alg = preg_replace("/b F'/","<261>",$alg); $alg = preg_replace("/b2 F2/","<262>",$alg); $alg = preg_replace("/b' F/","<263>",$alg);
      $alg = preg_replace("/f B'/","<264>",$alg); $alg = preg_replace("/f2 B2/","<265>",$alg); $alg = preg_replace("/f' B/","<266>",$alg);
      $alg = preg_replace("/d U'/","<267>",$alg); $alg = preg_replace("/d2 U2/","<268>",$alg); $alg = preg_replace("/d' U/","<269>",$alg);
      $alg = preg_replace("/u D'/","<270>",$alg); $alg = preg_replace("/u2 D2/","<271>",$alg); $alg = preg_replace("/u' D/","<272>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* S2-2 | S5-5 */
      $alg = preg_replace("/<219>/","S2-2R'",$alg); $alg = preg_replace("/<220>/","S2-2R2",$alg); $alg = preg_replace("/<221>/","S2-2R",$alg);
      $alg = preg_replace("/<222>/","S2-2L'",$alg); $alg = preg_replace("/<223>/","S2-2FL",$alg); $alg = preg_replace("/<224>/","S2-2L",$alg);
      $alg = preg_replace("/<225>/","S2-2F'",$alg); $alg = preg_replace("/<226>/","S2-2F2",$alg); $alg = preg_replace("/<227>/","S2-2F",$alg);
      $alg = preg_replace("/<228>/","S2-2B'",$alg); $alg = preg_replace("/<229>/","S2-2B2",$alg); $alg = preg_replace("/<230>/","S2-2B",$alg);
      $alg = preg_replace("/<231>/","S2-2U'",$alg); $alg = preg_replace("/<232>/","S2-2FU",$alg); $alg = preg_replace("/<233>/","S2-2U",$alg);
      $alg = preg_replace("/<234>/","S2-2D'",$alg); $alg = preg_replace("/<235>/","S2-2D2",$alg); $alg = preg_replace("/<236>/","S2-2D",$alg);
      
      /* S2-3 | S4-5 */
      $alg = preg_replace("/<237>/","S2-3R'",$alg); $alg = preg_replace("/<238>/","S2-3R2",$alg); $alg = preg_replace("/<239>/","S2-3R",$alg);
      $alg = preg_replace("/<240>/","S2-3L'",$alg); $alg = preg_replace("/<241>/","S2-3FL",$alg); $alg = preg_replace("/<242>/","S2-3L",$alg);
      $alg = preg_replace("/<243>/","S2-3F'",$alg); $alg = preg_replace("/<244>/","S2-3F2",$alg); $alg = preg_replace("/<245>/","S2-3F",$alg);
      $alg = preg_replace("/<246>/","S2-3B'",$alg); $alg = preg_replace("/<247>/","S2-3B2",$alg); $alg = preg_replace("/<248>/","S2-3B",$alg);
      $alg = preg_replace("/<249>/","S2-3U'",$alg); $alg = preg_replace("/<250>/","S2-3FU",$alg); $alg = preg_replace("/<251>/","S2-3U",$alg);
      $alg = preg_replace("/<252>/","S2-3D'",$alg); $alg = preg_replace("/<253>/","S2-3D2",$alg); $alg = preg_replace("/<254>/","S2-3D",$alg);
      
      /* S2-4 | S3-5 */
      $alg = preg_replace("/<255>/","S2-4R'",$alg); $alg = preg_replace("/<256>/","S2-4R2",$alg); $alg = preg_replace("/<257>/","S2-4R",$alg);
      $alg = preg_replace("/<258>/","S2-4L'",$alg); $alg = preg_replace("/<259>/","S2-4FL",$alg); $alg = preg_replace("/<260>/","S2-4L",$alg);
      $alg = preg_replace("/<261>/","S2-4F'",$alg); $alg = preg_replace("/<262>/","S2-4F2",$alg); $alg = preg_replace("/<263>/","S2-4F",$alg);
      $alg = preg_replace("/<264>/","S2-4B'",$alg); $alg = preg_replace("/<265>/","S2-4B2",$alg); $alg = preg_replace("/<266>/","S2-4B",$alg);
      $alg = preg_replace("/<267>/","S2-4U'",$alg); $alg = preg_replace("/<268>/","S2-4FU",$alg); $alg = preg_replace("/<269>/","S2-4U",$alg);
      $alg = preg_replace("/<270>/","S2-4D'",$alg); $alg = preg_replace("/<271>/","S2-4D2",$alg); $alg = preg_replace("/<272>/","S2-4D",$alg);
      
      /* S3-3 | S4-4 */
      $alg = preg_replace("/<273>/","S3-3R'",$alg); $alg = preg_replace("/<274>/","S3-3R2",$alg); $alg = preg_replace("/<275>/","S3-3R",$alg);
      $alg = preg_replace("/<276>/","S3-3L'",$alg); $alg = preg_replace("/<277>/","S3-3FL",$alg); $alg = preg_replace("/<278>/","S3-3L",$alg);
      $alg = preg_replace("/<279>/","S3-3F'",$alg); $alg = preg_replace("/<280>/","S3-3F2",$alg); $alg = preg_replace("/<281>/","S3-3F",$alg);
      $alg = preg_replace("/<282>/","S3-3B'",$alg); $alg = preg_replace("/<283>/","S3-3B2",$alg); $alg = preg_replace("/<284>/","S3-3B",$alg);
      $alg = preg_replace("/<285>/","S3-3U'",$alg); $alg = preg_replace("/<286>/","S3-3FU",$alg); $alg = preg_replace("/<287>/","S3-3U",$alg);
      $alg = preg_replace("/<288>/","S3-3D'",$alg); $alg = preg_replace("/<289>/","S3-3D2",$alg); $alg = preg_replace("/<290>/","S3-3D",$alg);
      
      
      /* Non-slice-twists */
      $alg = preg_replace("/<291>/","M2R' CR'",$alg);
      $alg = preg_replace("/<292>/","M2R CR",$alg);
      $alg = preg_replace("/<293>/","M2F' CF'",$alg);
      $alg = preg_replace("/<294>/","M2F CF",$alg);
      $alg = preg_replace("/<295>/","M2U' CU'",$alg);
      $alg = preg_replace("/<296>/","M2U CU",$alg);
      
      
      $alg = preg_replace("/<297>/","WR' CR'",$alg);
      $alg = preg_replace("/<298>/","WR CR",$alg);
      $alg = preg_replace("/<299>/","WF' CF'",$alg);
      $alg = preg_replace("/<2100>/","WF CF",$alg);
      $alg = preg_replace("/<2101>/","WU' CU'",$alg);
      $alg = preg_replace("/<2102>/","WU CU",$alg);
      
      
      $alg = preg_replace("/<2103>/","VR' CR'",$alg);
      $alg = preg_replace("/<2104>/","VL' CR",$alg);
      $alg = preg_replace("/<2105>/","VF' CF'",$alg);
      $alg = preg_replace("/<2106>/","VB' CF",$alg);
      $alg = preg_replace("/<2107>/","VU' CU'",$alg);
      $alg = preg_replace("/<2108>/","VD' CU",$alg);
      
      
      $alg = preg_replace("/<2109>/","NR' CR'",$alg);
      $alg = preg_replace("/<2110>/","NL' CR",$alg);
      $alg = preg_replace("/<2111>/","NF' CF'",$alg);
      $alg = preg_replace("/<2112>/","NB' CF",$alg);
      $alg = preg_replace("/<2113>/","NU' CU'",$alg);
      $alg = preg_replace("/<2114>/","ND' CU",$alg);
      
/* xxx */
      
      
      $alg = preg_replace("/<2115>/","MR' CR'",$alg);
      $alg = preg_replace("/<2116>/","ML' CR",$alg);
      $alg = preg_replace("/<2117>/","MF' CF'",$alg);
      $alg = preg_replace("/<2118>/","MB' CF",$alg);
      $alg = preg_replace("/<2119>/","MU' CU'",$alg);
      $alg = preg_replace("/<2120>/","MD' CU",$alg);
      
      
      $alg = preg_replace("/<2121>/","TR' TL'",$alg);
      $alg = preg_replace("/<2122>/","TF' TB'",$alg);
      $alg = preg_replace("/<2123>/","TU' TD'",$alg);
      
      
      $alg = preg_replace("/<2124>/","R' L'",$alg);
      $alg = preg_replace("/<2125>/","F' B'",$alg);
      $alg = preg_replace("/<2126>/","U' D'",$alg);
      
      
      $alg = preg_replace("/<2127>/","TR' L'",$alg);
      $alg = preg_replace("/<2128>/","R' TL'",$alg);
      $alg = preg_replace("/<2129>/","TF' B'",$alg);
      $alg = preg_replace("/<2130>/","F' TB'",$alg);
      $alg = preg_replace("/<2131>/","TU' D'",$alg);
      $alg = preg_replace("/<2132>/","U' TD'",$alg);
    }
  ...
  */
  function alg7xC_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N7R'/","<906>",$alg); $alg = preg_replace("/N7R-/","<906>",$alg);   $alg = preg_replace("/N7R2/","<905>",$alg);   $alg = preg_replace("/N7R/","<904>",$alg);
    $alg = preg_replace("/N7L'/","<903>",$alg); $alg = preg_replace("/N7L-/","<903>",$alg);   $alg = preg_replace("/N7L2/","<902>",$alg);   $alg = preg_replace("/N7L/","<901>",$alg);
    $alg = preg_replace("/N7F'/","<912>",$alg); $alg = preg_replace("/N7F-/","<912>",$alg);   $alg = preg_replace("/N7F2/","<911>",$alg);   $alg = preg_replace("/N7F/","<910>",$alg);
    $alg = preg_replace("/N7B'/","<909>",$alg); $alg = preg_replace("/N7B-/","<909>",$alg);   $alg = preg_replace("/N7B2/","<908>",$alg);   $alg = preg_replace("/N7B/","<907>",$alg);
    $alg = preg_replace("/N7U'/","<918>",$alg); $alg = preg_replace("/N7U-/","<918>",$alg);   $alg = preg_replace("/N7U2/","<917>",$alg);   $alg = preg_replace("/N7U/","<916>",$alg);
    $alg = preg_replace("/N7D'/","<915>",$alg); $alg = preg_replace("/N7D-/","<915>",$alg);   $alg = preg_replace("/N7D2/","<914>",$alg);   $alg = preg_replace("/N7D/","<913>",$alg);
    ...
    $alg = preg_replace("/T2R'/","<373>",$alg); $alg = preg_replace("/T2R-/","<373>",$alg);   $alg = preg_replace("/T2R2/","<374>",$alg);   $alg = preg_replace("/T2R/","<375>",$alg);
    $alg = preg_replace("/T2L'/","<376>",$alg); $alg = preg_replace("/T2L-/","<376>",$alg);   $alg = preg_replace("/T2L2/","<377>",$alg);   $alg = preg_replace("/T2L/","<378>",$alg);
    $alg = preg_replace("/T2F'/","<379>",$alg); $alg = preg_replace("/T2F-/","<379>",$alg);   $alg = preg_replace("/T2F2/","<380>",$alg);   $alg = preg_replace("/T2F/","<381>",$alg);
    $alg = preg_replace("/T2B'/","<382>",$alg); $alg = preg_replace("/T2B-/","<382>",$alg);   $alg = preg_replace("/T2B2/","<383>",$alg);   $alg = preg_replace("/T2B/","<384>",$alg);
    $alg = preg_replace("/T2U'/","<385>",$alg); $alg = preg_replace("/T2U-/","<385>",$alg);   $alg = preg_replace("/T2U2/","<386>",$alg);   $alg = preg_replace("/T2U/","<387>",$alg);
    $alg = preg_replace("/T2D'/","<388>",$alg); $alg = preg_replace("/T2D-/","<388>",$alg);   $alg = preg_replace("/T2D2/","<389>",$alg);   $alg = preg_replace("/T2D/","<390>",$alg);
  ...
  */
  function alg7xC_TwizzleToSse($alg) {
    ...
    $alg = preg_replace("/2-7R'/","<306>",$alg); $alg = preg_replace("/2-7R2/","<305>",$alg); $alg = preg_replace("/2-7R/","<304>",$alg);
    $alg = preg_replace("/2-7L'/","<303>",$alg); $alg = preg_replace("/2-7L2/","<302>",$alg); $alg = preg_replace("/2-7L/","<301>",$alg);
    $alg = preg_replace("/2-7F'/","<312>",$alg); $alg = preg_replace("/2-7F2/","<311>",$alg); $alg = preg_replace("/2-7F/","<310>",$alg);
    $alg = preg_replace("/2-7B'/","<309>",$alg); $alg = preg_replace("/2-7B2/","<308>",$alg); $alg = preg_replace("/2-7B/","<307>",$alg);
    $alg = preg_replace("/2-7U'/","<318>",$alg); $alg = preg_replace("/2-7U2/","<317>",$alg); $alg = preg_replace("/2-7U/","<316>",$alg);
    $alg = preg_replace("/2-7D'/","<315>",$alg); $alg = preg_replace("/2-7D2/","<314>",$alg); $alg = preg_replace("/2-7D/","<313>",$alg);
    ...
    $alg = preg_replace("/3-7R'/","<324>",$alg); $alg = preg_replace("/3-7R2/","<323>",$alg); $alg = preg_replace("/3-7R/","<322>",$alg);
    $alg = preg_replace("/3-7L'/","<321>",$alg); $alg = preg_replace("/3-7L2/","<320>",$alg); $alg = preg_replace("/3-7L/","<319>",$alg);
    $alg = preg_replace("/3-7F'/","<330>",$alg); $alg = preg_replace("/3-7F2/","<329>",$alg); $alg = preg_replace("/3-7F/","<328>",$alg);
    $alg = preg_replace("/3-7B'/","<327>",$alg); $alg = preg_replace("/3-7B2/","<326>",$alg); $alg = preg_replace("/3-7B/","<325>",$alg);
    $alg = preg_replace("/3-7U'/","<336>",$alg); $alg = preg_replace("/3-7U2/","<335>",$alg); $alg = preg_replace("/3-7U/","<334>",$alg);
    $alg = preg_replace("/3-7D'/","<333>",$alg); $alg = preg_replace("/3-7D2/","<332>",$alg); $alg = preg_replace("/3-7D/","<331>",$alg);
    ...
    $alg = preg_replace("/4-7R'/","<342>",$alg); $alg = preg_replace("/4-7R2/","<341>",$alg); $alg = preg_replace("/4-7R/","<340>",$alg);
    $alg = preg_replace("/4-7L'/","<339>",$alg); $alg = preg_replace("/4-7L2/","<338>",$alg); $alg = preg_replace("/4-7L/","<337>",$alg);
    $alg = preg_replace("/4-7F'/","<348>",$alg); $alg = preg_replace("/4-7F2/","<347>",$alg); $alg = preg_replace("/4-7F/","<346>",$alg);
    $alg = preg_replace("/4-7B'/","<345>",$alg); $alg = preg_replace("/4-7B2/","<344>",$alg); $alg = preg_replace("/4-7B/","<343>",$alg);
    $alg = preg_replace("/4-7U'/","<354>",$alg); $alg = preg_replace("/4-7U2/","<353>",$alg); $alg = preg_replace("/4-7U/","<352>",$alg);
    $alg = preg_replace("/4-7D'/","<351>",$alg); $alg = preg_replace("/4-7D2/","<350>",$alg); $alg = preg_replace("/4-7D/","<349>",$alg);
    ...
    $alg = preg_replace("/5-7R'/","<360>",$alg); $alg = preg_replace("/5-7R2/","<359>",$alg); $alg = preg_replace("/5-7R/","<358>",$alg);
    $alg = preg_replace("/5-7L'/","<357>",$alg); $alg = preg_replace("/5-7L2/","<356>",$alg); $alg = preg_replace("/5-7L/","<355>",$alg);
    $alg = preg_replace("/5-7F'/","<366>",$alg); $alg = preg_replace("/5-7F2/","<365>",$alg); $alg = preg_replace("/5-7F/","<364>",$alg);
    $alg = preg_replace("/5-7B'/","<363>",$alg); $alg = preg_replace("/5-7B2/","<362>",$alg); $alg = preg_replace("/5-7B/","<361>",$alg);
    $alg = preg_replace("/5-7U'/","<372>",$alg); $alg = preg_replace("/5-7U2/","<371>",$alg); $alg = preg_replace("/5-7U/","<370>",$alg);
    $alg = preg_replace("/5-7D'/","<369>",$alg); $alg = preg_replace("/5-7D2/","<368>",$alg); $alg = preg_replace("/5-7D/","<367>",$alg);
    ...
    $alg = preg_replace("/6-7R'/","<378>",$alg); $alg = preg_replace("/6-7R2/","<377>",$alg); $alg = preg_replace("/6-7R/","<376>",$alg);
    $alg = preg_replace("/6-7L'/","<375>",$alg); $alg = preg_replace("/6-7L2/","<374>",$alg); $alg = preg_replace("/6-7L/","<373>",$alg);
    $alg = preg_replace("/6-7F'/","<384>",$alg); $alg = preg_replace("/6-7F2/","<383>",$alg); $alg = preg_replace("/6-7F/","<382>",$alg);
    $alg = preg_replace("/6-7B'/","<381>",$alg); $alg = preg_replace("/6-7B2/","<380>",$alg); $alg = preg_replace("/6-7B/","<379>",$alg);
    $alg = preg_replace("/6-7U'/","<390>",$alg); $alg = preg_replace("/6-7U2/","<389>",$alg); $alg = preg_replace("/6-7U/","<388>",$alg);
    $alg = preg_replace("/6-7D'/","<387>",$alg); $alg = preg_replace("/6-7D2/","<386>",$alg); $alg = preg_replace("/6-7D/","<385>",$alg);
    
    /* --- 7xC: TWIZZLE -> CODE: [2] Slice twists --- */
    if ($optSSE == true) {
      /* Non-slice-twists */
      $alg = preg_replace("/3Rw' 3Lw'/","<2101>",$alg);
      $alg = preg_replace("/3Lw' 3Rw'/","<2101>",$alg);
      $alg = preg_replace("/3Fw' 3Bw'/","<2102>",$alg);
      $alg = preg_replace("/3Bw' 3Fw'/","<2102>",$alg);
      $alg = preg_replace("/3Uw' 3Dw'/","<2103>",$alg);
      $alg = preg_replace("/3Dw' 3Uw'/","<2103>",$alg);
      
      $alg = preg_replace("/3r' 3l'/","<2101>",$alg);
      $alg = preg_replace("/3l' 3r'/","<2101>",$alg);
      $alg = preg_replace("/3f' 3b'/","<2102>",$alg);
      $alg = preg_replace("/3b' 3f'/","<2102>",$alg);
      $alg = preg_replace("/3u' 3d'/","<2103>",$alg);
      $alg = preg_replace("/3d' 3u'/","<2103>",$alg);
      
      $alg = preg_replace("/4R' Rv'/","<2104>",$alg);
      $alg = preg_replace("/4L' Lv'/","<2105>",$alg);
      $alg = preg_replace("/4F' Fv'/","<2106>",$alg);
      $alg = preg_replace("/4B' Bv'/","<2107>",$alg);
      $alg = preg_replace("/4U' Uv'/","<2108>",$alg);
      $alg = preg_replace("/4D' Dv'/","<2109>",$alg);
      
      $alg = preg_replace("/4L Rv'/","<2104>",$alg);
      $alg = preg_replace("/4R Lv'/","<2105>",$alg);
      $alg = preg_replace("/4B Fv'/","<2106>",$alg);
      $alg = preg_replace("/4F Bv'/","<2107>",$alg);
      $alg = preg_replace("/4D Uv'/","<2108>",$alg);
      $alg = preg_replace("/4U Dv'/","<2109>",$alg);
      
      $alg = preg_replace("/M Rv'/", "<2104>",$alg);
      $alg = preg_replace("/M' Lv'/","<2105>",$alg);
      $alg = preg_replace("/S' Fv'/","<2106>",$alg);
      $alg = preg_replace("/S Bv'/", "<2107>",$alg);
      $alg = preg_replace("/E Uv'/", "<2108>",$alg);
      $alg = preg_replace("/E' Dv'/","<2109>",$alg);
      
      /* S3 = S4-4 */
      $alg = preg_replace("/3Rw 3Lw'/","<203>",$alg); $alg = preg_replace("/3Rw2 3Lw2/","<202>",$alg); $alg = preg_replace("/3Rw' 3Lw/","<201>",$alg);
      $alg = preg_replace("/3Lw 3Rw'/","<201>",$alg); $alg = preg_replace("/3Lw2 3Rw2/","<202>",$alg); $alg = preg_replace("/3Lw' 3Rw/","<203>",$alg);
      $alg = preg_replace("/3Fw 3Bw'/","<206>",$alg); $alg = preg_replace("/3Fw2 3Bw2/","<205>",$alg); $alg = preg_replace("/3Fw' 3Bw/","<204>",$alg);
      $alg = preg_replace("/3Bw 3Fw'/","<204>",$alg); $alg = preg_replace("/3Bw2 3Fw2/","<205>",$alg); $alg = preg_replace("/3Bw' 3Fw/","<206>",$alg);
      $alg = preg_replace("/3Uw 3Dw'/","<209>",$alg); $alg = preg_replace("/3Uw2 3Dw2/","<208>",$alg); $alg = preg_replace("/3Uw' 3Dw/","<207>",$alg);
      $alg = preg_replace("/3Dw 3Uw'/","<207>",$alg); $alg = preg_replace("/3Dw2 3Uw2/","<208>",$alg); $alg = preg_replace("/3Dw' 3Uw/","<209>",$alg);
      
      $alg = preg_replace("/3r 3l'/","<203>",$alg);   $alg = preg_replace("/3r2 3l2/","<202>",$alg);   $alg = preg_replace("/3r' 3l/","<201>",$alg);
      $alg = preg_replace("/3l 3r'/","<201>",$alg);   $alg = preg_replace("/3l2 3r2/","<202>",$alg);   $alg = preg_replace("/3l' 3r/","<203>",$alg);
      $alg = preg_replace("/3f 3b'/","<206>",$alg);   $alg = preg_replace("/3f2 3b2/","<205>",$alg);   $alg = preg_replace("/3f' 3b/","<204>",$alg);
      $alg = preg_replace("/3b 3f'/","<204>",$alg);   $alg = preg_replace("/3b2 3f2/","<205>",$alg);   $alg = preg_replace("/3b' 3f/","<206>",$alg);
      $alg = preg_replace("/3u 3d'/","<209>",$alg);   $alg = preg_replace("/3u2 3d2/","<208>",$alg);   $alg = preg_replace("/3u' 3d/","<207>",$alg);
      $alg = preg_replace("/3d 3u'/","<207>",$alg);   $alg = preg_replace("/3d2 3u2/","<208>",$alg);   $alg = preg_replace("/3d' 3u/","<209>",$alg);
      
      $alg = preg_replace("/4R Rv'/","<201>",$alg);   $alg = preg_replace("/4R2 Rv2/","<202>",$alg);   $alg = preg_replace("/4R' Rv/","<203>",$alg);
      $alg = preg_replace("/4L Lv'/","<203>",$alg);   $alg = preg_replace("/4L2 Lv2/","<202>",$alg);   $alg = preg_replace("/4L' Lv/","<201>",$alg);
      $alg = preg_replace("/4F Fv'/","<204>",$alg);   $alg = preg_replace("/4F2 Fv2/","<205>",$alg);   $alg = preg_replace("/4F' Fv/","<206>",$alg);
      $alg = preg_replace("/4B Bv'/","<206>",$alg);   $alg = preg_replace("/4B2 Bv2/","<205>",$alg);   $alg = preg_replace("/4B' Bv/","<204>",$alg);
      $alg = preg_replace("/4U Uv'/","<207>",$alg);   $alg = preg_replace("/4U2 Uv2/","<208>",$alg);   $alg = preg_replace("/4U' Uv/","<209>",$alg);
      $alg = preg_replace("/4D Dv'/","<209>",$alg);   $alg = preg_replace("/4D2 Dv2/","<208>",$alg);   $alg = preg_replace("/4D' Dv/","<207>",$alg);
      
      $alg = preg_replace("/4L' Rv'/","<201>",$alg);  $alg = preg_replace("/4L2 Rv2/","<202>",$alg);   $alg = preg_replace("/4L Rv/","<203>",$alg);
      $alg = preg_replace("/4R' Lv'/","<203>",$alg);  $alg = preg_replace("/4R2 Lv2/","<202>",$alg);   $alg = preg_replace("/4R Lv/","<201>",$alg);
      $alg = preg_replace("/4B' Fv'/","<204>",$alg);  $alg = preg_replace("/4B2 Fv2/","<205>",$alg);   $alg = preg_replace("/4B Fv/","<206>",$alg);
      $alg = preg_replace("/4F' Bv'/","<206>",$alg);  $alg = preg_replace("/4F2 Bv2/","<205>",$alg);   $alg = preg_replace("/4F Bv/","<204>",$alg);
      $alg = preg_replace("/4D' Uv'/","<207>",$alg);  $alg = preg_replace("/4D2 Uv2/","<208>",$alg);   $alg = preg_replace("/4D Uv/","<209>",$alg);
      $alg = preg_replace("/4U' Dv'/","<209>",$alg);  $alg = preg_replace("/4U2 Dv2/","<208>",$alg);   $alg = preg_replace("/4U Dv/","<207>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/3-5R' Rv'/","<2110>",$alg);
      $alg = preg_replace("/3-5L' Lv'/","<2111>",$alg);
      $alg = preg_replace("/3-5F' Fv'/","<2112>",$alg);
      $alg = preg_replace("/3-5B' Bv'/","<2113>",$alg);
      $alg = preg_replace("/3-5U' Uv'/","<2114>",$alg);
      $alg = preg_replace("/3-5D' Dv'/","<2115>",$alg);
      
      $alg = preg_replace("/3-5L Rv'/","<2110>",$alg);
      $alg = preg_replace("/3-5R Lv'/","<2111>",$alg);
      $alg = preg_replace("/3-5B Fv'/","<2112>",$alg);
      $alg = preg_replace("/3-5F Bv'/","<2113>",$alg);
      $alg = preg_replace("/3-5D Uv'/","<2114>",$alg);
      $alg = preg_replace("/3-5U Dv'/","<2115>",$alg);
      
      /* S2 = S3-5 */
      $alg = preg_replace("/3-5R Rv'/","<210>",$alg);  $alg = preg_replace("/3-5R2 Rv2/","<211>",$alg); $alg = preg_replace("/3-5R' Rv/","<212>",$alg);
      $alg = preg_replace("/3-5L Lv'/","<212>",$alg);  $alg = preg_replace("/3-5L2 Lv2/","<211>",$alg); $alg = preg_replace("/3-5L' Lv/","<210>",$alg);
      $alg = preg_replace("/3-5F Fv'/","<213>",$alg);  $alg = preg_replace("/3-5F2 Fv2/","<214>",$alg); $alg = preg_replace("/3-5F' Fv/","<215>",$alg);
      $alg = preg_replace("/3-5B Bv'/","<215>",$alg);  $alg = preg_replace("/3-5B2 Bv2/","<214>",$alg); $alg = preg_replace("/3-5B' Bv/","<213>",$alg);
      $alg = preg_replace("/3-5U Uv'/","<216>",$alg);  $alg = preg_replace("/3-5U2 Uv2/","<217>",$alg); $alg = preg_replace("/3-5U' Uv/","<218>",$alg);
      $alg = preg_replace("/3-5D Dv'/","<218>",$alg);  $alg = preg_replace("/3-5D2 Dv2/","<217>",$alg); $alg = preg_replace("/3-5D' Dv/","<216>",$alg);
      
      $alg = preg_replace("/3-5L' Rv'/","<210>",$alg); $alg = preg_replace("/3-5L2 Rv2/","<211>",$alg); $alg = preg_replace("/3-5L Rv/","<212>",$alg);
      $alg = preg_replace("/3-5R' Lv'/","<212>",$alg); $alg = preg_replace("/3-5R2 Lv2/","<211>",$alg); $alg = preg_replace("/3-5R Lv/","<210>",$alg);
      $alg = preg_replace("/3-5B' Fv'/","<213>",$alg); $alg = preg_replace("/3-5B2 Fv2/","<214>",$alg); $alg = preg_replace("/3-5B Fv/","<215>",$alg);
      $alg = preg_replace("/3-5F' Bv'/","<215>",$alg); $alg = preg_replace("/3-5F2 Bv2/","<214>",$alg); $alg = preg_replace("/3-5F Bv/","<213>",$alg);
      $alg = preg_replace("/3-5D' Uv'/","<216>",$alg); $alg = preg_replace("/3-5D2 Uv2/","<217>",$alg); $alg = preg_replace("/3-5D Uv/","<218>",$alg);
      $alg = preg_replace("/3-5U' Dv'/","<218>",$alg); $alg = preg_replace("/3-5U2 Dv2/","<217>",$alg); $alg = preg_replace("/3-5U Dv/","<216>",$alg);
      
      
      /* Non-slice-twists */
      $alg = preg_replace("/2-6R' Rv'/","<2116>",$alg);
      $alg = preg_replace("/2-6L' Lv'/","<2117>",$alg);
      $alg = preg_replace("/2-6F' Fv'/","<2118>",$alg);
      $alg = preg_replace("/2-6B' Bv'/","<2119>",$alg);
      $alg = preg_replace("/2-6U' Uv'/","<2120>",$alg);
      $alg = preg_replace("/2-6D' Dv'/","<2121>",$alg);
      
      $alg = preg_replace("/2-6L Rv'/","<2116>",$alg);
      $alg = preg_replace("/2-6R Lv'/","<2117>",$alg);
      $alg = preg_replace("/2-6B Fv'/","<2118>",$alg);
      $alg = preg_replace("/2-6F Bv'/","<2119>",$alg);
      $alg = preg_replace("/2-6D Uv'/","<2120>",$alg);
      $alg = preg_replace("/2-6U Dv'/","<2121>",$alg);
      
      /* S = S2-6 */
      $alg = preg_replace("/2-6R Rv'/","<219>",$alg);  $alg = preg_replace("/2-6R2 Rv2/","<220>",$alg); $alg = preg_replace("/2-6R' Rv/","<221>",$alg);
      $alg = preg_replace("/2-6L Lv'/","<221>",$alg);  $alg = preg_replace("/2-6L2 Lv2/","<220>",$alg); $alg = preg_replace("/2-6L' Lv/","<219>",$alg);
      $alg = preg_replace("/2-6F Fv'/","<222>",$alg);  $alg = preg_replace("/2-6F2 Fv2/","<223>",$alg); $alg = preg_replace("/2-6F' Fv/","<224>",$alg);
      $alg = preg_replace("/2-6B Bv'/","<224>",$alg);  $alg = preg_replace("/2-6B2 Bv2/","<223>",$alg); $alg = preg_replace("/2-6B' Bv/","<222>",$alg);
      $alg = preg_replace("/2-6U Uv'/","<225>",$alg);  $alg = preg_replace("/2-6U2 Uv2/","<226>",$alg); $alg = preg_replace("/2-6U' Uv/","<227>",$alg);
      $alg = preg_replace("/2-6D Dv'/","<227>",$alg);  $alg = preg_replace("/2-6D2 Dv2/","<226>",$alg); $alg = preg_replace("/2-6D' Dv/","<229>",$alg);
      
      $alg = preg_replace("/2-6L' Rv'/","<219>",$alg); $alg = preg_replace("/2-6L2 Rv2/","<220>",$alg); $alg = preg_replace("/2-6L Rv/","<221>",$alg);
      $alg = preg_replace("/2-6R' Lv'/","<221>",$alg); $alg = preg_replace("/2-6R2 Lv2/","<220>",$alg); $alg = preg_replace("/2-6R Lv/","<219>",$alg);
      $alg = preg_replace("/2-6B' Fv'/","<222>",$alg); $alg = preg_replace("/2-6B2 Fv2/","<223>",$alg); $alg = preg_replace("/2-6B Fv/","<224>",$alg);
      $alg = preg_replace("/2-6F' Bv'/","<224>",$alg); $alg = preg_replace("/2-6F2 Bv2/","<223>",$alg); $alg = preg_replace("/2-6F Bv/","<222>",$alg);
      $alg = preg_replace("/2-6D' Uv'/","<225>",$alg); $alg = preg_replace("/2-6D2 Uv2/","<226>",$alg); $alg = preg_replace("/2-6D Uv/","<227>",$alg);
      $alg = preg_replace("/2-6U' Dv'/","<227>",$alg); $alg = preg_replace("/2-6U2 Dv2/","<226>",$alg); $alg = preg_replace("/2-6U Dv/","<225>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2-3R' Rv'/","<2128>",$alg);
      $alg = preg_replace("/2-3L' Lv'/","<2129>",$alg);
      $alg = preg_replace("/2-3F' Fv'/","<2130>",$alg);
      $alg = preg_replace("/2-3B' Bv'/","<2131>",$alg);
      $alg = preg_replace("/2-3U' Uv'/","<2132>",$alg);
      $alg = preg_replace("/2-3D' Dv'/","<2133>",$alg);
      
      $alg = preg_replace("/5-6L Rv'/","<2128>",$alg);
      $alg = preg_replace("/5-6R Lv'/","<2129>",$alg);
      $alg = preg_replace("/5-6B Fv'/","<2130>",$alg);
      $alg = preg_replace("/5-6F Bv'/","<2131>",$alg);
      $alg = preg_replace("/5-6D Uv'/","<2132>",$alg);
      $alg = preg_replace("/5-6U Dv'/","<2133>",$alg);
      
      /* S2-3 | S5-6 */
      $alg = preg_replace("/2-3R Rv'/","<246>",$alg); $alg = preg_replace("/2-3R2 Rv2/","<247>",$alg); $alg = preg_replace("/2-3R' Rv/","<248>",$alg);
      $alg = preg_replace("/2-3L Lv'/","<249>",$alg); $alg = preg_replace("/2-3L2 Lv2/","<250>",$alg); $alg = preg_replace("/2-3L' Lv/","<251>",$alg);
      $alg = preg_replace("/2-3F Fv'/","<252>",$alg); $alg = preg_replace("/2-3F2 Fv2/","<253>",$alg); $alg = preg_replace("/2-3F' Fv/","<254>",$alg);
      $alg = preg_replace("/2-3B Bv'/","<255>",$alg); $alg = preg_replace("/2-3B2 Bv2/","<256>",$alg); $alg = preg_replace("/2-3B' Bv/","<257>",$alg);
      $alg = preg_replace("/2-3U Uv'/","<258>",$alg); $alg = preg_replace("/2-3U2 Uv2/","<259>",$alg); $alg = preg_replace("/2-3U' Uv/","<260>",$alg);
      $alg = preg_replace("/2-3D Dv'/","<261>",$alg); $alg = preg_replace("/2-3D2 Dv2/","<262>",$alg); $alg = preg_replace("/2-3D' Dv/","<263>",$alg);
      
      $alg = preg_replace("/5-6L' Rv'/","<246>",$alg); $alg = preg_replace("/5-6L2 Rv2/","<247>",$alg); $alg = preg_replace("/5-6L Rv/","<248>",$alg);
      $alg = preg_replace("/5-6R' Lv'/","<249>",$alg); $alg = preg_replace("/5-6R2 Lv2/","<250>",$alg); $alg = preg_replace("/5-6R Lv/","<251>",$alg);
      $alg = preg_replace("/5-6B' Fv'/","<252>",$alg); $alg = preg_replace("/5-6B2 Fv2/","<253>",$alg); $alg = preg_replace("/5-6B Fv/","<254>",$alg);
      $alg = preg_replace("/5-6F' Bv'/","<255>",$alg); $alg = preg_replace("/5-6F2 Bv2/","<256>",$alg); $alg = preg_replace("/5-6F Bv/","<257>",$alg);
      $alg = preg_replace("/5-6D' Uv'/","<258>",$alg); $alg = preg_replace("/5-6D2 Uv2/","<259>",$alg); $alg = preg_replace("/5-6D Uv/","<260>",$alg);
      $alg = preg_replace("/5-6U' Dv'/","<261>",$alg); $alg = preg_replace("/5-6U2 Dv2/","<262>",$alg); $alg = preg_replace("/5-6U Dv/","<263>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/2R' Rv'/","<2122>",$alg);
      $alg = preg_replace("/2L' Lv'/","<2123>",$alg);
      $alg = preg_replace("/2F' Fv'/","<2124>",$alg);
      $alg = preg_replace("/2B' Bv'/","<2125>",$alg);
      $alg = preg_replace("/2U' Uv'/","<2126>",$alg);
      $alg = preg_replace("/2D' Dv'/","<2127>",$alg);
      
      $alg = preg_replace("/6L Rv'/","<2122>",$alg);
      $alg = preg_replace("/6R Lv'/","<2123>",$alg);
      $alg = preg_replace("/6B Fv'/","<2124>",$alg);
      $alg = preg_replace("/6F Bv'/","<2125>",$alg);
      $alg = preg_replace("/6D Uv'/","<2126>",$alg);
      $alg = preg_replace("/6U Dv'/","<2127>",$alg);
      
      /* S2-2 | S6-6 */
      $alg = preg_replace("/2R Rv'/","<228>",$alg);  $alg = preg_replace("/2R2 Rv2/","<229>",$alg); $alg = preg_replace("/2R' Rv/","<230>",$alg);
      $alg = preg_replace("/2L Lv'/","<231>",$alg);  $alg = preg_replace("/2L2 Lv2/","<232>",$alg); $alg = preg_replace("/2L' Lv/","<233>",$alg);
      $alg = preg_replace("/2F Fv'/","<234>",$alg);  $alg = preg_replace("/2F2 Fv2/","<235>",$alg); $alg = preg_replace("/2F' Fv/","<236>",$alg);
      $alg = preg_replace("/2B Bv'/","<237>",$alg);  $alg = preg_replace("/2B2 Bv2/","<238>",$alg); $alg = preg_replace("/2B' Bv/","<239>",$alg);
      $alg = preg_replace("/2U Uv'/","<240>",$alg);  $alg = preg_replace("/2U2 Uv2/","<241>",$alg); $alg = preg_replace("/2U' Uv/","<242>",$alg);
      $alg = preg_replace("/2D Dv'/","<243>",$alg);  $alg = preg_replace("/2D2 Dv2/","<244>",$alg); $alg = preg_replace("/2D' Dv/","<245>",$alg);
      
      $alg = preg_replace("/6L' Rv'/","<228>",$alg); $alg = preg_replace("/6L2 Rv2/","<229>",$alg); $alg = preg_replace("/6L Rv/","<230>",$alg);
      $alg = preg_replace("/6R' Lv'/","<231>",$alg); $alg = preg_replace("/6R2 Lv2/","<232>",$alg); $alg = preg_replace("/6R Lv/","<233>",$alg);
      $alg = preg_replace("/6B' Fv'/","<234>",$alg); $alg = preg_replace("/6B2 Fv2/","<235>",$alg); $alg = preg_replace("/6B Fv/","<236>",$alg);
      $alg = preg_replace("/6F' Bv'/","<237>",$alg); $alg = preg_replace("/6F2 Bv2/","<238>",$alg); $alg = preg_replace("/6F Bv/","<239>",$alg);
      $alg = preg_replace("/6D' Uv'/","<240>",$alg); $alg = preg_replace("/6D2 Uv2/","<241>",$alg); $alg = preg_replace("/6D Uv/","<242>",$alg);
      $alg = preg_replace("/6U' Dv'/","<243>",$alg); $alg = preg_replace("/6U2 Dv2/","<244>",$alg); $alg = preg_replace("/6U Dv/","<245>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* S2 = S3-5 */
      $alg = preg_replace("/Rw Lw'/","<212>",$alg); $alg = preg_replace("/Rw2 Lw2/","<211>",$alg); $alg = preg_replace("/Rw' Lw/","<210>",$alg);
      $alg = preg_replace("/Lw Rw'/","<210>",$alg); $alg = preg_replace("/Lw2 Rw2/","<211>",$alg); $alg = preg_replace("/Lw' Rw/","<212>",$alg);
      $alg = preg_replace("/Fw Bw'/","<215>",$alg); $alg = preg_replace("/Fw2 Bw2/","<214>",$alg); $alg = preg_replace("/Fw' Bw/","<213>",$alg);
      $alg = preg_replace("/Bw Fw'/","<213>",$alg); $alg = preg_replace("/Bw2 Fw2/","<214>",$alg); $alg = preg_replace("/Bw' Fw/","<215>",$alg);
      $alg = preg_replace("/Uw Dw'/","<218>",$alg); $alg = preg_replace("/Uw2 Dw2/","<217>",$alg); $alg = preg_replace("/Uw' Dw/","<216>",$alg);
      $alg = preg_replace("/Dw Uw'/","<216>",$alg); $alg = preg_replace("/Dw2 Uw2/","<217>",$alg); $alg = preg_replace("/Dw' Uw/","<218>",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/R' L'/","<2201>",$alg);
      $alg = preg_replace("/L' R'/","<2201>",$alg);
      $alg = preg_replace("/F' B'/","<2202>",$alg);
      $alg = preg_replace("/B' F'/","<2202>",$alg);
      $alg = preg_replace("/U' D'/","<2203>",$alg);
      $alg = preg_replace("/D' U'/","<2203>",$alg);
      
      /* S = S2-6 */
      $alg = preg_replace("/R L'/","<221>",$alg); $alg = preg_replace("/R2 L2/","<220>",$alg); $alg = preg_replace("/R' L/","<219>",$alg);
      $alg = preg_replace("/L R'/","<219>",$alg); $alg = preg_replace("/L2 R2/","<220>",$alg); $alg = preg_replace("/L' R/","<221>",$alg);
      $alg = preg_replace("/F B'/","<224>",$alg); $alg = preg_replace("/F2 B2/","<223>",$alg); $alg = preg_replace("/F' B/","<222>",$alg);
      $alg = preg_replace("/B F'/","<222>",$alg); $alg = preg_replace("/B2 F2/","<223>",$alg); $alg = preg_replace("/B' F/","<224>",$alg);
      $alg = preg_replace("/U D'/","<227>",$alg); $alg = preg_replace("/U2 D2/","<226>",$alg); $alg = preg_replace("/U' D/","<225>",$alg);
      $alg = preg_replace("/D U'/","<225>",$alg); $alg = preg_replace("/D2 U2/","<226>",$alg); $alg = preg_replace("/D' U/","<227>",$alg);
    ...
    if ($optSSE == true) {
      ...
      /* S2-2 | S6-6 */
      $alg = preg_replace("/<228>/","S2-2R'",$alg); $alg = preg_replace("/<229>/","S2-2R2",$alg); $alg = preg_replace("/<230>/","S2-2R",$alg);
      $alg = preg_replace("/<231>/","S2-2L'",$alg); $alg = preg_replace("/<232>/","S2-2FL",$alg); $alg = preg_replace("/<233>/","S2-2L",$alg);
      $alg = preg_replace("/<234>/","S2-2F'",$alg); $alg = preg_replace("/<235>/","S2-2F2",$alg); $alg = preg_replace("/<236>/","S2-2F",$alg);
      $alg = preg_replace("/<237>/","S2-2B'",$alg); $alg = preg_replace("/<238>/","S2-2B2",$alg); $alg = preg_replace("/<239>/","S2-2B",$alg);
      $alg = preg_replace("/<240>/","S2-2U'",$alg); $alg = preg_replace("/<241>/","S2-2FU",$alg); $alg = preg_replace("/<242>/","S2-2U",$alg);
      $alg = preg_replace("/<243>/","S2-2D'",$alg); $alg = preg_replace("/<244>/","S2-2D2",$alg); $alg = preg_replace("/<245>/","S2-2D",$alg);
      
      /* S2-3 | S5-6 */
      $alg = preg_replace("/<246>/","S2-3R'",$alg); $alg = preg_replace("/<247>/","S2-3R2",$alg); $alg = preg_replace("/<248>/","S2-3R",$alg);
      $alg = preg_replace("/<249>/","S2-3L'",$alg); $alg = preg_replace("/<250>/","S2-3FL",$alg); $alg = preg_replace("/<251>/","S2-3L",$alg);
      $alg = preg_replace("/<252>/","S2-3F'",$alg); $alg = preg_replace("/<253>/","S2-3F2",$alg); $alg = preg_replace("/<254>/","S2-3F",$alg);
      $alg = preg_replace("/<255>/","S2-3B'",$alg); $alg = preg_replace("/<256>/","S2-3B2",$alg); $alg = preg_replace("/<257>/","S2-3B",$alg);
      $alg = preg_replace("/<258>/","S2-3U'",$alg); $alg = preg_replace("/<259>/","S2-3FU",$alg); $alg = preg_replace("/<260>/","S2-3U",$alg);
      $alg = preg_replace("/<261>/","S2-3D'",$alg); $alg = preg_replace("/<262>/","S2-3D2",$alg); $alg = preg_replace("/<263>/","S2-3D",$alg);
      ...
      /* Non-slice-twists */
      $alg = preg_replace("/<2101>/","T3R' T3L'",$alg);
      $alg = preg_replace("/<2102>/","T3F' T3B'",$alg);
      $alg = preg_replace("/<2103>/","T3U' T3D'",$alg);
      
      $alg = preg_replace("/<2104>/","MR' CR'",$alg);
      $alg = preg_replace("/<2105>/","MR CR",$alg);
      $alg = preg_replace("/<2106>/","MF' CF'",$alg);
      $alg = preg_replace("/<2107>/","MF CF",$alg);
      $alg = preg_replace("/<2108>/","MU' CU'",$alg);
      $alg = preg_replace("/<2109>/","MU CU",$alg);
      
      
      $alg = preg_replace("/<2110>/","M3R' CR'",$alg);
      $alg = preg_replace("/<2111>/","M3R CR",$alg);
      $alg = preg_replace("/<2112>/","M3F' CF'",$alg);
      $alg = preg_replace("/<2113>/","M3F CF",$alg);
      $alg = preg_replace("/<2114>/","M3U' CU'",$alg);
      $alg = preg_replace("/<2115>/","M3U CU",$alg);
      
      
      $alg = preg_replace("/<2116>/","WR' CR'",$alg);
      $alg = preg_replace("/<2117>/","WR CR",$alg);
      $alg = preg_replace("/<2118>/","WF' CF'",$alg);
      $alg = preg_replace("/<2119>/","WF CF",$alg);
      $alg = preg_replace("/<2120>/","WU' CU'",$alg);
      $alg = preg_replace("/<2121>/","WU CU",$alg);
      
      
      $alg = preg_replace("/<2122>/","NR' CR'",$alg);
      $alg = preg_replace("/<2123>/","NL' CR",$alg);
      $alg = preg_replace("/<2124>/","NF' CF'",$alg);
      $alg = preg_replace("/<2125>/","NB' CF",$alg);
      $alg = preg_replace("/<2126>/","NU' CU'",$alg);
      $alg = preg_replace("/<2127>/","ND' CU",$alg);
      
      
      $alg = preg_replace("/<2128>/","VR' CR'",$alg);
      $alg = preg_replace("/<2129>/","VL' CR",$alg);
      $alg = preg_replace("/<2130>/","VF' CF'",$alg);
      $alg = preg_replace("/<2131>/","VB' CF",$alg);
      $alg = preg_replace("/<2132>/","VU' CU'",$alg);
      $alg = preg_replace("/<2133>/","VD' CU",$alg);
      
      


      $alg = preg_replace("/<2201>/","R' L'",$alg);
      $alg = preg_replace("/<2202>/","F' B'",$alg);
      $alg = preg_replace("/<2203>/","U' D'",$alg);
    }
  ...
  */
  function alg3xP_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2U'/","<101>",$alg); $alg = preg_replace("/N2U-/","<101>",$alg);   $alg = preg_replace("/N2U/","<102>",$alg);
    $alg = preg_replace("/N2R'/","<103>",$alg); $alg = preg_replace("/N2R-/","<103>",$alg);   $alg = preg_replace("/N2R/","<104>",$alg);
    $alg = preg_replace("/N2L'/","<105>",$alg); $alg = preg_replace("/N2L-/","<105>",$alg);   $alg = preg_replace("/N2L/","<106>",$alg);
    $alg = preg_replace("/N2B'/","<107>",$alg); $alg = preg_replace("/N2B-/","<107>",$alg);   $alg = preg_replace("/N2B/","<108>",$alg);
    ...
    /* N1 (Corner twists) */
    $alg = preg_replace("/N1U'/","<901>",$alg); $alg = preg_replace("/N1U-/","<901>",$alg);   $alg = preg_replace("/N1U/","<902>",$alg);
    $alg = preg_replace("/N1R'/","<903>",$alg); $alg = preg_replace("/N1R-/","<903>",$alg);   $alg = preg_replace("/N1R/","<904>",$alg);
    $alg = preg_replace("/N1L'/","<905>",$alg); $alg = preg_replace("/N1L-/","<905>",$alg);   $alg = preg_replace("/N1L/","<906>",$alg);
    $alg = preg_replace("/N1B'/","<907>",$alg); $alg = preg_replace("/N1B-/","<907>",$alg);   $alg = preg_replace("/N1B/","<908>",$alg);
    ...
    $alg = preg_replace("/T2U'/","<301>",$alg); $alg = preg_replace("/T2U-/","<301>",$alg);   $alg = preg_replace("/T2U/","<302>",$alg);
    $alg = preg_replace("/T2R'/","<303>",$alg); $alg = preg_replace("/T2R-/","<303>",$alg);   $alg = preg_replace("/T2R/","<304>",$alg);
    $alg = preg_replace("/T2L'/","<305>",$alg); $alg = preg_replace("/T2L-/","<305>",$alg);   $alg = preg_replace("/T2L/","<306>",$alg);
    $alg = preg_replace("/T2B'/","<307>",$alg); $alg = preg_replace("/T2B-/","<307>",$alg);   $alg = preg_replace("/T2B/","<308>",$alg);
  ...
  */
  function alg4xP_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2U'/","<209>",$alg); $alg = preg_replace("/N2U-/","<209>",$alg);   $alg = preg_replace("/N2U/","<210>",$alg);
    $alg = preg_replace("/N2R'/","<211>",$alg); $alg = preg_replace("/N2R-/","<211>",$alg);   $alg = preg_replace("/N2R/","<212>",$alg);
    $alg = preg_replace("/N2L'/","<213>",$alg); $alg = preg_replace("/N2L-/","<213>",$alg);   $alg = preg_replace("/N2L/","<214>",$alg);
    $alg = preg_replace("/N2B'/","<215>",$alg); $alg = preg_replace("/N2B-/","<215>",$alg);   $alg = preg_replace("/N2B/","<216>",$alg);
    ...
    $alg = preg_replace("/T2U'/","<309>",$alg); $alg = preg_replace("/T2U-/","<309>",$alg);   $alg = preg_replace("/T2U/","<310>",$alg);
    $alg = preg_replace("/T2R'/","<311>",$alg); $alg = preg_replace("/T2R-/","<311>",$alg);   $alg = preg_replace("/T2R/","<312>",$alg);
    $alg = preg_replace("/T2L'/","<313>",$alg); $alg = preg_replace("/T2L-/","<313>",$alg);   $alg = preg_replace("/T2L/","<314>",$alg);
    $alg = preg_replace("/T2B'/","<315>",$alg); $alg = preg_replace("/T2B-/","<315>",$alg);   $alg = preg_replace("/T2B/","<316>",$alg);
  ...
  */
  function alg5xP_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2U'/","<233>",$alg); $alg = preg_replace("/N2U-/","<233>",$alg);   $alg = preg_replace("/N2U/","<234>",$alg);
    $alg = preg_replace("/N2R'/","<235>",$alg); $alg = preg_replace("/N2R-/","<235>",$alg);   $alg = preg_replace("/N2R/","<236>",$alg);
    $alg = preg_replace("/N2L'/","<237>",$alg); $alg = preg_replace("/N2L-/","<237>",$alg);   $alg = preg_replace("/N2L/","<238>",$alg);
    $alg = preg_replace("/N2B'/","<239>",$alg); $alg = preg_replace("/N2B-/","<239>",$alg);   $alg = preg_replace("/N2B/","<240>",$alg);
    ...
    $alg = preg_replace("/T2U'/","<317>",$alg); $alg = preg_replace("/T2U-/","<317>",$alg);   $alg = preg_replace("/T2U/","<318>",$alg);
    $alg = preg_replace("/T2R'/","<319>",$alg); $alg = preg_replace("/T2R-/","<319>",$alg);   $alg = preg_replace("/T2R/","<320>",$alg);
    $alg = preg_replace("/T2L'/","<321>",$alg); $alg = preg_replace("/T2L-/","<321>",$alg);   $alg = preg_replace("/T2L/","<322>",$alg);
    $alg = preg_replace("/T2B'/","<323>",$alg); $alg = preg_replace("/T2B-/","<323>",$alg);   $alg = preg_replace("/T2B/","<324>",$alg);
  ...
  */
  function alg3xD_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2UR2'/","<101>",$alg); $alg = preg_replace("/N2UR2-/","<101>",$alg);   $alg = preg_replace("/N2UR'/","<102>",$alg); $alg = preg_replace("/N2UR-/","<102>",$alg);   $alg = preg_replace("/N2UR2/","<103>",$alg);   $alg = preg_replace("/N2UR/","<104>",$alg);
    $alg = preg_replace("/N2DL2'/","<103>",$alg); $alg = preg_replace("/N2DL2-/","<103>",$alg);   $alg = preg_replace("/N2DL'/","<104>",$alg); $alg = preg_replace("/N2DL-/","<104>",$alg);   $alg = preg_replace("/N2DL2/","<101>",$alg);   $alg = preg_replace("/N2DL/","<102>",$alg);
    $alg = preg_replace("/N2UL2'/","<105>",$alg); $alg = preg_replace("/N2UL2-/","<105>",$alg);   $alg = preg_replace("/N2UL'/","<106>",$alg); $alg = preg_replace("/N2UL-/","<106>",$alg);   $alg = preg_replace("/N2UL2/","<107>",$alg);   $alg = preg_replace("/N2UL/","<108>",$alg);
    $alg = preg_replace("/N2DR2'/","<107>",$alg); $alg = preg_replace("/N2DR2-/","<107>",$alg);   $alg = preg_replace("/N2DR'/","<108>",$alg); $alg = preg_replace("/N2DR-/","<108>",$alg);   $alg = preg_replace("/N2DR2/","<105>",$alg);   $alg = preg_replace("/N2DR/","<106>",$alg);
    $alg = preg_replace("/N2BL2'/","<111>",$alg); $alg = preg_replace("/N2BL2-/","<111>",$alg);   $alg = preg_replace("/N2BL'/","<112>",$alg); $alg = preg_replace("/N2BL-/","<112>",$alg);   $alg = preg_replace("/N2BL2/","<109>",$alg);   $alg = preg_replace("/N2BL/","<110>",$alg);
    $alg = preg_replace("/N2BR2'/","<115>",$alg); $alg = preg_replace("/N2BR2-/","<115>",$alg);   $alg = preg_replace("/N2BR'/","<116>",$alg); $alg = preg_replace("/N2BR-/","<116>",$alg);   $alg = preg_replace("/N2BR2/","<113>",$alg);   $alg = preg_replace("/N2BR/","<114>",$alg);
    
    $alg = preg_replace("/N2R2'/", "<109>",$alg); $alg = preg_replace("/N2R2-/", "<109>",$alg);   $alg = preg_replace("/N2R'/", "<110>",$alg); $alg = preg_replace("/N2R-/", "<110>",$alg);   $alg = preg_replace("/N2R2/", "<111>",$alg);   $alg = preg_replace("/N2R/", "<112>",$alg);
    $alg = preg_replace("/N2L2'/", "<113>",$alg); $alg = preg_replace("/N2L2-/", "<113>",$alg);   $alg = preg_replace("/N2L'/", "<114>",$alg); $alg = preg_replace("/N2L-/", "<114>",$alg);   $alg = preg_replace("/N2L2/", "<115>",$alg);   $alg = preg_replace("/N2L/", "<116>",$alg);
    $alg = preg_replace("/N2F2'/", "<117>",$alg); $alg = preg_replace("/N2F2-/", "<117>",$alg);   $alg = preg_replace("/N2F'/", "<118>",$alg); $alg = preg_replace("/N2F-/", "<118>",$alg);   $alg = preg_replace("/N2F2/", "<119>",$alg);   $alg = preg_replace("/N2F/", "<120>",$alg);
    $alg = preg_replace("/N2B2'/", "<119>",$alg); $alg = preg_replace("/N2B2-/", "<119>",$alg);   $alg = preg_replace("/N2B'/", "<120>",$alg); $alg = preg_replace("/N2B-/", "<120>",$alg);   $alg = preg_replace("/N2B2/", "<117>",$alg);   $alg = preg_replace("/N2B/", "<118>",$alg);
    $alg = preg_replace("/N2U2'/", "<121>",$alg); $alg = preg_replace("/N2U2-/", "<121>",$alg);   $alg = preg_replace("/N2U'/", "<122>",$alg); $alg = preg_replace("/N2U-/", "<122>",$alg);   $alg = preg_replace("/N2U2/", "<123>",$alg);   $alg = preg_replace("/N2U/", "<124>",$alg);
    $alg = preg_replace("/N2D2'/", "<123>",$alg); $alg = preg_replace("/N2D2-/", "<123>",$alg);   $alg = preg_replace("/N2D'/", "<124>",$alg); $alg = preg_replace("/N2D-/", "<124>",$alg);   $alg = preg_replace("/N2D2/", "<121>",$alg);   $alg = preg_replace("/N2D/", "<122>",$alg);
    ...
    //<s> /* N1 | N3 (Face twists) */
    $alg = preg_replace("/N1UR2'/","<901>",$alg); $alg = preg_replace("/N1UR2-/","<901>",$alg);   $alg = preg_replace("/N1UR'/","<902>",$alg); $alg = preg_replace("/N1UR-/","<902>",$alg);   $alg = preg_replace("/N1UR2/","<903>",$alg);   $alg = preg_replace("/N1UR/","<904>",$alg);
    $alg = preg_replace("/N1DL2'/","<905>",$alg); $alg = preg_replace("/N1DL2-/","<905>",$alg);   $alg = preg_replace("/N1DL'/","<906>",$alg); $alg = preg_replace("/N1DL-/","<906>",$alg);   $alg = preg_replace("/N1DL2/","<907>",$alg);   $alg = preg_replace("/N1DL/","<908>",$alg);
    $alg = preg_replace("/N1UL2'/","<909>",$alg); $alg = preg_replace("/N1UL2-/","<909>",$alg);   $alg = preg_replace("/N1UL'/","<910>",$alg); $alg = preg_replace("/N1UL-/","<910>",$alg);   $alg = preg_replace("/N1UL2/","<911>",$alg);   $alg = preg_replace("/N1UL/","<912>",$alg);
    $alg = preg_replace("/N1DR2'/","<913>",$alg); $alg = preg_replace("/N1DR2-/","<913>",$alg);   $alg = preg_replace("/N1DR'/","<914>",$alg); $alg = preg_replace("/N1DR-/","<914>",$alg);   $alg = preg_replace("/N1DR2/","<915>",$alg);   $alg = preg_replace("/N1DR/","<916>",$alg);
    $alg = preg_replace("/N1BL2'/","<921>",$alg); $alg = preg_replace("/N1BL2-/","<921>",$alg);   $alg = preg_replace("/N1BL'/","<922>",$alg); $alg = preg_replace("/N1BL-/","<922>",$alg);   $alg = preg_replace("/N1BL2/","<923>",$alg);   $alg = preg_replace("/N1BL/","<924>",$alg);
    $alg = preg_replace("/N1BR2'/","<929>",$alg); $alg = preg_replace("/N1BR2-/","<929>",$alg);   $alg = preg_replace("/N1BR'/","<930>",$alg); $alg = preg_replace("/N1BR-/","<930>",$alg);   $alg = preg_replace("/N1BR2/","<931>",$alg);   $alg = preg_replace("/N1BR/","<932>",$alg);
    
    $alg = preg_replace("/N1R2'/", "<917>",$alg); $alg = preg_replace("/N1R2-/", "<917>",$alg);   $alg = preg_replace("/N1R'/", "<918>",$alg); $alg = preg_replace("/N1R-/", "<918>",$alg);   $alg = preg_replace("/N1R2/", "<919>",$alg);   $alg = preg_replace("/N1R/", "<920>",$alg);
    $alg = preg_replace("/N1L2'/", "<925>",$alg); $alg = preg_replace("/N1L2-/", "<925>",$alg);   $alg = preg_replace("/N1L'/", "<926>",$alg); $alg = preg_replace("/N1L-/", "<926>",$alg);   $alg = preg_replace("/N1L2/", "<927>",$alg);   $alg = preg_replace("/N1L/", "<928>",$alg);
    $alg = preg_replace("/N1F2'/", "<933>",$alg); $alg = preg_replace("/N1F2-/", "<933>",$alg);   $alg = preg_replace("/N1F'/", "<934>",$alg); $alg = preg_replace("/N1F-/", "<934>",$alg);   $alg = preg_replace("/N1F2/", "<935>",$alg);   $alg = preg_replace("/N1F/", "<936>",$alg);
    $alg = preg_replace("/N1B2'/", "<937>",$alg); $alg = preg_replace("/N1B2-/", "<937>",$alg);   $alg = preg_replace("/N1B'/", "<938>",$alg); $alg = preg_replace("/N1B-/", "<938>",$alg);   $alg = preg_replace("/N1B2/", "<939>",$alg);   $alg = preg_replace("/N1B/", "<940>",$alg);
    $alg = preg_replace("/N1U2'/", "<941>",$alg); $alg = preg_replace("/N1U2-/", "<941>",$alg);   $alg = preg_replace("/N1U'/", "<942>",$alg); $alg = preg_replace("/N1U-/", "<942>",$alg);   $alg = preg_replace("/N1U2/", "<943>",$alg);   $alg = preg_replace("/N1U/", "<944>",$alg);
    $alg = preg_replace("/N1D2'/", "<945>",$alg); $alg = preg_replace("/N1D2-/", "<945>",$alg);   $alg = preg_replace("/N1D'/", "<946>",$alg); $alg = preg_replace("/N1D-/", "<946>",$alg);   $alg = preg_replace("/N1D2/", "<947>",$alg);   $alg = preg_replace("/N1D/", "<948>",$alg);
    
    
    $alg = preg_replace("/N3UR2'/","<907>",$alg); $alg = preg_replace("/N3UR2-/","<907>",$alg);   $alg = preg_replace("/N3UR'/","<908>",$alg); $alg = preg_replace("/N3UR-/","<908>",$alg);   $alg = preg_replace("/N3UR2/","<905>",$alg);   $alg = preg_replace("/N3UR/","<906>",$alg);
    $alg = preg_replace("/N3DL2'/","<903>",$alg); $alg = preg_replace("/N3DL2-/","<903>",$alg);   $alg = preg_replace("/N3DL'/","<904>",$alg); $alg = preg_replace("/N3DL-/","<904>",$alg);   $alg = preg_replace("/N3DL2/","<901>",$alg);   $alg = preg_replace("/N3DL/","<902>",$alg);
    $alg = preg_replace("/N3UL2'/","<915>",$alg); $alg = preg_replace("/N3UL2-/","<915>",$alg);   $alg = preg_replace("/N3UL'/","<916>",$alg); $alg = preg_replace("/N3UL-/","<916>",$alg);   $alg = preg_replace("/N3UL2/","<913>",$alg);   $alg = preg_replace("/N3UL/","<914>",$alg);
    $alg = preg_replace("/N3DR2'/","<911>",$alg); $alg = preg_replace("/N3DR2-/","<911>",$alg);   $alg = preg_replace("/N3DR'/","<912>",$alg); $alg = preg_replace("/N3DR-/","<912>",$alg);   $alg = preg_replace("/N3DR2/","<909>",$alg);   $alg = preg_replace("/N3DR/","<910>",$alg);
    $alg = preg_replace("/N3BL2'/","<919>",$alg); $alg = preg_replace("/N3BL2-/","<919>",$alg);   $alg = preg_replace("/N3BL'/","<920>",$alg); $alg = preg_replace("/N3BL-/","<920>",$alg);   $alg = preg_replace("/N3BL2/","<917>",$alg);   $alg = preg_replace("/N3BL/","<918>",$alg);
    $alg = preg_replace("/N3BR2'/","<927>",$alg); $alg = preg_replace("/N3BR2-/","<927>",$alg);   $alg = preg_replace("/N3BR'/","<928>",$alg); $alg = preg_replace("/N3BR-/","<928>",$alg);   $alg = preg_replace("/N3BR2/","<925>",$alg);   $alg = preg_replace("/N3BR/","<926>",$alg);
    
    $alg = preg_replace("/N3R2'/", "<923>",$alg); $alg = preg_replace("/N3R2-/", "<923>",$alg);   $alg = preg_replace("/N3R'/", "<924>",$alg); $alg = preg_replace("/N3R-/", "<924>",$alg);   $alg = preg_replace("/N3R2/", "<921>",$alg);   $alg = preg_replace("/N3R/", "<922>",$alg);
    $alg = preg_replace("/N3L2'/", "<931>",$alg); $alg = preg_replace("/N3L2-/", "<931>",$alg);   $alg = preg_replace("/N3L'/", "<932>",$alg); $alg = preg_replace("/N3L-/", "<932>",$alg);   $alg = preg_replace("/N3L2/", "<929>",$alg);   $alg = preg_replace("/N3L/", "<930>",$alg);
    $alg = preg_replace("/N3F2'/", "<939>",$alg); $alg = preg_replace("/N3F2-/", "<939>",$alg);   $alg = preg_replace("/N3F'/", "<940>",$alg); $alg = preg_replace("/N3F-/", "<940>",$alg);   $alg = preg_replace("/N3F2/", "<937>",$alg);   $alg = preg_replace("/N3F/", "<938>",$alg);
    $alg = preg_replace("/N3B2'/", "<935>",$alg); $alg = preg_replace("/N3B2-/", "<935>",$alg);   $alg = preg_replace("/N3B'/", "<936>",$alg); $alg = preg_replace("/N3B-/", "<936>",$alg);   $alg = preg_replace("/N3B2/", "<933>",$alg);   $alg = preg_replace("/N3B/", "<934>",$alg);
    $alg = preg_replace("/N3U2'/", "<947>",$alg); $alg = preg_replace("/N3U2-/", "<947>",$alg);   $alg = preg_replace("/N3U'/", "<948>",$alg); $alg = preg_replace("/N3U-/", "<948>",$alg);   $alg = preg_replace("/N3U2/", "<945>",$alg);   $alg = preg_replace("/N3U/", "<946>",$alg);
    $alg = preg_replace("/N3D2'/", "<943>",$alg); $alg = preg_replace("/N3D2-/", "<943>",$alg);   $alg = preg_replace("/N3D'/", "<944>",$alg); $alg = preg_replace("/N3D-/", "<944>",$alg);   $alg = preg_replace("/N3D2/", "<941>",$alg);   $alg = preg_replace("/N3D/", "<942>",$alg);
    ...
    $alg = preg_replace("/T2UR2'/","<301>",$alg); $alg = preg_replace("/T2UR2-/","<301>",$alg);   $alg = preg_replace("/T2UR'/","<302>",$alg); $alg = preg_replace("/T2UR-/","<302>",$alg);   $alg = preg_replace("/T2UR2/","<303>",$alg);   $alg = preg_replace("/T2UR/","<304>",$alg);
    $alg = preg_replace("/T2DL2'/","<305>",$alg); $alg = preg_replace("/T2DL2-/","<305>",$alg);   $alg = preg_replace("/T2DL'/","<306>",$alg); $alg = preg_replace("/T2DL-/","<306>",$alg);   $alg = preg_replace("/T2DL2/","<307>",$alg);   $alg = preg_replace("/T2DL/","<308>",$alg);
    $alg = preg_replace("/T2UL2'/","<309>",$alg); $alg = preg_replace("/T2UL2-/","<309>",$alg);   $alg = preg_replace("/T2UL'/","<310>",$alg); $alg = preg_replace("/T2UL-/","<310>",$alg);   $alg = preg_replace("/T2UL2/","<311>",$alg);   $alg = preg_replace("/T2UL/","<312>",$alg);
    $alg = preg_replace("/T2DR2'/","<313>",$alg); $alg = preg_replace("/T2DR2-/","<313>",$alg);   $alg = preg_replace("/T2DR'/","<314>",$alg); $alg = preg_replace("/T2DR-/","<314>",$alg);   $alg = preg_replace("/T2DR2/","<315>",$alg);   $alg = preg_replace("/T2DR/","<316>",$alg);
    $alg = preg_replace("/T2BL2'/","<321>",$alg); $alg = preg_replace("/T2BL2-/","<321>",$alg);   $alg = preg_replace("/T2BL'/","<322>",$alg); $alg = preg_replace("/T2BL-/","<322>",$alg);   $alg = preg_replace("/T2BL2/","<323>",$alg);   $alg = preg_replace("/T2BL/","<324>",$alg);
    $alg = preg_replace("/T2BR2'/","<329>",$alg); $alg = preg_replace("/T2BR2-/","<329>",$alg);   $alg = preg_replace("/T2BR'/","<330>",$alg); $alg = preg_replace("/T2BR-/","<330>",$alg);   $alg = preg_replace("/T2BR2/","<331>",$alg);   $alg = preg_replace("/T2BR/","<332>",$alg);
    
    $alg = preg_replace("/T2R2'/", "<317>",$alg); $alg = preg_replace("/T2R2-/", "<317>",$alg);   $alg = preg_replace("/T2R'/", "<318>",$alg); $alg = preg_replace("/T2R-/", "<318>",$alg);   $alg = preg_replace("/T2R2/", "<319>",$alg);   $alg = preg_replace("/T2R/", "<320>",$alg);
    $alg = preg_replace("/T2L2'/", "<325>",$alg); $alg = preg_replace("/T2L2-/", "<325>",$alg);   $alg = preg_replace("/T2L'/", "<326>",$alg); $alg = preg_replace("/T2L-/", "<326>",$alg);   $alg = preg_replace("/T2L2/", "<327>",$alg);   $alg = preg_replace("/T2L/", "<328>",$alg);
    $alg = preg_replace("/T2F2'/", "<333>",$alg); $alg = preg_replace("/T2F2-/", "<333>",$alg);   $alg = preg_replace("/T2F'/", "<334>",$alg); $alg = preg_replace("/T2F-/", "<334>",$alg);   $alg = preg_replace("/T2F2/", "<335>",$alg);   $alg = preg_replace("/T2F/", "<336>",$alg);
    $alg = preg_replace("/T2B2'/", "<337>",$alg); $alg = preg_replace("/T2B2-/", "<337>",$alg);   $alg = preg_replace("/T2B'/", "<338>",$alg); $alg = preg_replace("/T2B-/", "<338>",$alg);   $alg = preg_replace("/T2B2/", "<339>",$alg);   $alg = preg_replace("/T2B/", "<340>",$alg);
    $alg = preg_replace("/T2U2'/", "<341>",$alg); $alg = preg_replace("/T2U2-/", "<341>",$alg);   $alg = preg_replace("/T2U'/", "<342>",$alg); $alg = preg_replace("/T2U-/", "<342>",$alg);   $alg = preg_replace("/T2U2/", "<343>",$alg);   $alg = preg_replace("/T2U/", "<344>",$alg);
    $alg = preg_replace("/T2D2'/", "<345>",$alg); $alg = preg_replace("/T2D2-/", "<345>",$alg);   $alg = preg_replace("/T2D'/", "<346>",$alg); $alg = preg_replace("/T2D-/", "<346>",$alg);   $alg = preg_replace("/T2D2/", "<347>",$alg);   $alg = preg_replace("/T2D/", "<348>",$alg);
  ...
  */
  function alg3xD_TwizzleToSse($alg) {
    ...
    if ($optSSE == true) {
      $alg = " ". $alg ." ";                       // Leerzeichen am Anfang und Schluss des Strings hinzufügen.
      $alg = preg_replace("/\r\n/"," \r\n ",$alg); // Leerzeichen vor und nach Zeilenschaltung hinzufügen.
    ...
    $alg = preg_replace("/2-3BR2'/","<307>",$alg);   $alg = preg_replace("/2-3BR'/","<308>",$alg);   $alg = preg_replace("/2-3BR2/","<305>",$alg);   $alg = preg_replace("/2-3BR/","<306>",$alg);
    $alg = preg_replace("/2-3FL2'/","<303>",$alg);   $alg = preg_replace("/2-3FL'/","<304>",$alg);   $alg = preg_replace("/2-3FL2/","<301>",$alg);   $alg = preg_replace("/2-3FL/","<302>",$alg);
    $alg = preg_replace("/2-3BL2'/","<315>",$alg);   $alg = preg_replace("/2-3BL'/","<316>",$alg);   $alg = preg_replace("/2-3BL2/","<313>",$alg);   $alg = preg_replace("/2-3BL/","<314>",$alg);
    $alg = preg_replace("/2-3FR2'/","<311>",$alg);   $alg = preg_replace("/2-3FR'/","<312>",$alg);   $alg = preg_replace("/2-3FR2/","<309>",$alg);   $alg = preg_replace("/2-3FR/","<310>",$alg);
    $alg = preg_replace("/2-3DL2'/","<319>",$alg);   $alg = preg_replace("/2-3DL'/","<320>",$alg);   $alg = preg_replace("/2-3DL2/","<317>",$alg);   $alg = preg_replace("/2-3DL/","<318>",$alg);
    $alg = preg_replace("/2-3DR2'/","<327>",$alg);   $alg = preg_replace("/2-3DR'/","<328>",$alg);   $alg = preg_replace("/2-3DR2/","<325>",$alg);   $alg = preg_replace("/2-3DR/","<326>",$alg);
    
    $alg = preg_replace("/2-3R2'/", "<323>",$alg);   $alg = preg_replace("/2-3R'/", "<324>",$alg);   $alg = preg_replace("/2-3R2/", "<321>",$alg);   $alg = preg_replace("/2-3R/", "<322>",$alg);
    $alg = preg_replace("/2-3L2'/", "<331>",$alg);   $alg = preg_replace("/2-3L'/", "<332>",$alg);   $alg = preg_replace("/2-3L2/", "<329>",$alg);   $alg = preg_replace("/2-3L/", "<330>",$alg);
    $alg = preg_replace("/2-3F2'/", "<339>",$alg);   $alg = preg_replace("/2-3F'/", "<340>",$alg);   $alg = preg_replace("/2-3F2/", "<337>",$alg);   $alg = preg_replace("/2-3F/", "<338>",$alg);
    $alg = preg_replace("/2-3B2'/", "<335>",$alg);   $alg = preg_replace("/2-3B'/", "<336>",$alg);   $alg = preg_replace("/2-3B2/", "<333>",$alg);   $alg = preg_replace("/2-3B/", "<334>",$alg);
    $alg = preg_replace("/2-3U2'/", "<347>",$alg);   $alg = preg_replace("/2-3U'/", "<348>",$alg);   $alg = preg_replace("/2-3U2/", "<345>",$alg);   $alg = preg_replace("/2-3U/", "<346>",$alg);
    $alg = preg_replace("/2-3D2'/", "<343>",$alg);   $alg = preg_replace("/2-3D'/", "<344>",$alg);   $alg = preg_replace("/2-3D2/", "<341>",$alg);   $alg = preg_replace("/2-3D/", "<342>",$alg);
    ...
    if ($optSSE == true) {
      ...
      // Dadurch werden aber Tokens am Anfang und am Schluss einer Zeile, bzw. vor oder nach einer Klammer nicht erkannt!
      // Deshalb werden zuerst Leerzeichen vor und nach Klammern eingefügt und am Schluss dann wieder entfernt.
      // 
...
/* xxx   xxx */
    ...
    /* --- Optimize: Clean up --- */
    if ($optSSE == true) {
      $alg = preg_replace("'^\s+'",'',$alg);       // Leerzeichen am Anfang des Strings entfernen.
      $alg = preg_replace("'\s+$'",'',$alg);       // Leerzeichen am Schluss des Strings entfernen.
      $alg = preg_replace("/ \r\n /","\r\n",$alg); // Leerzeichen vor und nach Zeilenschaltung entfernen.
  ...
  */
  function alg5xD_SseToTwizzle($alg) {
    ...
    $alg = preg_replace("/N2UR2'/","<101>",$alg); $alg = preg_replace("/N2UR2-/","<101>",$alg);   $alg = preg_replace("/N2UR'/","<102>",$alg); $alg = preg_replace("/N2UR-/","<102>",$alg);   $alg = preg_replace("/N2UR2/","<103>",$alg);   $alg = preg_replace("/N2UR/","<104>",$alg);
    $alg = preg_replace("/N2DL2'/","<105>",$alg); $alg = preg_replace("/N2DL2-/","<105>",$alg);   $alg = preg_replace("/N2DL'/","<106>",$alg); $alg = preg_replace("/N2DL-/","<106>",$alg);   $alg = preg_replace("/N2DL2/","<107>",$alg);   $alg = preg_replace("/N2DL/","<108>",$alg);
    $alg = preg_replace("/N2UL2'/","<109>",$alg); $alg = preg_replace("/N2UL2-/","<109>",$alg);   $alg = preg_replace("/N2UL'/","<110>",$alg); $alg = preg_replace("/N2UL-/","<110>",$alg);   $alg = preg_replace("/N2UL2/","<111>",$alg);   $alg = preg_replace("/N2UL/","<112>",$alg);
    $alg = preg_replace("/N2DR2'/","<113>",$alg); $alg = preg_replace("/N2DR2-/","<113>",$alg);   $alg = preg_replace("/N2DR'/","<114>",$alg); $alg = preg_replace("/N2DR-/","<114>",$alg);   $alg = preg_replace("/N2DR2/","<115>",$alg);   $alg = preg_replace("/N2DR/","<116>",$alg);
    $alg = preg_replace("/N2BL2'/","<121>",$alg); $alg = preg_replace("/N2BL2-/","<121>",$alg);   $alg = preg_replace("/N2BL'/","<122>",$alg); $alg = preg_replace("/N2BL-/","<122>",$alg);   $alg = preg_replace("/N2BL2/","<123>",$alg);   $alg = preg_replace("/N2BL/","<124>",$alg);
    $alg = preg_replace("/N2BR2'/","<129>",$alg); $alg = preg_replace("/N2BR2-/","<129>",$alg);   $alg = preg_replace("/N2BR'/","<130>",$alg); $alg = preg_replace("/N2BR-/","<130>",$alg);   $alg = preg_replace("/N2BR2/","<131>",$alg);   $alg = preg_replace("/N2BR/","<132>",$alg);
    
    $alg = preg_replace("/N2R2'/", "<117>",$alg); $alg = preg_replace("/N2R2-/", "<117>",$alg);   $alg = preg_replace("/N2R'/", "<118>",$alg); $alg = preg_replace("/N2R-/", "<118>",$alg);   $alg = preg_replace("/N2R2/", "<119>",$alg);   $alg = preg_replace("/N2R/", "<120>",$alg);
    $alg = preg_replace("/N2L2'/", "<125>",$alg); $alg = preg_replace("/N2L2-/", "<125>",$alg);   $alg = preg_replace("/N2L'/", "<126>",$alg); $alg = preg_replace("/N2L-/", "<126>",$alg);   $alg = preg_replace("/N2L2/", "<127>",$alg);   $alg = preg_replace("/N2L/", "<128>",$alg);
    $alg = preg_replace("/N2F2'/", "<133>",$alg); $alg = preg_replace("/N2F2-/", "<133>",$alg);   $alg = preg_replace("/N2F'/", "<134>",$alg); $alg = preg_replace("/N2F-/", "<134>",$alg);   $alg = preg_replace("/N2F2/", "<135>",$alg);   $alg = preg_replace("/N2F/", "<136>",$alg);
    $alg = preg_replace("/N2B2'/", "<137>",$alg); $alg = preg_replace("/N2B2-/", "<137>",$alg);   $alg = preg_replace("/N2B'/", "<138>",$alg); $alg = preg_replace("/N2B-/", "<138>",$alg);   $alg = preg_replace("/N2B2/", "<139>",$alg);   $alg = preg_replace("/N2B/", "<140>",$alg);
    $alg = preg_replace("/N2U2'/", "<141>",$alg); $alg = preg_replace("/N2U2-/", "<141>",$alg);   $alg = preg_replace("/N2U'/", "<142>",$alg); $alg = preg_replace("/N2U-/", "<142>",$alg);   $alg = preg_replace("/N2U2/", "<143>",$alg);   $alg = preg_replace("/N2U/", "<144>",$alg);
    $alg = preg_replace("/N2D2'/", "<145>",$alg); $alg = preg_replace("/N2D2-/", "<145>",$alg);   $alg = preg_replace("/N2D'/", "<146>",$alg); $alg = preg_replace("/N2D-/", "<146>",$alg);   $alg = preg_replace("/N2D2/", "<147>",$alg);   $alg = preg_replace("/N2D/", "<148>",$alg);
    ...
    $alg = preg_replace("/N5UR2'/","<907>",$alg); $alg = preg_replace("/N5UR2-/","<907>",$alg);   $alg = preg_replace("/N5UR'/","<908>",$alg); $alg = preg_replace("/N5UR-/","<908>",$alg);   $alg = preg_replace("/N5UR2/","<905>",$alg);   $alg = preg_replace("/N5UR/","<906>",$alg);
    $alg = preg_replace("/N5DL2'/","<903>",$alg); $alg = preg_replace("/N5DL2-/","<903>",$alg);   $alg = preg_replace("/N5DL'/","<904>",$alg); $alg = preg_replace("/N5DL-/","<904>",$alg);   $alg = preg_replace("/N5DL2/","<901>",$alg);   $alg = preg_replace("/N5DL/","<902>",$alg);
    $alg = preg_replace("/N5UL2'/","<915>",$alg); $alg = preg_replace("/N5UL2-/","<915>",$alg);   $alg = preg_replace("/N5UL'/","<916>",$alg); $alg = preg_replace("/N5UL-/","<916>",$alg);   $alg = preg_replace("/N5UL2/","<913>",$alg);   $alg = preg_replace("/N5UL/","<914>",$alg);
    $alg = preg_replace("/N5DR2'/","<911>",$alg); $alg = preg_replace("/N5DR2-/","<911>",$alg);   $alg = preg_replace("/N5DR'/","<912>",$alg); $alg = preg_replace("/N5DR-/","<912>",$alg);   $alg = preg_replace("/N5DR2/","<909>",$alg);   $alg = preg_replace("/N5DR/","<910>",$alg);
    $alg = preg_replace("/N5BL2'/","<919>",$alg); $alg = preg_replace("/N5BL2-/","<919>",$alg);   $alg = preg_replace("/N5BL'/","<920>",$alg); $alg = preg_replace("/N5BL-/","<920>",$alg);   $alg = preg_replace("/N5BL2/","<917>",$alg);   $alg = preg_replace("/N5BL/","<918>",$alg);
    $alg = preg_replace("/N5BR2'/","<927>",$alg); $alg = preg_replace("/N5BR2-/","<927>",$alg);   $alg = preg_replace("/N5BR'/","<928>",$alg); $alg = preg_replace("/N5BR-/","<928>",$alg);   $alg = preg_replace("/N5BR2/","<925>",$alg);   $alg = preg_replace("/N5BR/","<926>",$alg);
    
    $alg = preg_replace("/N5R2'/", "<923>",$alg); $alg = preg_replace("/N5R2-/", "<923>",$alg);   $alg = preg_replace("/N5R'/", "<924>",$alg); $alg = preg_replace("/N5R-/", "<924>",$alg);   $alg = preg_replace("/N5R2/", "<921>",$alg);   $alg = preg_replace("/N5R/", "<922>",$alg);
    $alg = preg_replace("/N5L2'/", "<931>",$alg); $alg = preg_replace("/N5L2-/", "<931>",$alg);   $alg = preg_replace("/N5L'/", "<932>",$alg); $alg = preg_replace("/N5L-/", "<932>",$alg);   $alg = preg_replace("/N5L2/", "<929>",$alg);   $alg = preg_replace("/N5L/", "<930>",$alg);
    $alg = preg_replace("/N5F2'/", "<939>",$alg); $alg = preg_replace("/N5F2-/", "<939>",$alg);   $alg = preg_replace("/N5F'/", "<940>",$alg); $alg = preg_replace("/N5F-/", "<940>",$alg);   $alg = preg_replace("/N5F2/", "<937>",$alg);   $alg = preg_replace("/N5F/", "<938>",$alg);
    $alg = preg_replace("/N5B2'/", "<935>",$alg); $alg = preg_replace("/N5B2-/", "<935>",$alg);   $alg = preg_replace("/N5B'/", "<936>",$alg); $alg = preg_replace("/N5B-/", "<936>",$alg);   $alg = preg_replace("/N5B2/", "<933>",$alg);   $alg = preg_replace("/N5B/", "<934>",$alg);
    $alg = preg_replace("/N5U2'/", "<947>",$alg); $alg = preg_replace("/N5U2-/", "<947>",$alg);   $alg = preg_replace("/N5U'/", "<948>",$alg); $alg = preg_replace("/N5U-/", "<948>",$alg);   $alg = preg_replace("/N5U2/", "<945>",$alg);   $alg = preg_replace("/N5U/", "<946>",$alg);
    $alg = preg_replace("/N5D2'/", "<943>",$alg); $alg = preg_replace("/N5D2-/", "<943>",$alg);   $alg = preg_replace("/N5D'/", "<944>",$alg); $alg = preg_replace("/N5D-/", "<944>",$alg);   $alg = preg_replace("/N5D2/", "<941>",$alg);   $alg = preg_replace("/N5D/", "<942>",$alg);
    ...
    $alg = preg_replace("/T2UR2'/", "<397>",$alg); $alg = preg_replace("/T2UR2-/", "<397>",$alg);   $alg = preg_replace("/T2UR'/", "<398>",$alg); $alg = preg_replace("/T2UR-/", "<398>",$alg);   $alg = preg_replace("/T2UR2/", "<399>",$alg);   $alg = preg_replace("/T2UR/","<3100>",$alg);
    $alg = preg_replace("/T2DL2'/","<3101>",$alg); $alg = preg_replace("/T2DL2-/","<3101>",$alg);   $alg = preg_replace("/T2DL'/","<3102>",$alg); $alg = preg_replace("/T2DL-/","<3102>",$alg);   $alg = preg_replace("/T2DL2/","<3103>",$alg);   $alg = preg_replace("/T2DL/","<3104>",$alg);
    $alg = preg_replace("/T2UL2'/","<3105>",$alg); $alg = preg_replace("/T2UL2-/","<3105>",$alg);   $alg = preg_replace("/T2UL'/","<3106>",$alg); $alg = preg_replace("/T2UL-/","<3106>",$alg);   $alg = preg_replace("/T2UL2/","<3107>",$alg);   $alg = preg_replace("/T2UL/","<3108>",$alg);
    $alg = preg_replace("/T2DR2'/","<3109>",$alg); $alg = preg_replace("/T2DR2-/","<3109>",$alg);   $alg = preg_replace("/T2DR'/","<3110>",$alg); $alg = preg_replace("/T2DR-/","<3110>",$alg);   $alg = preg_replace("/T2DR2/","<3111>",$alg);   $alg = preg_replace("/T2DR/","<3112>",$alg);
    $alg = preg_replace("/T2BL2'/","<3117>",$alg); $alg = preg_replace("/T2BL2-/","<3117>",$alg);   $alg = preg_replace("/T2BL'/","<3118>",$alg); $alg = preg_replace("/T2BL-/","<3118>",$alg);   $alg = preg_replace("/T2BL2/","<3119>",$alg);   $alg = preg_replace("/T2BL/","<3120>",$alg);
    $alg = preg_replace("/T2BR2'/","<3125>",$alg); $alg = preg_replace("/T2BR2-/","<3125>",$alg);   $alg = preg_replace("/T2BR'/","<3126>",$alg); $alg = preg_replace("/T2BR-/","<3126>",$alg);   $alg = preg_replace("/T2BR2/","<3127>",$alg);   $alg = preg_replace("/T2BR/","<3128>",$alg);
    
    $alg = preg_replace("/T2R2'/", "<3113>",$alg); $alg = preg_replace("/T2R2-/", "<3113>",$alg);   $alg = preg_replace("/T2R'/", "<3114>",$alg); $alg = preg_replace("/T2R-/", "<3114>",$alg);   $alg = preg_replace("/T2R2/", "<3115>",$alg);   $alg = preg_replace("/T2R/", "<3116>",$alg);
    $alg = preg_replace("/T2L2'/", "<3121>",$alg); $alg = preg_replace("/T2L2-/", "<3121>",$alg);   $alg = preg_replace("/T2L'/", "<3122>",$alg); $alg = preg_replace("/T2L-/", "<3122>",$alg);   $alg = preg_replace("/T2L2/", "<3123>",$alg);   $alg = preg_replace("/T2L/", "<3124>",$alg);
    $alg = preg_replace("/T2F2'/", "<3129>",$alg); $alg = preg_replace("/T2F2-/", "<3129>",$alg);   $alg = preg_replace("/T2F'/", "<3130>",$alg); $alg = preg_replace("/T2F-/", "<3130>",$alg);   $alg = preg_replace("/T2F2/", "<3131>",$alg);   $alg = preg_replace("/T2F/", "<3132>",$alg);
    $alg = preg_replace("/T2B2'/", "<3133>",$alg); $alg = preg_replace("/T2B2-/", "<3133>",$alg);   $alg = preg_replace("/T2B'/", "<3134>",$alg); $alg = preg_replace("/T2B-/", "<3134>",$alg);   $alg = preg_replace("/T2B2/", "<3135>",$alg);   $alg = preg_replace("/T2B/", "<3136>",$alg);
    $alg = preg_replace("/T2U2'/", "<3137>",$alg); $alg = preg_replace("/T2U2-/", "<3137>",$alg);   $alg = preg_replace("/T2U'/", "<3138>",$alg); $alg = preg_replace("/T2U-/", "<3138>",$alg);   $alg = preg_replace("/T2U2/", "<3139>",$alg);   $alg = preg_replace("/T2U/", "<3140>",$alg);
    $alg = preg_replace("/T2D2'/", "<3141>",$alg); $alg = preg_replace("/T2D2-/", "<3141>",$alg);   $alg = preg_replace("/T2D'/", "<3142>",$alg); $alg = preg_replace("/T2D-/", "<3142>",$alg);   $alg = preg_replace("/T2D2/", "<3143>",$alg);   $alg = preg_replace("/T2D/", "<3144>",$alg);
  ...
  */
  function alg5xD_TwizzleToSse($alg) {
    ...
    $alg = preg_replace("/2-5BR2'/","<307>",$alg);   $alg = preg_replace("/2-5BR'/","<308>",$alg);   $alg = preg_replace("/2-5BR2/","<305>",$alg);   $alg = preg_replace("/2-5BR/","<306>",$alg);
    $alg = preg_replace("/2-5FL2'/","<303>",$alg);   $alg = preg_replace("/2-5FL'/","<304>",$alg);   $alg = preg_replace("/2-5FL2/","<301>",$alg);   $alg = preg_replace("/2-5FL/","<302>",$alg);
    $alg = preg_replace("/2-5BL2'/","<315>",$alg);   $alg = preg_replace("/2-5BL'/","<316>",$alg);   $alg = preg_replace("/2-5BL2/","<313>",$alg);   $alg = preg_replace("/2-5BL/","<314>",$alg);
    $alg = preg_replace("/2-5FR2'/","<311>",$alg);   $alg = preg_replace("/2-5FR'/","<312>",$alg);   $alg = preg_replace("/2-5FR2/","<309>",$alg);   $alg = preg_replace("/2-5FR/","<310>",$alg);
    $alg = preg_replace("/2-5DL2'/","<319>",$alg);   $alg = preg_replace("/2-5DL'/","<320>",$alg);   $alg = preg_replace("/2-5DL2/","<317>",$alg);   $alg = preg_replace("/2-5DL/","<318>",$alg);
    $alg = preg_replace("/2-5DR2'/","<327>",$alg);   $alg = preg_replace("/2-5DR'/","<328>",$alg);   $alg = preg_replace("/2-5DR2/","<325>",$alg);   $alg = preg_replace("/2-5DR/","<326>",$alg);
    
    $alg = preg_replace("/2-5R2'/", "<323>",$alg);   $alg = preg_replace("/2-5R'/", "<324>",$alg);   $alg = preg_replace("/2-5R2/", "<321>",$alg);   $alg = preg_replace("/2-5R/", "<322>",$alg);
    $alg = preg_replace("/2-5L2'/", "<331>",$alg);   $alg = preg_replace("/2-5L'/", "<332>",$alg);   $alg = preg_replace("/2-5L2/", "<329>",$alg);   $alg = preg_replace("/2-5L/", "<330>",$alg);
    $alg = preg_replace("/2-5F2'/", "<339>",$alg);   $alg = preg_replace("/2-5F'/", "<340>",$alg);   $alg = preg_replace("/2-5F2/", "<337>",$alg);   $alg = preg_replace("/2-5F/", "<338>",$alg);
    $alg = preg_replace("/2-5B2'/", "<335>",$alg);   $alg = preg_replace("/2-5B'/", "<336>",$alg);   $alg = preg_replace("/2-5B2/", "<333>",$alg);   $alg = preg_replace("/2-5B/", "<334>",$alg);
    $alg = preg_replace("/2-5U2'/", "<347>",$alg);   $alg = preg_replace("/2-5U'/", "<348>",$alg);   $alg = preg_replace("/2-5U2/", "<345>",$alg);   $alg = preg_replace("/2-5U/", "<346>",$alg);
    $alg = preg_replace("/2-5D2'/", "<343>",$alg);   $alg = preg_replace("/2-5D'/", "<344>",$alg);   $alg = preg_replace("/2-5D2/", "<341>",$alg);   $alg = preg_replace("/2-5D/", "<342>",$alg);
    ...
    $alg = preg_replace("/3-5BR2'/","<355>",$alg);   $alg = preg_replace("/3-5BR'/","<356>",$alg);   $alg = preg_replace("/3-5BR2/","<353>",$alg);   $alg = preg_replace("/3-5BR/","<354>",$alg);
    $alg = preg_replace("/3-5FL2'/","<351>",$alg);   $alg = preg_replace("/3-5FL'/","<352>",$alg);   $alg = preg_replace("/3-5FL2/","<349>",$alg);   $alg = preg_replace("/3-5FL/","<350>",$alg);
    $alg = preg_replace("/3-5BL2'/","<363>",$alg);   $alg = preg_replace("/3-5BL'/","<364>",$alg);   $alg = preg_replace("/3-5BL2/","<361>",$alg);   $alg = preg_replace("/3-5BL/","<362>",$alg);
    $alg = preg_replace("/3-5FR2'/","<359>",$alg);   $alg = preg_replace("/3-5FR'/","<360>",$alg);   $alg = preg_replace("/3-5FR2/","<357>",$alg);   $alg = preg_replace("/3-5FR/","<358>",$alg);
    $alg = preg_replace("/3-5DL2'/","<367>",$alg);   $alg = preg_replace("/3-5DL'/","<368>",$alg);   $alg = preg_replace("/3-5DL2/","<365>",$alg);   $alg = preg_replace("/3-5DL/","<366>",$alg);
    $alg = preg_replace("/3-5DR2'/","<375>",$alg);   $alg = preg_replace("/3-5DR'/","<376>",$alg);   $alg = preg_replace("/3-5DR2/","<373>",$alg);   $alg = preg_replace("/3-5DR/","<374>",$alg);
    
    $alg = preg_replace("/3-5R2'/","<371>", $alg);   $alg = preg_replace("/3-5R'/","<372>", $alg);   $alg = preg_replace("/3-5R2/", "<369>",$alg);   $alg = preg_replace("/3-5R/", "<370>",$alg);
    $alg = preg_replace("/3-5L2'/","<379>", $alg);   $alg = preg_replace("/3-5L'/","<380>", $alg);   $alg = preg_replace("/3-5L2/", "<377>",$alg);   $alg = preg_replace("/3-5L/", "<378>",$alg);
    $alg = preg_replace("/3-5F2'/","<387>", $alg);   $alg = preg_replace("/3-5F'/","<388>", $alg);   $alg = preg_replace("/3-5F2/", "<385>",$alg);   $alg = preg_replace("/3-5F/", "<386>",$alg);
    $alg = preg_replace("/3-5B2'/","<383>", $alg);   $alg = preg_replace("/3-5B'/","<384>", $alg);   $alg = preg_replace("/3-5B2/", "<381>",$alg);   $alg = preg_replace("/3-5B/", "<382>",$alg);
    $alg = preg_replace("/3-5U2'/","<395>", $alg);   $alg = preg_replace("/3-5U'/","<396>", $alg);   $alg = preg_replace("/3-5U2/", "<393>",$alg);   $alg = preg_replace("/3-5U/", "<394>",$alg);
    $alg = preg_replace("/3-5D2'/","<391>", $alg);   $alg = preg_replace("/3-5D'/","<392>", $alg);   $alg = preg_replace("/3-5D2/", "<389>",$alg);   $alg = preg_replace("/3-5D/", "<390>",$alg);
    ...
    $alg = preg_replace("/4-5BR2'/","<3103>",$alg);   $alg = preg_replace("/4-5BR'/","<3104>",$alg);   $alg = preg_replace("/4-5BR2/","<3101>",$alg);   $alg = preg_replace("/4-5BR/","<3102>",$alg);
    $alg = preg_replace("/4-5FL2'/", "<399>",$alg);   $alg = preg_replace("/4-5FL'/","<3100>",$alg);   $alg = preg_replace("/4-5FL2/", "<397>",$alg);   $alg = preg_replace("/4-5FL/", "<398>",$alg);
    $alg = preg_replace("/4-5BL2'/","<3111>",$alg);   $alg = preg_replace("/4-5BL'/","<3112>",$alg);   $alg = preg_replace("/4-5BL2/","<3109>",$alg);   $alg = preg_replace("/4-5BL/","<3110>",$alg);
    $alg = preg_replace("/4-5FR2'/","<3107>",$alg);   $alg = preg_replace("/4-5FR'/","<3108>",$alg);   $alg = preg_replace("/4-5FR2/","<3105>",$alg);   $alg = preg_replace("/4-5FR/","<3106>",$alg);
    $alg = preg_replace("/4-5DL2'/","<3115>",$alg);   $alg = preg_replace("/4-5DL'/","<3116>",$alg);   $alg = preg_replace("/4-5DL2/","<3113>",$alg);   $alg = preg_replace("/4-5DL/","<3114>",$alg);
    $alg = preg_replace("/4-5DR2'/","<3123>",$alg);   $alg = preg_replace("/4-5DR'/","<3124>",$alg);   $alg = preg_replace("/4-5DR2/","<3121>",$alg);   $alg = preg_replace("/4-5DR/","<3122>",$alg);
    
    $alg = preg_replace("/4-5R2'/","<3119>", $alg);   $alg = preg_replace("/4-5R'/","<3120>", $alg);   $alg = preg_replace("/4-5R2/", "<3117>",$alg);   $alg = preg_replace("/4-5R/", "<3118>",$alg);
    $alg = preg_replace("/4-5L2'/","<3127>", $alg);   $alg = preg_replace("/4-5L'/","<3128>", $alg);   $alg = preg_replace("/4-5L2/", "<3125>",$alg);   $alg = preg_replace("/4-5L/", "<3126>",$alg);
    $alg = preg_replace("/4-5F2'/","<3135>", $alg);   $alg = preg_replace("/4-5F'/","<3136>", $alg);   $alg = preg_replace("/4-5F2/", "<3133>",$alg);   $alg = preg_replace("/4-5F/", "<3134>",$alg);
    $alg = preg_replace("/4-5B2'/","<3131>", $alg);   $alg = preg_replace("/4-5B'/","<3132>", $alg);   $alg = preg_replace("/4-5B2/", "<3129>",$alg);   $alg = preg_replace("/4-5B/", "<3130>",$alg);
    $alg = preg_replace("/4-5U2'/","<3143>", $alg);   $alg = preg_replace("/4-5U'/","<3144>", $alg);   $alg = preg_replace("/4-5U2/", "<3141>",$alg);   $alg = preg_replace("/4-5U/", "<3142>",$alg);
    $alg = preg_replace("/4-5D2'/","<3139>", $alg);   $alg = preg_replace("/4-5D'/","<3140>", $alg);   $alg = preg_replace("/4-5D2/", "<3137>",$alg);   $alg = preg_replace("/4-5D/", "<3138>",$alg);
...
/* xxx XXX xxx */
  ...
  */
  ...
  /* 
  ---------------------------------------------------------------------------------------------------
  * sse_moveCountBTM($alg)
  * 
  * Move count for SSE Algorithms in BTM metric (2x2 Cube).
  * 
  * Parameter: $alg (STRING>: the algorithm.
  * 
  * Return: 
  ---------------------------------------------------------------------------------------------------
  */
  function sse_moveCountBTM($alg) {
    /* --- Arrays --- */
    $btm_count = [
      /* --- [1] Numbered layer, [2] Slice, [3] Trier, [4] Verge, [5] Mid-layer, [6] Wide-layer rwists and [7] Cube rotations --- */
      "N" => "1", // [1] Numbered layer twists
      "S" => "1", // [2] Slice twists
      "T" => "1", // [3] Trier twists
      "V" => "1", // [4] Verge twists
      "M" => "1", // [5] Mid-layer twists
      "W" => "1", // [6] Wide-layer twists
      "C" => "0", // [7] Cube rotations
       
      /* --- [9] Face twists --- */
      "U" => "1", // Up, Up right, Up left
      "D" => "1", // Down, Down right, Down left
      "R" => "1", // Right
      "L" => "1", // Left
      "F" => "1", // Front
      "B" => "1", // Back, Back right, Back left
    ...
    /* --- Algorithm zeilenweise in Array ablegen --- */
    if (preg_match('/\r\n/', $alg) == 1) {      // Wenn String "\r\n" enthält:
      $alg = preg_replace("/\r\n/","\n",$alg); //   Replace "\r\n" with "\n";
    }
    $arrAlg = explode("\n", $alg);
    
    /* --- Algorithm zeilenweise verarbeiten --- */
    $lineCount = 0; // Zähler Zeile
    for ($i = 0; $i < count($arrAlg); $i++) {
      $line = $arrAlg[$i];
      
      /* --- Leerzeichen am Anfang --- */
      $line = preg_replace("'^\s+'",'',$line); // Leerzeichen am Anfang des Strings entfernen
      
      
      /* --- Doppel-Slash (Bemerkung) entfernen --- */
      $pos = strpos($line, "//");              // Erstes Vorkommen von "//" (Bemerkung) ermitteln
      if ($pos === false) {                    // Bei Bemerkung: (Vergleich mit === statt miz ==, da erster String-Offset 0 ist!)
        // not found
      } else {
        $line = substr($line, 0, $pos);        //   Restliche Zeichen nach erstem Vorkommen von "//" abschneiden
        $line .= " ";                          //   Leerzeichen hinzufügen
      }
      
      /* --- Punkt, Mittelstehender Punkt --- */
      $line = str_replace("·",".",$line);      // Mittelstehener Punkt durch Punkt ersetzen!
      ...
      $line = preg_replace("'  *'"," ",$line); // Überflüssige Leerzeichen entfernen
      
      /* --- Leerzeichen am Schluss --- */
      $line = preg_replace("'\s+$'",'',$line); // Leerzeichen am Schluss des Strings entfernen
      
      
      /* --- Zeile hinzufügen --- */
      if ($line != "") {                       // Wenn Zeile nicht leer:
        $lineCount = $lineCount + 1;
        if ($lineCount > 1) {                  //   Bei $lineCount > 1:
          $newAlg .= " ";                      //     Leerzeichen hinzufügen
        }
        $newAlg .= $line;                      //    Inhalt von Zeile hinzufügen
      }
    }
    
    
    /* --- Nicht unterstützte Zeichen --- */
    if (preg_match('/\//', $newAlg) == 1)  {$alg_err = true;} // Algorithm darf nicht "/" enthalten!
    ...
    if ($newAlg == "") {$alg_err = true;}                     // Algorithm darf nicht leer sein!
    
    $tokenCount = 0; // Zähler Token
    if ($alg_err == false) {
      $tokens = explode(' ', $newAlg);                        // Algorithm in Tokens aufsplitten (und in Array speichern)
      $tokenCount = count($tokens);                           // Anzahl Tokens ermitteln.
      
      if ($tokenCount >= 1) {
        foreach ($tokens as $key => $val) {
          if ($btm_count[substr($val,0,1)] != "") {
            $moveCount = $moveCount + $btm_count[substr($val,0,1)];
          } else {
            $alg_err = true;
          }
        }
        if ($alg_err == true) {$moveCount = "";}              // Fehler bei unbekanntem Token!
      }
    } else {
      $moveCount = "";
    }
    
    //$txt_out = "[". $newAlg ."]<br/>[". $moveCount ."]"; // [TEST]
    $txt_out = $moveCount;
    return $txt_out;
  }
?>
