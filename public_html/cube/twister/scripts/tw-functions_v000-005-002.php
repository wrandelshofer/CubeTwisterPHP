<?php 
  ...
  * – Slice twist optimization: Test with One-Pass-Method (Gigaminx) and Two-Pass-Method (Megaminx).
  ***************************************************************************************************
  */
  ...
  */
  function alg3xD_TwizzleToSse($alg) {
    ...
    if ($optSSE == true) {
/* xxx   xxx */
      // ACHTUNG! Vorangestelltes Leerzeichen soll nicht beabsichtigte Ersetzungen verhindern.
      // Dadurch werden aber Tokens am Anfang einer Zeile oder vor einer Klammer nicht erkannt!
    ...
    if ($optSSE == true) {
/* xxx   xxx */
      // ACHTUNG! Benötigt vorangestelltes Leerzeichen (siehe TWIZZLE -> CODE).
...
/* xxx   xxx */
  ...
  */
  function alg5xD_TwizzleToSse($alg) {
    ...
    //<s> /* N3 = M */
    $alg = preg_replace("/3BF2'/","<169>",$alg);   $alg = preg_replace("/3BF'/","<170>",$alg);   $alg = preg_replace("/3BF2/","<171>",$alg);   $alg = preg_replace("/3BF/","<172>",$alg); // BF -> B.
    ...
    $alg = preg_replace("/3B2'/", "<169>",$alg);   $alg = preg_replace("/3B'/", "<170>",$alg);   $alg = preg_replace("/3B2/", "<171>",$alg);   $alg = preg_replace("/3B/", "<172>",$alg);
    $alg = preg_replace("/3U2'/", "<173>",$alg);   $alg = preg_replace("/3U'/", "<174>",$alg);   $alg = preg_replace("/3U2/", "<175>",$alg);   $alg = preg_replace("/3U/", "<176>",$alg);
    $alg = preg_replace("/3D2'/", "<177>",$alg);   $alg = preg_replace("/3D'/", "<178>",$alg);   $alg = preg_replace("/3D2/", "<179>",$alg);   $alg = preg_replace("/3D/", "<180>",$alg);
...
// ACHTUNG! Test ohne vorangestelltes Leerzeichen! Prüfen ob ein zweiter Durchlauf benötigt wird!
