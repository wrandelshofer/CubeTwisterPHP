<?php 
  ...
  require_once($_SERVER['DOCUMENT_ROOT'].'/cube/twister/scripts/tw-functions_v000-001-004.php'); // v0.1.4
  ...
  /* --- Variabeln --- */
  $cgiFile           = "index_v000-002-001.php"; // Name dieses CGIs.
  $cgiVersion        = "0.2.1"; // CGI Version.
  $cgiYear           = "2021";  // CGI Year.
  
  $url_cubing        = "cdn.cubing.net/esm/cubing/twisty";          // URL cubing.js Library
  ...
  if ($in{'puzzle'} != "2xCube" && 
      ...
      $in{'puzzle'} != "5xMegaminx") {$in{'puzzle'} = "3xCube";} // Akzeptierte Werte: 2xCube; 3xCube (Default); 4..7xCube; 3...5xPyraminx; 3...5xMegaminx.
  ...
  if ($in{'puzzle'} == "2xCube") {                    // Bei 2x2 Pocket Cube:
    ...
    if ($in{'notation'} == "twizzle") {               //   Bei TWIZZLE:
      $out{'alg'}  = $in{'alg'};                      //     OUT  = TWIZZLE
      $alg_SSE     = alg3xD_TwizzleToSse($in{'alg'}); //     SSE  = TWIZZLE --> SSE
      $alg_TWIZZLE = $in{'alg'};                      //     TWIZZLE = TWIZZLE
    } else {                                          //   Sonst (SSE):
    ...
    if ($in{'notation'} == "twizzle") {               //   Bei TWIZZLE:
      $out{'alg'}  = $in{'alg'};                      //     OUT  = TWIZZLE
      $alg_SSE     = alg5xD_TwizzleToSse($in{'alg'}); //     SSE  = TWIZZLE --> SSE
      $alg_TWIZZLE = $in{'alg'};                      //     TWIZZLE = TWIZZLE
    } else {                                          //   Sonst (SSE):
