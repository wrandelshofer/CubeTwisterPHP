<?php 
  ...
  require_once($_SERVER['DOCUMENT_ROOT'].'/cube/twister/scripts/tw-functions_v000-005-001.php'); // v0.5.1
  ...
  $useJavaPlayer     = false; // true  = Verwendet für SSE-Link den Java-Applet-Player.
                              // false = Verwendet für SSE-Link den Twister-Player (Default).
  
  
  
  
  /* --- Variabeln --- */
  $cgiFile           = "index_v000-005-001.php"; // Name dieses CGIs.
  $cgiVersion        = "0.5.1"; // CGI Version.
  $cgiYear           = "2022";  // CGI Year.
  
  /* ··································································································· */
  if ($runLatestTwLib == true) {
    //$url_cubing      = "cdn.cubing.net/esm/cubing/twisty";                                      // URL Aktuellste Twisty-Player Version (cubing.net).
    $url_cubing      = "cdn.cubing.net/js/cubing/twisty";                                       // URL Aktuellste Twisty-Player Version (cubing.net).
  } else {
  ...
  $sseLink = "";
  if ($alg_SSE != "") {
    // Get SSE-Link (Java-Applet-Player)
    if ($useJavaPlayer == true) {
      if ($sse_puzzle_param[$in{'puzzle'}] != "") {
        // Get SSE-Link-Algorithm
        $sseAlg = alg_SseToSseLink($alg_SSE); // SSE in SSE-Link-Algorithm konvertieren (Java-Applet-Player)
    ...
    // Get SSE-Link (Twister)
    } else {
      if ($twister_puzzle_param[$in{'puzzle'}] != "") {
        // Get Twister-Link-Algorithm
        $sseAlg = alg_SseToTwisterLink($alg_SSE); // SSE in Twister-Link-Algorithm konvertieren (Twister)
        
        // Get Twister-Link
        if ($in{'puzzle'} != "") {
          $sseLink  = "https://". $url_twister;    // Scheme: HTTPS
          $sseLink .= "?puzzle=". $twister_puzzle_param[$in{'puzzle'}];
          $sseLink .= "&alg=". $sseAlg;
        } else {
          $sseLink = "";
        }
