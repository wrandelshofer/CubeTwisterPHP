import { e as experimentalAppendBlockMove } from '../common/chunk.UPUZR3G6-9b0ab4b1.js';
import { E as EquivalentTransformations, b as EquivalentStates, S as SVG } from '../common/chunk.3US24MG2-2101427f.js';
import { m as modifiedBlockMove } from '../common/chunk.OPVJKBVF-cc213492.js';
import { p as parseAlg } from '../common/chunk.E6ZILKIO-7d465ce8.js';
import { s as stateForBlockMove, a as Invert, C as Combine, I as IdentityTransformation, K as KPuzzle } from '../common/chunk.2VGDWMHM-ce7c26fb.js';
import { _ as __toModule, j as blockMoveToString, B as BlockMove, l as expand, S as Sequence, d as BareBlockMove, m as invert, b as algToString, q as __commonJS, h as TraversalUp, T as TraversalDownUp } from '../common/chunk.RSGKTTJ7-5cf68302.js';
import { _ as __async, p as puzzles } from '../common/chunk.HJK2XRK6-c8fd6423.js';
import { WebGLRenderer, PerspectiveCamera, Vector3, TextureLoader, MeshBasicMaterial, DoubleSide, BackSide, Euler, Matrix4, Quaternion, Object3D, Group, BoxGeometry, Mesh, PlaneGeometry, Geometry, Color, Spherical, Vector2, Triangle, Face3, Scene } from '../three.js';
import '../common/chunk.IZKEXG6K-9f5d428e.js';
...
var ManagedCustomElement = class extends HTMLElementShim {
  constructor(options) {
    ...
    var _a;
    this.shadow = this.attachShadow({mode: (_a = options == null ? void 0 : options.mode) != null ? _a : "closed"});
...
var Twisty3DCanvas2 = class extends ManagedCustomElement {
  constructor(scene, options = {}) {
    ...
    this.scene = scene;
    (_a = this.scene) == null ? void 0 : _a.addRenderTarget(this);
    if (SHOW_STATS) {
    ...
    this.camera.position.copy((_b = options.experimentalCameraPosition) != null ? _b : new Vector3(2, 4, 4));
...
var cubeCameraPosition = new Vector3(3, 4, 5);
...
function replace(l, indices, value) {
...
var appearances4x4x4 = {
  ...
  OLL: {
    orbits: {
      EDGES: {
        pieces: replace(new Array(24).fill(d), [18, 19, 22, 23, 8, 11, 16, 21], o)
      },
      CORNERS: {
  ...
  PLL: {
    orbits: {
      EDGES: {
        pieces: replace(new Array(24).fill(d), [18, 19, 22, 23, 8, 11, 16, 21], p)
      },
      CORNERS: {
  ...
  "experimental-centers-U-D": {
    orbits: {
      ...
      CORNERS: {pieces: new Array(8).fill(i)},
      CENTERS: {
        pieces: replace(new Array(24).fill(i), [18, 19, 22, 23, 3, 6, 13, 20], r)
      }
    }
  },
  "experimental-centers-U-L-D": {
    orbits: {
      ...
      CORNERS: {pieces: new Array(8).fill(i)},
      CENTERS: {
        pieces: replace(new Array(24).fill(i), [18, 19, 22, 23, 3, 6, 13, 20, 9, 15, 17, 21], r)
      }
    }
  },
  "experimental-centers-U-L-B-D": {
    orbits: {
      ...
      CORNERS: {pieces: new Array(8).fill(i)},
      CENTERS: {
        pieces: replace(new Array(24).fill(i), [18, 19, 22, 23, 3, 6, 13, 20, 9, 15, 17, 21, 7, 10, 14, 16], r)
      }
    }
  },
...
var axesInfo = [
  new AxisInfo(new Vector3(0, 1, 0), new Euler(-TAU / 4, 0, 0), 16777215, 14540253),
  new AxisInfo(new Vector3(-1, 0, 0), new Euler(0, -TAU / 4, 0), 16746496, 8930304),
  new AxisInfo(new Vector3(0, 0, 1), new Euler(0, 0, 0), 65280, 34816),
  new AxisInfo(new Vector3(1, 0, 0), new Euler(0, TAU / 4, 0), 16711680, 6684672),
  new AxisInfo(new Vector3(0, 0, -1), new Euler(0, TAU / 2, 0), 255, 136),
  new AxisInfo(new Vector3(0, -1, 0), new Euler(TAU / 4, 0, 0), 16776960, 8947712)
];
var face = {
  U: 0,
  L: 1,
...
var Cube3D = class extends Object3D {
  constructor(def, cursor, scheduleRenderCallback, options = {}) {
    ...
    cursor == null ? void 0 : cursor.addPositionListener(this);
  }
  ...
  setAppearance(appearance2) {
    var _a;
    for (const [orbitName, orbitAppearance] of Object.entries(appearance2.orbits)) {
        ...
        if (pieceAppearance) {
          ...
          for (let faceletIdx = 0; faceletIdx < pieceInfo.length; faceletIdx++) {
            ...
            if (faceletAppearance) {
              ...
              const appearance3 = typeof faceletAppearance === "string" ? faceletAppearance : faceletAppearance == null ? void 0 : faceletAppearance.appearance;
              faceletInfo.facelet.material = axesInfo[faceletInfo.faceIdx].stickerMaterial[appearance3];
              const hintAppearance = typeof faceletAppearance === "string" ? appearance3 : (_a = faceletAppearance.hintAppearance) != null ? _a : appearance3;
...
// src/cubing/twisty/3D/puzzles/appearance.ts
function getFaceletAppearance(appearance2, orbitName, pieceIdx, faceletIdx, hint) {
  var _a;
  const orbitAppearance = appearance2.orbits[orbitName];
  const pieceAppearance = orbitAppearance.pieces[pieceIdx];
  ...
  if (hint) {
    return (_a = faceletAppearance.hintAppearance) != null ? _a : faceletAppearance.appearance;
  }
...
var StickerDef = class {
  constructor(fixedGeo, stickerDat, hintStickers, options) {
    this.faceArray = [];
    this.twistVal = -1;
    this.origColor = new Color(stickerDat.color);
    this.origColorAppearance = new Color(stickerDat.color);
    if (options == null ? void 0 : options.appearance) {
  ...
  setAppearance(faceletMeshAppearance) {
    switch (faceletMeshAppearance) {
      case "regular":
        ...
        break;
      case "dim":
        if (this.origColor.getHex() === 16777215) {
          this.origColorAppearance.setHex(14540253);
        } else {
          this.origColorAppearance.copy(this.origColor);
          this.origColorAppearance.multiplyScalar(0.5);
        }
        break;
      case "oriented":
...
var PG3D = class extends Object3D {
  constructor(cursor, scheduleRenderCallback, definition, pgdat, showFoundation = false, hintStickers = false, params = {}) {
    ...
    for (let si = 0; si < stickers.length; si++) {
      ...
      const stickerdef = new StickerDef(fixedGeo, sticker, hintStickers, options);
  ...
  experimentalSetAppearance(appearance2) {
    this.params.appearance = appearance2;
    for (const orbitName in this.definition.orbits) {
      ...
      for (let pieceIdx = 0; pieceIdx < numPieces; pieceIdx++) {
        for (let faceletIdx = 0; faceletIdx < orientations; faceletIdx++) {
          const faceletAppearance = getFaceletAppearance(appearance2, orbitName, pieceIdx, faceletIdx, false);
      ...
      for (const orbit in this.stickers) {
        ...
        for (let ori = 0; ori < orin; ori++) {
          ...
          for (let i2 = 0; i2 < pieces2.length; i2++) {
            ...
            colormods += pieces2[i2].setColor(pieces[nori][ni].origColorAppearance);
          }
        }
      }
      this.lastPos = pos;
...
var TwistyControlButton = class extends ManagedCustomElement {
  constructor(timeline, timelineCommand, options) {
    ...
    var _a, _b;
    this.fullscreenElement = (_a = options == null ? void 0 : options.fullscreenElement) != null ? _a : null;
    this.visitTwizzleLinkCallback = (_b = options == null ? void 0 : options.visitTwizzleLinkCallback) != null ? _b : null;
    if (!timeline) {
...
var TwistyControlButtonPanel = class extends ManagedCustomElement {
  constructor(timeline, options) {
    ...
    __privateGet(this, _viewerLinkClassListManager).setValue((_a = options == null ? void 0 : options.viewerLink) != null ? _a : "none");
    this.addElement(new TwistyControlButton(timeline, "fullscreen", {
      fullscreenElement: options == null ? void 0 : options.fullscreenElement
    }));
    ...
    this.addElement(new TwistyControlButton(timeline, "twizzle-link", {
      visitTwizzleLinkCallback: options == null ? void 0 : options.viewerLinkCallback
...
var Twisty2DSVG = class extends ManagedCustomElement {
  constructor(cursor, def, svgSource, options) {
    super();
    ...
    this.resetSVG();
    cursor == null ? void 0 : cursor.addPositionListener(this);
    if ((_a = this.options) == null ? void 0 : _a.experimentalStickering) {
  ...
  experimentalSetStickering(stickering) {
    const appearance2 = appearances3x3x3[stickering];
    this.resetSVG(appearance2);
  }
  resetSVG(appearance2) {
    if (this.svg) {
    ...
    this.svg = new SVG(this.definition, this.svgSource, appearance2);
...
var TwistyPlayer2 = class extends ManagedCustomElement {
  ...
  setCursorStartState() {
    var _a;
    if (this.cursor) {
      this.cursor.setStartState((_a = __privateGet(this, _experimentalStartStateOverride)) != null ? _a : this.cursor.algToState(this.cursorStartAlg()));
    }
  }
  cursorStartAlg() {
  ...
  set experimentalStickering(experimentalStickering) {
    if (__privateGet(this, _config).attributes["experimental-stickering"].setValue(experimentalStickering)) {
      ...
      if (this.twisty3D instanceof PG3D) {
        this.twisty3D.experimentalSetAppearance(this.getPG3DAppearance());
      }
  ...
  set experimentalCameraPosition(cameraPosition) {
    ...
    if (this.viewerElems && ["3D", "PG3D"].includes(__privateGet(this, _config).attributes["visualization"].value)) {
      ...
      (_b = this.viewerElems[0]) == null ? void 0 : _b.camera.lookAt(new Vector3(0, 0, 0));
      ...
      (_e = this.viewerElems[1]) == null ? void 0 : _e.camera.lookAt(new Vector3(0, 0, 0));
  ...
  set viewerLink(viewerLinkPage) {
    ...
    if (maybePanel == null ? void 0 : maybePanel.setViewerLink) {
  ...
  get effectiveCameraPosition() {
    var _a;
    return (_a = this.experimentalCameraPosition) != null ? _a : this.defaultCameraPosition;
  }
  ...
  twizzleLink() {
    ...
    if (this.experimentalSetupAlg.nestedUnits.length > 0) {
      url.searchParams.set("experimental-setup-alg", algToString(this.experimentalSetupAlg));
    }
  ...
  updatePuzzleDOM(initial = false) {
    return __async(this, null, function* () {
      ...
      let cursor;
      try {
        cursor = new AlgCursor(this.timeline, def, this.alg, this.cursorStartAlg());
      } catch (e) {
        if (initial) {
          this.experimentalInvalidInitialAlgCallback(this.alg);
        }
        cursor = new AlgCursor(this.timeline, def, new Sequence([]), new Sequence([]));
      }
      this.setCursor(cursor);
      if (initial && this.experimentalSetupAlg.nestedUnits.length === 0 && this.experimentalSetupAnchor !== "end") {
        this.timeline.jumpToEnd();
      }
        ...
        case "experimental-2D-LL":
          {
            ...
            this.setRenderMode2D();
            const svgPromiseFn = this.visualization === "2D" ? puzzleManager.svg : (_a = puzzleManager.llSVG) != null ? _a : puzzleManager.svg;
            const mainViewer = new Twisty2DSVG(cursor, def, yield svgPromiseFn(), options);
            if (!pendingPuzzleUpdate.cancelled) {
        ...
        case "PG3D":
          {
            ...
            if (this.visualization === "3D" && this.puzzle === "3x3x3") {
              ...
              const options = {};
              const appearance2 = this.getPG3DAppearance();
              if (appearance2) {
                options.appearance = appearance2;
              }
              const pg3d = new PG3D(cursor, scene.scheduleRender.bind(scene), def2, stickerDat, (_c = (_b = __privateGet(this, _legacyExperimentalPG3DViewConfig)) == null ? void 0 : _b.showFoundation) != null ? _c : true, (_e = (_d = __privateGet(this, _legacyExperimentalPG3DViewConfig)) == null ? void 0 : _d.hintStickers) != null ? _e : this.hintFacelets === "floating", options);
  ...
  getPG3DAppearance() {
    ...
    if (this.puzzle === "4x4x4") {
      return (_b = appearances4x4x4[(_a = this.experimentalStickering) != null ? _a : "full"]) != null ? _b : appearances4x4x4["full"];
    }
    return null;
  }
  createBackViewer() {
...
var TwistyAlgWrapperElem = class extends HTMLElementShim {
  ...
  flushQueue(direction = Direction.Forwards) {
...
var AlgToDOMTree = class extends TraversalDownUp {
  traverseSequence(sequence, dataDown) {
    ...
    let first = true;
    for (const unit of maybeReverseList(sequence.nestedUnits, dataDown.direction)) {
      if (!first) {
        element.addString(" ");
      }
      first = false;
      moveCount += element.addElem(this.traverse(unit, {
  ...
  traverseGroup(group, dataDown) {
    ...
    element.addString("(");
    moveCount += element.addElem(this.traverse(group.nestedSequence, {
      earliestMoveIndex: dataDown.earliestMoveIndex + moveCount,
      twistyAlgViewer: dataDown.twistyAlgViewer,
      direction
    }));
  ...
  traverseBlockMove(blockMove, dataDown) {
    return {
      moveCount: 1,
      element: new TwistyAlgLeafElem("twisty-alg-blockMove", blockMoveToString(blockMove), dataDown)
    };
  }
  traverseCommutator(commutator, dataDown) {
    ...
    element.addString("[");
    element.flushQueue();
    const direction = updateDirectionByAmount(dataDown.direction, commutator.amount);
    const [first, second] = maybeReverseList([commutator.A, commutator.B], direction);
    moveCount += element.addElem(this.traverse(first, {
      earliestMoveIndex: dataDown.earliestMoveIndex + moveCount,
      twistyAlgViewer: dataDown.twistyAlgViewer,
      direction
    }));
    element.addString(", ");
    moveCount += element.addElem(this.traverse(second, {
      earliestMoveIndex: dataDown.earliestMoveIndex + moveCount,
      twistyAlgViewer: dataDown.twistyAlgViewer,
      direction
    }));
  ...
  traverseConjugate(conjugate, dataDown) {
    ...
    element.addString("[");
    const direction = updateDirectionByAmount(dataDown.direction, conjugate.amount);
    const aLen = element.addElem(this.traverse(conjugate.A, {
      earliestMoveIndex: dataDown.earliestMoveIndex + moveCount,
      twistyAlgViewer: dataDown.twistyAlgViewer,
      direction
    }));
    moveCount += aLen;
    element.addString(": ");
    moveCount += element.addElem(this.traverse(conjugate.B, {
      earliestMoveIndex: dataDown.earliestMoveIndex + moveCount,
      twistyAlgViewer: dataDown.twistyAlgViewer,
      direction
    }));
  ...
  traverseComment(comment, dataDown) {
    return {
      moveCount: 0,
      element: new TwistyAlgLeafElem("twisty-alg-comment", `//${comment.comment}`, dataDown)
    };
  }
};
var algToDOMTreeInstance = new AlgToDOMTree();
var algToDOMTree = algToDOMTreeInstance.traverse.bind(algToDOMTreeInstance);
var _domTree;
var ExperimentalTwistyAlgViewer = class extends HTMLElementShim {
  constructor(options) {
    ...
    if (options == null ? void 0 : options.twistyPlayer) {
      this.setTwistyPlayer(options == null ? void 0 : options.twistyPlayer);
    }
  }
  connectedCallback() {
  }
  setAlg(alg14) {
    __privateSet(this, _domTree, algToDOMTree(alg14, {
      earliestMoveIndex: 0,
  ...
  setTwistyPlayer(twistyPlayer) {
    ...
    twistyPlayer.timeline.addTimestampListener({
      onTimelineTimestampChange: (timestamp) => {
        ...
        (_c = (_b = (_a = this.twistyPlayer) == null ? void 0 : _a.cursor) == null ? void 0 : _b.experimentalIndexFromTimestamp(timestamp)) != null ? _c : null;
      },
      onTimeRangeChange: (_timeRange) => {
      }
    });
  ...
  jumpToIndex(index) {
    ...
    if (this.twistyPlayer && this.twistyPlayer.cursor) {
      const timestamp = ((_a = this.twistyPlayer.cursor.experimentalTimestampFromIndex(index)) != null ? _a : -250) + 250;
      (_b = this.twistyPlayer) == null ? void 0 : _b.timeline.setTimestamp(timestamp);
  ...
  attributeChangedCallback(attributeName, _oldValue, newValue) {
    if (attributeName === "for") {
      ...
      if (!(elem instanceof TwistyPlayer2)) {
...
export { Cube3D, ExperimentalTwistyAlgViewer, KPuzzleWrapper as KSolvePuzzle, PG3D, SimpleAlgIndexer, TimestampLocationType, TreeAlgIndexer, Twisty3DCanvas2 as Twisty3DCanvas, TwistyPlayer2 as TwistyPlayer, experimentalSetShareAllNewRenderers, experimentalShowRenderStats };
